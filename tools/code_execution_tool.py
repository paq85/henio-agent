...
Lets the LLM write a Python script that calls Henio tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `henio_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `henio_tools.py` with file-based RPC stubs
...
def generate_henio_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the henio_tools.py stub module.
...
"""Auto-generated Henio tools RPC stubs."""
...
def _connect():
    ...
    if _sock is None:
        ...
        _sock.connect(os.environ["HENIO_RPC_SOCKET"])
...
"""Auto-generated Henio tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, time

_RPC_DIR = os.environ.get("HENIO_RPC_DIR") or os.path.join(tempfile.gettempdir(), "henio_rpc")
_seq = 0
''' + _COMMON_HELPERS + '''\
...
def _execute_remote(
    ...
    The script and the generated henio_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/henio_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_henio_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/henio_tools.py", tools_src)
        ...
        env_prefix = (
            f"HENIO_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("HENIO_TIMEZONE", "").strip()
        if tz:
            env_prefix += f" TZ={tz}"
...
def execute_code(
    ...
    # --- Set up temp directory with henio_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="henio_sandbox_")
    # Use /tmp on macOS to avoid the long /var/folders/... path that pushes
    # Unix domain socket paths past the 104-byte macOS AF_UNIX limit.
    # On Linux, tempfile.gettempdir() already returns /tmp.
    _sock_tmpdir = "/tmp" if sys.platform == "darwin" else tempfile.gettempdir()
    sock_path = os.path.join(_sock_tmpdir, f"henio_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated henio_tools module
        # sandbox_tools is already the correct set (intersection with session
        # tools, or SANDBOX_ALLOWED_TOOLS as fallback — see lines above).
        tools_src = generate_henio_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "henio_tools.py"), "w") as f:
            f.write(tools_src)
        ...
        for k, v in os.environ.items():
            ...
            if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
                child_env[k] = v
        child_env["HENIO_RPC_SOCKET"] = sock_path
        child_env["PYTHONDONTWRITEBYTECODE"] = "1"
        # Ensure the henio-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.
        _henio_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        child_env["PYTHONPATH"] = _henio_root + (os.pathsep + _existing_pp if _existing_pp else "")
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.
        _tz_name = os.getenv("HENIO_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name

        # Per-profile HOME isolation: redirect system tool configs into
        # {HENIO_HOME}/home/ when that directory exists.
        from henio_constants import get_subprocess_home
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.henio/.env')).
...
def build_execute_code_schema(enabled_sandbox_tools: set = None) -> dict:
    ...
    When tools are disabled via ``henio tools`` (e.g. web is turned off),
    ...
    description = (
        "Run a Python script that can call Henio tools programmatically. "
        ...
        f"Available via `from henio_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into henio_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from henio_tools import {import_str}` "
