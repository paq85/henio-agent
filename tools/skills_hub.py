#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Henio Skills Hub.
...
Used by henio_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            henio_meta = metadata.get("henio", {})
            if isinstance(henio_meta, dict):
                tags = henio_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by the Henio project) but not activated
    by default — they don't appear in the system prompt and aren't copied to
    ~/.henio/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                henio_meta = meta_block.get("henio", {})
                if isinstance(henio_meta, dict):
                    tags = henio_meta.get("tags", [])
