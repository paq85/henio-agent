...
# (henio-* prefixed), and scenario toolsets.
...
def _build_child_agent(
    ...
    model on OpenRouter while the parent runs on Henio Portal).
    """
    ...
    # Extract parent's API key so subagents inherit auth (e.g. Henio Portal).
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'henio auth'."
        )
...
def _load_config() -> dict:
    ...
    to the persistent config (henio_cli/config.py load_config()) so that
