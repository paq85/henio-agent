...
from tools.environments.local import _HENIO_PROVIDER_ENV_BLOCKLIST
...
def _load_henio_env_vars() -> dict[str, str]:
    """Load ~/.henio/.env values without failing Docker command execution."""
    try:
        from henio_cli.config import load_env
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.henio/sandboxes/). Non-persistent
        ...
        container_name = f"henio-{uuid.uuid4().hex[:8]}"
        run_cmd = [
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Henio secret blocklist. Only implicit passthrough
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _HENIO_PROVIDER_ENV_BLOCKLIST)
        henio_env = _load_henio_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if value is None:
                value = henio_env.get(key)
