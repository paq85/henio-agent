...
it is automatically downloaded from GitHub releases to $HENIO_HOME/bin/tirith.
...
def _get_henio_home() -> str:
    """Return the Henio home directory, respecting HENIO_HOME env var."""
    return str(get_henio_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_henio_home(), ".tirith-install-failed")
...
def _henio_bin_dir() -> str:
    """Return $HENIO_HOME/bin, creating it if needed."""
    d = os.path.join(_get_henio_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $HENIO_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_henio_bin_dir(), "tirith")
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $HENIO_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $HENIO_HOME/bin/tirith
    ...
    henio_bin = os.path.join(_henio_bin_dir(), "tirith")
    if os.path.isfile(henio_bin) and os.access(henio_bin, os.X_OK):
        _resolved_path = henio_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return henio_bin
