...
This module provides the glue:
    - ``HenioTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Henio Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses HENIO_HOME so each profile gets its own OAuth tokens.
    Layout: ``HENIO_HOME/mcp-tokens/``
    """
    try:
        from henio_constants import get_henio_home
        base = Path(get_henio_home())
    except ImportError:
        base = Path(os.environ.get("HENIO_HOME", str(Path.home() / ".henio")))
...
# ---------------------------------------------------------------------------
# HenioTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class HenioTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        HENIO_HOME/mcp-tokens/<server_name>.json         -- tokens
        HENIO_HOME/mcp-tokens/<server_name>.client.json   -- client info
    """
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Henio.</p></body></html>"
            ) if code else (
...
def remove_oauth_tokens(server_name: str) -> None:
    ...
    storage = HenioTokenStorage(server_name)
    storage.remove()
...
def build_oauth_auth(
    ...
    # --- Client metadata ---
    client_name = cfg.get("client_name", "Henio Agent")
    scope = cfg.get("scope")
