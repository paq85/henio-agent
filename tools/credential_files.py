...
def _resolve_henio_home() -> Path:
    from henio_constants import get_henio_home
    return get_henio_home()


def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.henio",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``HENIO_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside HENIO_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    henio_home = _resolve_henio_home()

    # Reject absolute paths — they bypass the HENIO_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to HENIO_HOME)",
            relative_path,
        )
        return False

    host_path = henio_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape HENIO_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, henio_home)
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.henio/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="henio-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching henio_constants.get_henio_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_henio_dir()`` for backward compatibility with old directory layouts.
    """
    from henio_constants import get_henio_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_henio_dir(new_subpath, old_name)
        if host_dir.is_dir():
