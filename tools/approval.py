...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("HENIO_SESSION_KEY", default)

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $HENIO_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_HENIO_ENV_PATH = (
    r'(?:~\/\.henio/|'
    r'(?:\$home|\$\{home\})/\.henio/|'
    r'(?:\$henio_home|\$\{henio_home\})/)'
    r'\.env\b'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart henio-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart henio-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(henio|gateway|cli\.py)\b', "kill henio/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill henio` but not
    # `kill -9 $(pgrep -f henio)` because the substitution is opaque
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    os.environ["HENIO_SPINNER_PAUSE"] = "1"
    try:
        while True:
            print()
        ...
        if "HENIO_SPINNER_PAUSE" in os.environ:
            del os.environ["HENIO_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if os.getenv("HENIO_YOLO_MODE") or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    is_cli = os.getenv("HENIO_INTERACTIVE")
    is_gateway = os.getenv("HENIO_GATEWAY_SESSION")

    if not is_cli and not is_gateway:
        return {"approved": True, "message": None}

    if is_gateway or os.getenv("HENIO_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if os.getenv("HENIO_YOLO_MODE") or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = os.getenv("HENIO_INTERACTIVE")
    is_gateway = os.getenv("HENIO_GATEWAY_SESSION")
    is_ask = os.getenv("HENIO_EXEC_ASK")
    ...
    try:
        ...
        # Warm tirith availability in the background so interactive approval
        # flows don't block on a first-run binary download before the user even
        # sees the prompt. If tirith is still unavailable, the subsequent scan
        # will fail open quickly via its normal OSError handling.
