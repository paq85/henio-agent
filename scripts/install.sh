...
# Configuration
REPO_URL_SSH="git@github.com:paq85/henio-agent.git"
REPO_URL_HTTPS="https://github.com/paq85/henio-agent.git"
HENIO_HOME="$HOME/.henio"
INSTALL_DIR="${HENIO_INSTALL_DIR:-$HENIO_HOME/henio-agent}"
PYTHON_VERSION="3.11"
NODE_VERSION="22"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --dir PATH     Installation directory (default: ~/.henio/henio-agent)"
...
get_henio_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/henio" ]; then
        echo "$link_dir/henio"
    else
        echo "henio"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/paq85/henio-agent/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
check_node() {
    ...
    if [ -x "$HENIO_HOME/node/bin/node" ]; then
        export PATH="$HENIO_HOME/node/bin:$PATH"
        local found_ver=$("$HENIO_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (Henio-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.henio/node/..."
    ...
    # Place into ~/.henio/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$HENIO_HOME/node"
    mkdir -p "$HENIO_HOME"
    mv "$extracted_dir" "$HENIO_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$HENIO_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$HENIO_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$HENIO_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$HENIO_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$HENIO_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.henio/node/"
    HAS_NODE=true
}
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Henio Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="henio-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Henio behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up henio command..."

    if [ "$USE_VENV" = true ]; then
        HENIO_BIN="$INSTALL_DIR/venv/bin/henio"
    else
        HENIO_BIN="$(which henio 2>/dev/null || echo "")"
        if [ -z "$HENIO_BIN" ]; then
            log_warn "henio not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$HENIO_BIN" ]; then
        log_warn "henio entry point not found at $HENIO_BIN"
    ...
    # Create a user-facing shim for the henio command.
    mkdir -p "$command_link_dir"
    ln -sf "$HENIO_BIN" "$command_link_dir/henio"
    log_success "Symlinked henio → $command_link_display_dir/henio"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "henio command ready"
        return 0
    fi
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Henio Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
    ...
    # Export for current session so henio works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.henio directory structure (config at top level, code in subdir)
    mkdir -p "$HENIO_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills,whatsapp/session}

    # Create .env at ~/.henio/.env (top level, easy to find)
    if [ ! -f "$HENIO_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$HENIO_HOME/.env"
            log_success "Created ~/.henio/.env from template"
        else
            touch "$HENIO_HOME/.env"
            log_success "Created ~/.henio/.env"
        fi
    else
        log_info "~/.henio/.env already exists, keeping it"
    fi

    # Create config.yaml at ~/.henio/config.yaml (top level, easy to find)
    if [ ! -f "$HENIO_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$HENIO_HOME/config.yaml"
            log_success "Created ~/.henio/config.yaml from template"
        fi
    else
        log_info "~/.henio/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$HENIO_HOME/SOUL.md" ]; then
        cat > "$HENIO_HOME/SOUL.md" << 'SOUL_EOF'
# Henio Agent Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how Henio communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.henio/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.henio/"

    # Seed bundled skills into ~/.henio/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.henio/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.henio/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$HENIO_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$HENIO_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.henio/skills/"
        fi
    fi
}
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        case "$DISTRO" in
            ubuntu|debian|raspbian|pop|linuxmint|elementary|zorin|kali|parrot)
                ...
                log_info "This is standard Playwright setup — Henio itself does not require root access."
...
run_setup_wizard() {
    ...
    if ! [ -e /dev/tty ]; then
        log_info "Setup wizard skipped (no terminal available). Run 'henio setup' after install."
        return 0
    fi
    ...
    # Run henio setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m henio_cli.main setup < /dev/tty
    else
        python -m henio_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$HENIO_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Henio to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$HENIO_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'henio whatsapp' to pair via QR code..."
            echo ""
            read -p "Pair WhatsApp now? [Y/n] " -n 1 -r
            echo
            if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
                HENIO_CMD="$(get_henio_command_path)"
                $HENIO_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'henio whatsapp' to pair."
        fi
    fi

    if ! [ -e /dev/tty ]; then
        log_info "Gateway setup skipped (no terminal available). Run 'henio gateway install' later."
        return 0
    fi
    ...
    if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $HENIO_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $HENIO_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: henio gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: henio gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $HENIO_CMD gateway > "$HENIO_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.henio/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: henio gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: henio gateway"
    fi
}
...
print_success() {
    ...
    # Show file locations
    echo -e "${CYAN}${BOLD}📁 Your files (all in ~/.henio/):${NC}"
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    ~/.henio/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  ~/.henio/.env"
    echo -e "   ${YELLOW}Data:${NC}      ~/.henio/cron/, sessions/, logs/"
    echo -e "   ${YELLOW}Code:${NC}      ~/.henio/henio-agent/"
    echo ""
    ...
    echo ""
    echo -e "   ${GREEN}henio${NC}              Start chatting"
    echo -e "   ${GREEN}henio setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}henio config${NC}       View/edit configuration"
    echo -e "   ${GREEN}henio config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}henio gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}henio update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'henio' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'henio' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
