...
VERSION_FILE = REPO_ROOT / "henio_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "henio@nousresearch.com": "NousResearch",
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/paq85/henio-agent",
    ...
    lines.append(f"# Henio Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Henio Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Henio Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Henio Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Henio Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Henio Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Henio Agent v{new_version} ({calver_date})' "
