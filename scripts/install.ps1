# ============================================================================
# Henio Agent Installer for Windows
# ============================================================================
# Installation script for Windows (PowerShell).
# Uses uv for fast Python provisioning and package management.
#
# Usage:
#   irm https://raw.githubusercontent.com/paq85/henio-agent/main/scripts/install.ps1 | iex
#
...
param(
    ...
    [switch]$SkipSetup,
    [string]$Branch = "main",
    [string]$HenioHome = "$env:LOCALAPPDATA\henio",
    [string]$InstallDir = "$env:LOCALAPPDATA\henio\henio-agent"
)
...
$RepoUrlSsh = "git@github.com:paq85/henio-agent.git"
$RepoUrlHttps = "https://github.com/paq85/henio-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "│             ⚕ Henio Agent Installer                    │" -ForegroundColor Magenta
    Write-Host "├─────────────────────────────────────────────────────────┤" -ForegroundColor Magenta
    Write-Host "│  An open source AI agent framework.                     │" -ForegroundColor Magenta
    Write-Host "└─────────────────────────────────────────────────────────┘" -ForegroundColor Magenta
    Write-Host ""
}
...
function Test-Node {
    ...
    $managedNode = "$HenioHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$HenioHome\node;$env:Path"
        Write-Success "Node.js $version found (Henio-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # Fallback: download binary zip to ~/.henio/node/
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\henio-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$HenioHome\node") { Remove-Item -Recurse -Force "$HenioHome\node" }
                Move-Item $extractedDir.FullName "$HenioHome\node"
                $env:Path = "$HenioHome\node;$env:Path"

                $version = & "$HenioHome\node\node.exe" --version
                Write-Success "Node.js $version installed to ~/.henio/node/"
                $script:HasNode = $true
...
function Install-Repository {
    ...
    if (Test-Path $InstallDir) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                $zipUrl = "https://github.com/paq85/henio-agent/archive/refs/heads/$Branch.zip"
                $zipPath = "$env:TEMP\henio-agent-$Branch.zip"
                $extractPath = "$env:TEMP\henio-agent-extract"
...
function Set-PathVariable {
    Write-Info "Setting up henio command..."
    
    if ($NoVenv) {
        $henioBin = "$InstallDir"
    } else {
        $henioBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so henio is globally available
    # On Windows, the henio.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$henioBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$henioBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $henioBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set HENIO_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\henio instead
    # of the Unix default ~/.henio
    $currentHenioHome = [Environment]::GetEnvironmentVariable("HENIO_HOME", "User")
    if (-not $currentHenioHome -or $currentHenioHome -ne $HenioHome) {
        [Environment]::SetEnvironmentVariable("HENIO_HOME", $HenioHome, "User")
        Write-Success "Set HENIO_HOME=$HenioHome"
    }
    $env:HENIO_HOME = $HenioHome
    
    # Update current session
    $env:Path = "$henioBin;$env:Path"
    
    Write-Success "henio command ready"
}

function Copy-ConfigTemplates {
    Write-Info "Setting up configuration files..."
    
    # Create ~/.henio directory structure
    New-Item -ItemType Directory -Force -Path "$HenioHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HenioHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HenioHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HenioHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HenioHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HenioHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HenioHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HenioHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HenioHome\skills" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HenioHome\whatsapp\session" | Out-Null
    
    # Create .env
    $envPath = "$HenioHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.henio/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.henio/.env"
        }
    } else {
        Write-Info "~/.henio/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$HenioHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.henio/config.yaml from template"
        }
    } else {
        Write-Info "~/.henio/config.yaml already exists, keeping it"
    }
    
    # Create SOUL.md if it doesn't exist (global persona file)
    $soulPath = "$HenioHome\SOUL.md"
...
"@ | Set-Content -Path $soulPath -Encoding UTF8
        Write-Success "Created ~/.henio/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.henio/"
    
    # Seed bundled skills into ~/.henio/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.henio/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.henio/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$HenioHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.henio/skills/"
            }
        }
    }
}
...
function Invoke-SetupWizard {
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m henio_cli.main setup
    } else {
        python -m henio_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $henioCmd = "$InstallDir\venv\Scripts\henio.exe"
    if (-not (Test-Path $henioCmd)) {
        $henioCmd = "henio"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$HenioHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'henio whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$HenioHome\logs\gateway.log"
            Start-Process -FilePath $henioCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$HenioHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: henio gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: henio gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$HenioHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$HenioHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$HenioHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$HenioHome\henio-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   henio              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   henio setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   henio config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   henio config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   henio gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   henio update       " -NoNewline -ForegroundColor Green
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/paq85/henio-agent/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
