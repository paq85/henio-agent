"""
Status command for henio CLI.

Shows the status of all Henio Agent components.
"""
...
from henio_cli.config import get_env_path, get_env_value, get_henio_home, load_config
from henio_cli.models import provider_label
from henio_cli.nous_subscription import get_nous_subscription_features
from henio_cli.runtime_provider import resolve_requested_provider
...
def show_status(args):
    """Show status of all Henio Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)
    
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Henio Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        from henio_cli.auth import get_nous_auth_status, get_codex_auth_status, get_qwen_auth_status
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: henio model)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: henio model)"
    ...
    if not terminal_env:
        ...
        # (henio status doesn't go through cli.py's config loading)
        try:
            _cfg = load_config()
    ...
    if _is_termux():
        try:
            from henio_cli.gateway import find_gateway_pids
        ...
        if gateway_pids:
            ...
            print("  Start with:   henio gateway")
        ...
        from henio_cli.gateway import get_launchd_label
        try:
            result = subprocess.run(
    ...
    jobs_file = get_henio_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_henio_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'henio doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'henio setup' to configure", Colors.DIM))
    print()
