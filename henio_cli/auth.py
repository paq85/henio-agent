...
from henio_cli.config import get_henio_home, get_config_path, read_raw_config
...
DEFAULT_COPILOT_ACP_PROBE_TIMEOUT_SECONDS = 15.0
...
def get_anthropic_key() -> str:
    ...
    """
    from henio_cli.config import get_env_value
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from henio_cli.copilot_auth import resolve_copilot_token
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `henio model` to re-authenticate."
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("HENIO_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.henio/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    return get_henio_home() / "auth.json"
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in Henio.
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `henio logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from henio_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""

        lines = ["Config issue detected — run 'henio doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'henio model' for available providers, or run 'henio doctor' to diagnose config issues."
    ...
    raise AuthError(
        "No inference provider configured. Run 'henio model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.henio/.env.",
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("HENIO_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.henio/auth.json (not ~/.codex/)
#
# Henio maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Henio auth store (~/.henio/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `henio auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `henio auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `henio auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `henio auth` to re-authenticate.",
            provider="openai-codex",
...
def _write_codex_cli_tokens(
    ...
    *,
    ...
    When Henio refreshes a token it consumes the old refresh_token; if we
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to Henio auth store (~/.henio/auth.json)."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Henio auth state."""
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `henio auth` to re-authenticate."
            )
            relogin_required = True
        raise AuthError(
            message,
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Henio auth store automatically.
    """
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Henio's own Codex token store."""
    try:
        ...
        if cli_tokens:
            logger.info("Migrating Codex credentials from ~/.codex/ to Henio auth store")
            print("⚠️  Migrating Codex credentials to Henio's own auth store.")
            print("   This avoids conflicts with Codex CLI and VS Code.")
            print("   Run `henio auth` to create a fully independent session.\n")
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("HENIO_CODEX_REFRESH_TIMEOUT_SECONDS", "20"))

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other Henio processes
    ...
    base_url = (
        os.getenv("HENIO_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    *,
    ...
    if effective_ca:
        ...
        if not os.path.isfile(ca_path):
            import logging
            logging.getLogger("henio.auth").warning(
    ...
    *,
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Henio models — they're not reliable for agentic tool-calling
            if "henio" in mid.lower():
                continue
    ...
    *,
    ...
    with _auth_store_lock():
        ...
        portal_base_url = (
            ...
            or os.getenv("HENIO_PORTAL_BASE_URL")
...
def get_nous_auth_status() -> Dict[str, Any]:
    """Status snapshot for `henio status` output."""
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `henio auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `henio auth` and
    # `henio model` store device_code tokens.
    try:
...
def _resolve_external_process_provider_runtime(provider_id: str) -> Dict[str, Any]:
    pconfig = PROVIDER_REGISTRY.get(provider_id)
    ...
    raw_args = os.getenv("HENIO_COPILOT_ACP_ARGS", "").strip()
...
def _probe_external_process_provider(runtime: Dict[str, Any]) -> Dict[str, Any]:
    provider_id = str(runtime.get("provider") or "")
    base_url = str(runtime.get("base_url") or "")
    resolved_command = str(runtime.get("resolved_command") or runtime.get("command") or "")
    ...
    if not resolved_command:
        return {
            "ok": False,
            "provider": provider_id,
            "command": runtime.get("command") or "copilot",
            "args": args,
            "session_id": "",
            "error": "",
            "error_code": "missing_copilot_cli",
            "stderr_tail": "",
            "probe_skipped": True,
        }
    ...
    timeout_seconds = float(
        os.getenv(
            "HENIO_COPILOT_ACP_PROBE_TIMEOUT_SECONDS",
            str(DEFAULT_COPILOT_ACP_PROBE_TIMEOUT_SECONDS),
        )
    )
    return probe_copilot_acp(
        command=resolved_command,
        args=args,
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    runtime = _resolve_external_process_provider_runtime(provider_id)
    configured = bool(runtime.get("resolved_command") or runtime.get("base_url", "").startswith("acp+tcp://"))
    status = {
        ...
        "base_url": runtime.get("base_url") or pconfig.inference_base_url,
        "logged_in": False,
        "ready": False,
        "error": "",
        "error_code": "",
    ...
    if not configured:
        status["error"] = (
            f"Could not find the Copilot CLI command '{status['command']}'. "
            "Install GitHub Copilot CLI or set HENIO_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH."
        )
        status["error_code"] = "missing_copilot_cli"
        return status

    probe = _probe_external_process_provider(runtime)
    status["logged_in"] = bool(probe.get("ok"))
    status["ready"] = bool(probe.get("ok"))
    status["error"] = str(probe.get("error") or "")
    status["error_code"] = str(probe.get("error_code") or "")
    status["stderr_tail"] = str(probe.get("stderr_tail") or "")
    if probe.get("session_id"):
        status["session_id"] = probe.get("session_id")
    if probe.get("probe_skipped"):
        status["probe_skipped"] = True
    return status
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    base_url = str(status.get("base_url") or pconfig.inference_base_url)
    command = str(status.get("command") or "copilot")
    resolved_command = status.get("resolved_command")
    args = list(status.get("args") or [])

    if not status.get("configured") and not base_url.startswith("acp+tcp://"):
        raise AuthError(
    ...
    if not status.get("logged_in") and not base_url.startswith("acp+tcp://"):
        detail = str(status.get("error") or "Copilot ACP startup failed.")
        stderr_tail = str(status.get("stderr_tail") or "")
        if stderr_tail and stderr_tail not in detail:
            detail = f"{detail}\nCLI stderr:\n{stderr_tail}"
        raise AuthError(
            (
                f"Copilot CLI was found at '{resolved_command or command}', but ACP startup failed. "
                f"{detail}\nRun `copilot login` and retry."
            ),
            provider=provider_id,
            code=str(status.get("error_code") or "copilot_acp_probe_failed"),
        )
...
def _prompt_model_selection(
    ...
    """
    from henio_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from henio_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'henio model' or 'henio setup' instead."""
    print("The 'henio login' command has been removed.")
    print("Use 'henio auth' to manage credentials,")
    print("'henio model' to select a provider, or 'henio setup' for full setup.")
    raise SystemExit(0)


def _login_openai_codex(args, pconfig: ProviderConfig) -> None:
    """OpenAI Codex login via device code flow. Tokens stored in ~/.henio/auth.json."""

    # Check for existing Henio-owned credentials
    try:
        ...
        if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
            print("Existing Codex credentials found in Henio auth store.")
            try:
    ...
    if cli_tokens:
        ...
        print("Henio will create its own session to avoid conflicts with Codex CLI / VS Code.")
        try:
        ...
        if do_import in ("y", "yes"):
            _save_codex_tokens(cli_tokens)
            base_url = os.getenv("HENIO_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
            config_path = _update_config_for_provider("openai-codex", base_url)
            print()
            print("Credentials imported. Note: if Codex CLI refreshes its token,")
            print("Henio will keep working independently with its own session.")
            print(f"  Config updated: {config_path} (model.provider=openai-codex)")
            return

    # Run a fresh device code flow — Henio gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Henio creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to Henio auth store
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting Henio login via {pconfig.name}...")
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target):
        ...
        if os.getenv("OPENROUTER_API_KEY"):
            print("Henio will use OpenRouter for inference.")
        else:
            print("Run `henio model` or configure an API key to use Henio.")
    else:
