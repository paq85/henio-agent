"""
Profile management for multiple isolated Henio instances.

Each profile is a fully independent HENIO_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.henio/profiles/<name>/`` by default.

The "default" profile is ``~/.henio`` itself — backward compatible,
zero migration needed.

Usage::

    henio profile create coder          # fresh profile + bundled skills
    henio profile create coder --clone  # also copy config, .env, SOUL.md
    henio profile create coder --clone-all  # full copy of source profile
    coder chat                          # use via wrapper alias
    henio -p coder chat                 # or via flag
    henio profile use coder             # set as sticky default
    henio profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See henio_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# Directories/files to exclude when exporting the default (~/.henio) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "henio-agent",          # repo checkout (multi-GB)
    "henio-agent",         # repo checkout (multi-GB)
...
_RESERVED_NAMES = frozenset({
    "henio", "henio", "default", "test", "tmp", "root", "sudo",
})

# Henio subcommands that cannot be used as profile names/aliases
_HENIO_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the henio root, NOT to the current HENIO_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where HENIO_HOME points outside
    ``~/.henio``, profiles live under ``HENIO_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_henio_home() / "profiles"


def _get_default_henio_home() -> Path:
    """Return the default (pre-profile) HENIO_HOME path.

    In standard deployments this is ``~/.henio``.
    In Docker/custom deployments where HENIO_HOME is outside ``~/.henio``
    (e.g. ``/opt/data``), returns HENIO_HOME directly.
    """
    from henio_constants import get_default_henio_root
    return get_default_henio_root()


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_henio_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name == "default":
        return  # special alias for ~/.henio
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its HENIO_HOME directory."""
    if name == "default":
        return _get_default_henio_home()
    return _get_profiles_root() / name


def _is_profile_wrapper_content(content: str) -> bool:
    return "henio -p" in content or "henio -p" in content


def _build_wrapper_script(profile_name: str) -> str:
    return (
        "#!/bin/sh\n"
        "if command -v henio >/dev/null 2>&1; then\n"
        f"    exec henio -p {profile_name} \"$@\"\n"
        "fi\n"
        f"exec henio -p {profile_name} \"$@\"\n"
    )
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, henio subcommands, existing binaries in PATH.
    """
    if name in _RESERVED_NAMES:
        return f"'{name}' is a reserved name"
    if name in _HENIO_SUBCOMMANDS:
        return f"'{name}' conflicts with a henio subcommand"
        ...
        if result.returncode == 0:
            ...
            if existing_path == str(wrapper_dir / name):
                try:
                    ...
                    if _is_profile_wrapper_content(content):
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_henio_home()
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.henio, or ~/.henio on legacy installs)."
        )
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches HENIO_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "HENIO_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.henio, or ~/.henio on legacy installs).\n"
            "To remove everything, use: henio uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set HENIO_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("HENIO_HOME")
    try:
        os.environ["HENIO_HOME"] = str(profile_dir)
        from henio_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["HENIO_HOME"] = old_home
        elif "HENIO_HOME" in os.environ:
            del os.environ["HENIO_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.henio/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: henio profile create {name}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from HENIO_HOME.

    Returns ``"default"`` if HENIO_HOME is not set or points to ``~/.henio``.
    Returns the profile name if HENIO_HOME points into ``~/.henio/profiles/<name>``.
    Returns ``"custom"`` if HENIO_HOME is set to an unrecognized path.
    """
    from henio_constants import get_henio_home
    henio_home = get_henio_home()
    resolved = henio_home.resolve()

    default_resolved = _get_default_henio_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.henio itself — its parent is ~/ and its
        # directory name is ".henio", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: henio profile import <archive> --name <name>"
        )

    # Archives exported from the default profile have "default/" as top-level
    # dir.  Importing as "default" would target ~/.henio itself — disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.henio, or ~/.henio on legacy installs). "
            "Specify a different name: henio profile import <archive> --name <name>"
        )
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for henio profile names."""
    return '''# Henio Agent profile completion
# Add to ~/.bashrc: eval "$(henio completion bash)"

_henio_profiles() {
    local profiles_dir="$HOME/.henio/profiles"
    if [ ! -d "$profiles_dir" ] && [ -d "$HOME/.henio/profiles" ]; then
        profiles_dir="$HOME/.henio/profiles"
    fi
...
_henio_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_henio_profiles)" -- "$cur"))
        return
    fi
...
complete -F _henio_completion henio
complete -F _henio_completion henio
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for henio profile names."""
    return '''#compdef henio henio
# Henio Agent profile completion
# Add to ~/.zshrc: eval "$(henio completion zsh)"
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a HENIO_HOME path string.

    Called early in the CLI entry point, before any henio modules
    are imported, to set the HENIO_HOME environment variable.
    """
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: henio profile create {profile_name}"
        )
