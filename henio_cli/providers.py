"""
Single source of truth for provider identity in Henio Agent.
...
2. **Henio overlays** — transport type, auth patterns, aggregator flags,
...
# -- Henio overlay ----------------------------------------------------------
# Henio-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class HenioOverlay:
    """Henio-specific provider metadata layered on top of models.dev."""
...
HENIO_OVERLAYS: Dict[str, HenioOverlay] = {
    "openrouter": HenioOverlay(
    ...
    "openai-codex": HenioOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "qwen-oauth": HenioOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="HENIO_QWEN_BASE_URL",
    ),
    "copilot-acp": HenioOverlay(
        transport="openai_chat",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": HenioOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": HenioOverlay(
    ...
    "kimi-for-coding": HenioOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "minimax": HenioOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-cn": HenioOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": HenioOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": HenioOverlay(
        transport="openai_chat",
    ...
    "opencode": HenioOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": HenioOverlay(
        transport="openai_chat",
        is_aggregator=True,
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "henio", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Henio overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Henio overlay
    ...
    overlay = HENIO_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + henio extra
    ...
    if overlay is not None:
        # Henio-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
