...
from henio_cli.auth import get_nous_auth_status
from henio_cli.config import get_env_value, load_config
...
def apply_nous_provider_defaults(config: Dict[str, object]) -> set[str]:
    """Apply provider-level Nous defaults shared by `henio setup` and `henio model`."""
    if not managed_nous_tools_enabled():
        return set()
