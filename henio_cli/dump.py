"""
Dump command for henio CLI.

Outputs a compact, plain-text summary of the user's Henio setup
...
from henio_cli.config import get_henio_home, get_env_path, get_project_root, load_config
...
def _count_skills(henio_home: Path) -> int:
...
def _cron_summary(henio_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = henio_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"henio_home:      {display_henio_home()}")
    ...
    toolsets = config.get("toolsets", ["henio-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(henio_home)}")
    lines.append(f"  skills:             {_count_skills(henio_home)}")
