"""henio claw — OpenClaw migration commands.

Usage:
    henio claw migrate               # Preview then migrate (always shows preview first)
    henio claw migrate --dry-run     # Preview only, no changes
    henio claw migrate --yes         # Skip confirmation prompt
    henio claw migrate --preset full --overwrite   # Full migration, overwrite conflicts
    henio claw cleanup               # Archive leftover OpenClaw directories
    henio claw cleanup --dry-run     # Preview what would be archived
"""
...
from henio_cli.config import get_henio_home, get_config_path, load_config, save_config
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a Henio gateway is running with connected platforms.
    ...
    print()
    print_error(
        "Henio gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
    ...
    print_info("Recommendation: stop the gateway first with 'henio gateway stop'.")
    print()
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_henio.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_henio", script_path)
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print("Usage: henio claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to Henio")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'henio claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → Henio migration."""
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: henio claw migrate --source /path/to/.openclaw")
        return
    ...
    print_info(f"Target:      {henio_home}")
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: henio claw migrate --yes")
            return
...
def _cmd_cleanup(args):
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: henio claw cleanup --yes")
        else:
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  henio claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  henio claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  henio config set OPENROUTER_API_KEY sk-or-v1-...")
