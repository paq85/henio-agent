...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_henio_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:henio_pkce",
    ...
    raise SystemExit(f"`henio auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    if removed.source.startswith("env:"):
        ...
        if env_var:
            from henio_cli.config import remove_env_value
            cleared = remove_env_value(env_var)
            if cleared:
                print(f"Cleared {env_var} from .env")

    # If this was a singleton-seeded credential (OAuth device_code, henio_pkce),
    ...
    # re-seeded on the next load_pool() call.
    ...
    elif removed.source == "henio_pkce" and provider == "anthropic":
        from henio_constants import get_henio_home
        oauth_file = get_henio_home() / ".anthropic_oauth.json"
        if oauth_file.exists():
            oauth_file.unlink()
            print("Cleared Henio Anthropic OAuth credentials")

    elif removed.source == "claude_code" and provider == "anthropic":
        from henio_cli.auth import suppress_credential_source
        ...
        print("Run `henio auth add anthropic` to re-enable if needed.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `henio auth` is called bare."""
