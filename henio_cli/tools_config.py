"""
Unified tool configuration for Henio Agent.

`henio tools` and `henio setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.henio/config.yaml under
...
from henio_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _HENIO_CORE_TOOLS (available at runtime if enabled),
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from henio_cli.plugins import discover_plugins, get_plugin_toolsets
...
from henio_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "browser": {
        ...
        "providers": [
            {
                "name": "Henio Subscription (Browser Use cloud)",
    ...
    if post_setup_key in ("agent_browser", "browserbase"):
        ...
        if not node_modules.exists() and shutil.which("npm"):
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_henio_home()}/henio-agent && npm install")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in henio-agent directory)")
...
def _get_platform_tools(
    ...
    *,
    ...
    # "henio-cli" (which include all _HENIO_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "henio-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
    ...
    # A plugin toolset is "known" for a platform once `henio tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by henio tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (henio-cli, henio-telegram, etc.)
...
def _configure_provider(provider: dict, config: dict):
    ...
    if provider.get("requires_nous_auth"):
        ...
        if not features.nous_auth_present:
            _print_warning("  Henio Subscription is only available after logging into Henio Portal.")
            return
    ...
    if not env_vars:
        ...
        if managed_feature:
            _print_info("  Requests for this tool will be billed to your Henio subscription.")
            override_envs = provider.get("override_env_vars", [])
            if any(get_env_value(env_var) for env_var in override_envs):
                _print_warning(
                    "  Direct credentials are still configured and may take precedence until you remove them from ~/.henio/.env."
                )
        return
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `henio tools` and `henio setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Henio Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://paq85.github.io/henio-agent/docs/user-guide/features/tools", Colors.DIM))
    print()
        ...
        for pkey in enabled_platforms:
            ...
            if managed_nous_tools_enabled():
                for ts_key in sorted(auto_configured):
                    ...
                    print(color(f"  ✓ {label}: using your Henio subscription defaults", Colors.GREEN))
    ...
    print()
    from henio_constants import display_henio_home
    print(color(f"  Tool configuration saved to {display_henio_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'henio' or gateway restart.", Colors.DIM))
    print()
