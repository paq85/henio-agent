"""
Doctor command for the Henio CLI.

Diagnoses issues with Henio Agent setup.
"""
...
from henio_cli.config import get_project_root, get_henio_home, get_env_path
from henio_constants import display_henio_home

PROJECT_ROOT = get_project_root()
HENIO_HOME = get_henio_home()
_DHH = display_henio_home()  # user-facing display path (e.g. ~/.henio or ~/.henio/profiles/coder)

# Load environment variables from ~/.henio/.env so API key checks work
...
from henio_constants import OPENROUTER_MODELS_URL
...
from henio_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.henio/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `henio`.
    os.environ.setdefault("HENIO_INTERACTIVE", "1")
    ...
    print(color("│                 🩺 Henio Doctor                         │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Check ~/.henio/.env (primary location for user config)
    env_path = HENIO_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'henio setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'henio setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'henio setup' to create one")
                issues.append("Run 'henio setup' to create .env")
    
    # Check ~/.henio/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = HENIO_HOME / 'config.yaml'
    ...
    if config_path.exists():
        try:
            from henio_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'henio setup' to migrate config")
                else:
                    issues.append("Run 'henio doctor --fix' or 'henio setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'henio doctor --fix'")
        except Exception:
            pass
    ...
    try:
        from henio_cli.auth import get_nous_auth_status, get_codex_auth_status

        nous_status = get_nous_auth_status()
        if nous_status.get("logged_in"):
            check_ok("Henio Portal auth", "(logged in)")
        else:
            check_warn("Henio Portal auth", "(not logged in)")
    ...
    henio_home = HENIO_HOME
    if henio_home.exists():
        check_ok(f"{_DHH} directory exists")
    else:
        if should_fix:
            henio_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = henio_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Henio a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Henio Agent Persona\n\n"
                "<!-- Edit this file to customize how Henio communicates. -->\n\n"
                "You are Henio, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = henio_home / "memories"
    ...
    state_db_path = henio_home / "state.db"
    ...
    wal_path = henio_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'henio doctor --fix' to checkpoint")
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'henio setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: henio skills list)")
    ...
    try:
        ...
        _mem_cfg_path = HENIO_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: henio memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                check_fail("Honcho API key or base URL not set", "run: henio memory setup")
                issues.append("No Honcho API key — run 'henio memory setup'")
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                check_fail("Mem0 API key not set", "(set MEM0_API_KEY in .env or run henio memory setup)")
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: henio memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: henio memory setup")
        except Exception as _e:
    ...
    # =========================================================================
    try:
        from henio_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "henio -p" in content:
                            _m = _re.search(r"henio -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'henio doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
