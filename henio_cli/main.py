#!/usr/bin/env python3
"""
Henio CLI - Main entry point.

Usage:
    henio                     # Interactive chat (default)
    henio chat                # Interactive chat
    henio gateway             # Run gateway in foreground
    henio gateway start       # Start gateway as service
    henio gateway stop        # Stop gateway service
    henio gateway status      # Show gateway status
    henio gateway install     # Install gateway service
    henio gateway uninstall   # Uninstall gateway service
    henio setup               # Interactive setup wizard
    henio logout              # Clear stored authentication
    henio status              # Show status of all components
    henio cron                # Manage cron jobs
    henio cron list           # List cron jobs
    henio cron status         # Check if cron scheduler is running
    henio doctor              # Check configuration and dependencies
    henio honcho setup                    # Configure Honcho AI memory integration
    henio honcho status                   # Show Honcho config and connection status
    henio honcho sessions                 # List directory → session name mappings
    henio honcho map <name>               # Map current directory to a session name
    henio honcho peer                     # Show peer names and dialectic settings
    henio honcho peer --user NAME         # Set user peer name
    henio honcho peer --ai NAME           # Set AI peer name
    henio honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    henio honcho mode                     # Show current memory mode
    henio honcho mode [hybrid|honcho|local]  # Set memory mode
    henio honcho tokens                   # Show token budget settings
    henio honcho tokens --context N       # Set session.context() token cap
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (henio tools, henio setup, henio model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'henio {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any henio module import.
#
# Many modules cache HENIO_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("HENIO_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.henio/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set HENIO_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # 2. If no flag, check active_profile in the henio root
    if profile_name is None:
        try:
            from henio_constants import get_default_henio_root
            active_path = get_default_henio_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set HENIO_HOME
    if profile_name is not None:
        try:
            from henio_cli.profiles import resolve_profile_env
            henio_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent henio from starting
            print(f"Warning: profile override failed ({exc}), using default", file=sys.stderr)
            return
        os.environ["HENIO_HOME"] = henio_home
...
# Load .env from ~/.henio/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from henio_cli.config import get_henio_home
from henio_cli.env_loader import load_henio_dotenv
load_henio_dotenv(project_env=PROJECT_ROOT / '.env')

# Initialize centralized file logging early — all `henio` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from henio_logging import setup_logging as _setup_logging
...
try:
    from henio_cli.config import load_config as _load_config_early
    from henio_constants import apply_ipv4_preference as _apply_ipv4
...
from henio_cli import __version__, __release_date__
from henio_constants import OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from henio_cli.config import get_env_path, get_henio_home, load_config
    from henio_cli.auth import get_auth_status

    # Determine whether Henio itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from henio_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_henio_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from henio_cli.auth import PROVIDER_REGISTRY
    ...
    # Only count these if Henio has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Henio to use their tokens.
    if _has_henio_config:
        try:
...
def _resolve_last_cli_session() -> Optional[str]:
    ...
    try:
        from henio_state import SessionDB
        db = SessionDB()
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, henio_bin
        cli_args: the original CLI arguments (everything after 'henio')
    """
    import shutil
    ...
    henio_bin = container_info["henio_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f'  }}];\n'
                    f"\n"
                    f"Or run: sudo henio {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo henio {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'henio sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print("It looks like Henio Agent isn't configured yet -- no API keys or providers found.")
        print()
        print("  Run:  henio setup")
        print()

        from henio_cli.setup import is_interactive_stdin, print_noninteractive_setup_guidance
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'henio setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from henio_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["HENIO_YOLO_MODE"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["HENIO_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from henio_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    import subprocess
    from pathlib import Path
    from henio_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Henio Agent?")
        print()
    ...
    session_dir = get_henio_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: henio gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  henio gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Henio Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  henio gateway")
        ...
        print()
        print("  Or install as a service: henio gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'henio whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from henio_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``henio model``) and the setup wizard
    ...
    from henio_cli.config import load_config, get_env_value
    ...
    effective_provider = (
        config_provider
        or os.getenv("HENIO_INFERENCE_PROVIDER")
        or "auto"
    )
    try:
    ...
    # a leftover OPENAI_BASE_URL in ~/.henio/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.henio/.env if the active provider is not 'custom'.
    ...
    """
    from henio_cli.config import get_env_value, save_env_value, load_config
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from henio_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from henio_cli.auth import _prompt_model_selection, _save_model_choice, deactivate_provider
    from henio_cli.config import get_env_value, save_env_value
    ...
    from henio_cli.models import model_ids, get_pricing_for_provider
    ...
    if selected:
        ...
        from henio_cli.config import load_config, save_config
        cfg = load_config()
        model = cfg.get("model")
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from henio_cli.config import get_env_value, save_config, save_env_value
    from henio_cli.nous_subscription import (
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from henio_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from henio_cli.codex_models import get_codex_model_ids
    import argparse
    ...
    _codex_token = None
    # Prefer credential pool (where `henio auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from henio_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from henio_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from henio_cli.auth import _save_model_choice, deactivate_provider
    from henio_cli.config import get_env_value, load_config, save_config
    ...
    from henio_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `henio model` to set a model.")
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from henio_cli.curses_ui import flush_stdin
        flush_stdin()
        print()
...
from henio_cli.models import _PROVIDER_MODELS
...
def _model_flow_copilot(config, current_model=""):
    ...
    from henio_cli.config import save_env_value, load_config, save_config
    from henio_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from henio_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from henio_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Henio turns to `copilot --acp`.")
    print("  Henio currently starts its own ACP subprocess for each request.")
    print("  Copilot CLI decides which model to use for ACP sessions.")
    print("  Henio does not send Copilot CLI a model hint.")
    print("  Any model saved here stays local to Henio for config/UI compatibility.")
    print(f"  Command: {resolved_command}")
    print(f"  Backend marker: {effective_base}")
    if status.get("ready"):
        print("  ACP readiness: ✓ ready")
    elif status.get("configured"):
        print("  ACP readiness: ✗ probe failed")
    else:
        print("  ACP readiness: ✗ command not found")
    print()

    if not status.get("configured"):
        print(f"  ⚠ {status.get('error') or 'Copilot CLI command was not found.'}")
        print("  Set HENIO_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere.")
        return
    ...
    if not status.get("logged_in"):
        ...
        if status.get("stderr_tail"):
            print("  Copilot CLI stderr:")
            for line in str(status.get("stderr_tail") or "").splitlines():
                print(f"    {line}")
        print("  Run `copilot login` and retry.")
        print("  Set HENIO_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere.")
        return
...
def _model_flow_kimi(config, current_model=""):
    ...
    from henio_cli.config import get_env_value, save_env_value, load_config, save_config
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    from henio_cli.config import get_env_value, save_env_value, load_config, save_config
    from henio_cli.models import fetch_api_models, opencode_model_api_mode, normalize_opencode_model_id
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from henio_constants import display_henio_home as _dhh_fn
            print(f"    Henio will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env.")
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from henio_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'henio model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate the Henio CLI with a provider."""
    from henio_cli.auth import login_command
    login_command(args)


def cmd_logout(args):
    """Clear provider authentication."""
    from henio_cli.auth import logout_command
    logout_command(args)


def cmd_auth(args):
    """Manage pooled credentials."""
    from henio_cli.auth_commands import auth_command
    auth_command(args)


def cmd_status(args):
    """Show status of all components."""
    from henio_cli.status import show_status
    show_status(args)


def cmd_cron(args):
    """Cron job management."""
    from henio_cli.cron import cron_command
    cron_command(args)


def cmd_webhook(args):
    """Webhook subscription management."""
    from henio_cli.webhook import webhook_command
    webhook_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from henio_cli.doctor import run_doctor
    run_doctor(args)


def cmd_dump(args):
    """Dump setup summary for support/debugging."""
    from henio_cli.dump import run_dump
    run_dump(args)


def cmd_config(args):
    """Configuration management."""
    from henio_cli.config import config_command
    config_command(args)


def cmd_backup(args):
    """Back up Henio home directory to a zip file."""
...
def cmd_version(args):
    """Show version."""
    print(f"Henio Agent v{__version__} ({__release_date__})")
    ...
    try:
        from henio_cli.banner import check_for_updates
        from henio_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Henio Agent."""
    _require_tty("uninstall")
    from henio_cli.uninstall import run_uninstall
    run_uninstall(args)
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``henio update --gateway`` so interactive prompts (stash restore,
...
def _update_via_zip(args):
    """Update Henio Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = f"https://github.com/paq85/henio-agent/archive/refs/heads/{branch}.zip"
    
    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="henio-update-")
        zip_path = os.path.join(tmp_dir, f"henio-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to henio-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"henio-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime("henio-update-autostash-%Y%m%d-%H%M%S")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Henio behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes henio completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print("⚠ Local changes were restored, but Henio couldn't find the stash entry to drop.")
        ...
        if drop.returncode != 0:
            print("⚠ Local changes were restored, but Henio couldn't drop the saved stash entry.")
    ...
    print("  Review `git diff` / `git status` if Henio behaves unexpectedly.")
    return True

# =========================================================================
# Fork detection and upstream management for `henio update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/paq85/henio-agent.git",
    "git@github.com:paq85/henio-agent.git",
    "https://github.com/paq85/henio-agent",
    "git@github.com:paq85/henio-agent",
}
OFFICIAL_REPO_URL = "https://github.com/paq85/henio-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    from henio_constants import get_henio_home
    return (get_henio_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        from henio_constants import get_henio_home
        (get_henio_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Henio repository.")
        print("  This means you may miss updates from paq85/henio-agent.")
        print()
        try:
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print("  ✓ Added upstream: https://github.com/paq85/henio-agent.git")
                has_upstream = True
            else:
                print("  ✗ Failed to add upstream remote. Skipping upstream sync.")
                return
        else:
            print("  Skipped. Run 'git remote add upstream https://github.com/paq85/henio-agent.git' to add later.")
...
def _invalidate_update_cache():
    ...
    ``henio update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from henio_constants import get_default_henio_root
    default_home = get_default_henio_root()
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "henio-agent[matrix]" or "package-name[extra]".
...
def cmd_update(args):
    """Update Henio Agent to the latest version."""
    import shutil
    from henio_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Henio Agent")
        return
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print("  curl -fsSL https://raw.githubusercontent.com/paq85/henio-agent/main/scripts/install.sh | bash")
            sys.exit(1)
    ...
    try:
        ...
        # the old bytecode (e.g. get_henio_home added to henio_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}")
        ...
        # modules in this process.  Reload henio_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_henio_home() added since the last release.
        try:
            import importlib
            import henio_constants as _hc
        ...
        try:
            from henio_cli.profiles import list_profiles, get_active_profile_name, seed_profile_skills
        ...
        if needs_migration:
            ...
            if gateway_mode:
                ...
                print("    Run 'henio config migrate' later to apply any new config/env options.")
                response = "n"
            else:
                try:
            ...
            if response in ('', 'y', 'yes'):
                ...
                if gateway_mode and missing_env:
                    print("  ℹ API keys require manual entry: henio config migrate")
            else:
                print()
                print("Skipped. Run 'henio config migrate' later to configure.")
        else:
        ...
        # When running as ``henio update --gateway`` (spawned by the gateway's
        # /update command), this process lives inside the gateway's systemd
        # cgroup.  ``systemctl restart henio-gateway`` kills everything in the
        ...
        if gateway_mode:
            _exit_code_path = get_henio_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            # Discover all henio-gateway* units (default + profiles)
            if supports_systemd_services():
                ...
                for scope, scope_cmd in [("user", ["systemctl", "--user"]), ("system", ["systemctl"])]:
                    try:
                        result = subprocess.run(
                            scope_cmd + ["list-units", "henio-gateway*", "--plain", "--no-legend", "--no-pager"],
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            if not parts:
                                continue
                            unit = parts[0]  # e.g. henio-gateway.service or henio-gateway-coder.service
            ...
            if is_macos():
                try:
                    from henio_cli.gateway import launchd_restart, get_launchd_label, get_launchd_plist_path
            ...
            if restarted_services or killed_pids:
                ...
                if killed_pids:
                    ...
                    print("    Restart manually: henio gateway run")
                    # Also restart for each profile if needed
                    if len(killed_pids) > 1:
                        print("    (or: henio -p <profile> gateway run  for each profile)")
        ...
        print("  henio model               # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``henio -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from henio_cli.profiles import (
        ...
        _build_wrapper_script, _is_wrapper_dir_in_path, _get_wrapper_dir,
    )
    from henio_constants import display_henio_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `henio profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_henio_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → henio -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default ({display_henio_home()})")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(f"  Choose a custom alias:  henio profile alias {name} --name <custom>")
                    print(f"  Or access via flag:     henio -p {name} chat")
                else:
        ...
        name = args.profile_name
        from henio_cli.profiles import get_profile_dir, profile_exists, _read_config_model, _check_gateway_running, _count_skills
        ...
        from henio_cli.profiles import profile_exists
        if not profile_exists(name):
            print(f"Error: Profile '{name}' does not exist.")
            sys.exit(1)
        ...
        if remove:
            ...
            if wrapper_path:
                ...
                if custom_name:
                    wrapper_path.write_text(_build_wrapper_script(name))
                print(f"✓ Alias created: {wrapper_path}")
                if not _is_wrapper_dir_in_path():
                    print(f"⚠ {_get_wrapper_dir()} is not in your PATH.")

    elif action == "rename":
        from henio_cli.profiles import rename_profile
        try:
        ...
        from henio_cli.profiles import export_profile
        name = args.profile_name
        ...
        from henio_cli.profiles import import_profile
        try:
...
def cmd_completion(args):
    ...
    from henio_cli.profiles import generate_bash_completion, generate_zsh_completion
...
def cmd_logs(args):
    """View and filter Henio log files."""
    from henio_cli.logs import tail_log, list_logs
...
def main():
    """Main entry point for the Henio CLI."""
    parser = argparse.ArgumentParser(
        prog="henio",
        description="Henio Agent - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    henio                         Start interactive chat
    henio chat -q "Hello"         Single query mode
    henio -c                      Resume the most recent session
    henio -c "my project"         Resume a session by name (latest in lineage)
    henio --resume <session_id>   Resume a specific session by ID
    henio setup                   Run setup wizard
    henio logout                  Clear stored authentication
    henio auth add <provider>     Add a pooled credential
    henio auth list               List pooled credentials
    henio auth remove <p> <t>     Remove pooled credential by index, id, or label
    henio auth reset <provider>   Clear exhaustion status for a provider
    henio model                   Select default model
    henio config                  View configuration
    henio config edit             Edit config in $EDITOR
    henio config set model gpt-4  Set a config value
    henio gateway                 Run messaging gateway
    henio -s henio-agent-dev,github-auth
    henio -w                      Start in isolated git worktree
    henio gateway install         Install gateway background service
    henio sessions list           List past sessions
    henio sessions browse         Interactive session picker
    henio sessions rename ID T    Rename/title a session
    henio logs                    View agent.log (last 50 lines)
    henio logs -f                 Follow agent.log in real time
    henio logs errors             View errors.log
    henio logs --since 1h         Lines from the last hour
    henio update                  Update to latest version
...
"""
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Henio Agent"
    )
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: henio-cli)"
    )
    model_parser.add_argument(
        "--scope",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Henio Agent with an interactive wizard. "
                    "Run a specific section: henio setup model|tts|terminal|gateway|tools|agent"
    )
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for the Henio CLI"
    )
    ...
    login_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use (default: henio-cli)"
    )
    login_parser.add_argument(
        "--scope",
        default=None,
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Henio Agent components"
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Henio Agent setup"
    )
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Henio setup "
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Henio home directory to a zip file",
        description="Create a zip archive of your entire Henio configuration, "
                    "skills, sessions, and data (excludes the henio-agent codebase)"
    )
    backup_parser.add_argument(
        "-o", "--output",
        help="Output path for the zip file (default: ~/henio-backup-<timestamp>.zip)"
    )
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Henio backup from a zip file",
        description="Extract a previously created Henio backup into your "
                    "Henio home directory, restoring configuration, skills, "
                    "sessions, and data"
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Henio Agent configuration"
    )
    ...
    def cmd_pairing(args):
        from henio_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, 'skills_action', None) == 'config':
            ...
            from henio_cli.skills_config import skills_command as skills_config_command
            skills_config_command(args)
        else:
            from henio_cli.skills_hub import skills_command
            skills_command(args)
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/henio-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
    ...
    def cmd_plugins(args):
        from henio_cli.plugins_cmd import plugins_command
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            from henio_cli.memory_setup import memory_command
            memory_command(args)
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'henio tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # henio tools disable <name...> [--platform cli]
    ...
    # henio tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from henio_cli.tools_config import tools_disable_enable_command
            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from henio_cli.tools_config import tools_command
            tools_command(args)
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Henio as an MCP server",
        description=(
            "Manage MCP server connections and run Henio as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'henio mcp add' to connect to a new server, or\n"
            "'henio mcp serve' to expose Henio conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Henio as an MCP server (expose conversations to other agents)",
    )
    ...
    def cmd_mcp(args):
        from henio_cli.mcp_config import mcp_command
        mcp_command(args)
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch henio --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            import shutil
            henio_bin = shutil.which("henio") or shutil.which("henio")
            if henio_bin:
                os.execvp(henio_bin, [Path(henio_bin).name, "--resume", selected_id])
            else:
                # Fallback: re-invoke via python -m
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "henio_cli.main", "--resume", selected_id],
                )
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Henio Agent to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Henio Agent",
        description="Remove Henio Agent from your system. Can keep configs/data for reinstall."
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Henio Agent as an ACP (Agent Client Protocol) server",
        description="Start Henio Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )

    def cmd_acp(args):
        """Launch Henio Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Henio instances",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Henio log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    henio logs                     Show last 50 lines of agent.log
    henio logs -f                  Follow agent.log in real time
    henio logs errors              Show last 50 lines of errors.log
    henio logs gateway -n 100      Show last 100 lines of gateway.log
    henio logs --level WARNING     Only show WARNING and above
    henio logs --session abc123    Filter by session ID
    henio logs --component tools   Only show tool-related lines
    henio logs --since 1h          Lines from the last hour
    henio logs --since 30m -f      Follow, starting from 30 min ago
    henio logs list                List available log files with sizes
""",
    )
    ...
    # e.g. ``henio -c Pokemon Agent Dev`` → ``henio -c 'Pokemon Agent Dev'``
    ...
    from henio_cli.config import get_container_exec_info
