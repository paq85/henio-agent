...
1. **User plugins**   – ``~/.henio/plugins/<name>/``
2. **Project plugins** – ``./.henio/plugins/<name>/`` (opt-in via
   ``HENIO_ENABLE_PROJECT_PLUGINS``)
3. **Pip plugins**     – packages that expose the ``henio_agent.plugins``
   entry-point group.
...
ENTRY_POINTS_GROUP = "henio_agent.plugins"

_NS_PARENT = "henio_plugins"
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``henio honcho ...``).
...
class PluginManager:
    ...
    def discover_and_load(self) -> None:
        ...
        # 1. User plugins (~/.henio/plugins/)
        user_dir = get_henio_home() / "plugins"
        manifests.extend(self._scan_directory(user_dir, source="user"))

        # 2. Project plugins (./.henio/plugins/)
        if _env_enabled("HENIO_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".henio" / "plugins"
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``henio_plugins.<name>``."""
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``henio tools`` TUI so plugin-provided toolsets appear
