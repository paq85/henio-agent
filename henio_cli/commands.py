"""Slash command definitions and autocomplete for the Henio CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("update", "Update Henio Agent to the latest version", "Info",
               gateway_only=True),
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``henio skills
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /henio handler.

    Maps both canonical names and aliases so /henio bg do stuff works
    the same as /henio background do stuff.
    """
    overrides = _resolve_config_gates()
