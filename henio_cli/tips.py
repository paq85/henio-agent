...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or henio -c.",
    ...
    "Ctrl+Z suspends Henio to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "henio -c resumes your most recent CLI session. henio -c \"project name\" resumes by title.",
    "henio -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "henio -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "henio chat -t web,terminal enables only specific toolsets for a focused session.",
    "henio chat -s github-pr-workflow preloads a skill at launch.",
    "henio chat -q \"query\" runs a single non-interactive query and exits.",
    "henio chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "henio chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "henio --yolo bypasses all dangerous command approval prompts for the entire session.",
    "henio chat --source telegram tags the session for filtering in henio sessions list.",
    "henio -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "henio doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "henio dump outputs a compact setup summary — great for bug reports.",
    "henio config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "henio config edit opens config.yaml in your default editor.",
    "henio config check scans for missing or stale configuration options.",
    "henio sessions browse opens an interactive session picker with search.",
    "henio sessions stats shows session counts by platform and database size.",
    "henio sessions prune --older-than 30 cleans up old sessions.",
    "henio skills search react --source skills-sh searches the skills.sh public directory.",
    "henio skills check scans installed hub skills for upstream updates.",
    "henio skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "henio skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "henio mcp add github --command npx adds MCP servers from the command line.",
    "henio mcp serve runs Henio itself as an MCP server for other agents.",
    "henio auth add lets you add multiple API keys for credential pool rotation.",
    "henio completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "henio logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "henio backup creates a zip backup of your entire Henio home directory.",
    "henio profile create coder creates an isolated profile that becomes its own command.",
    "henio profile create work --clone copies your current config and keys to a new profile.",
    "henio update syncs new bundled skills to ALL profiles automatically.",
    "henio gateway install sets up Henio as a system service (systemd/launchd).",
    "henio memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "henio webhook subscribe creates event-driven webhook routes with HMAC validation.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as henio -w).",
    ...
    "HENIO_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Henio tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'henio profile create coder' creates the 'coder' command.",
    "henio profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Henio prints a resume command with session ID and stats.",
    "henio sessions export backup.jsonl exports all sessions for backup or analysis.",
    "henio -r SESSION_ID resumes any specific past session by its ID.",
    ...
    # --- Skills ---
    "Fresh installs seed a small bundled core, and `henio skills browse` lets you add official optional or community skills on demand.",
    "Every installed skill automatically becomes a slash command — type / to see them all.",
    "henio skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .henio/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: henio cron add --skill blogwatcher \"Check for new posts\".",
    "Cron delivery targets include telegram, discord, slack, email, and many more platforms.",
    ...
    # --- Gateway & Messaging ---
    "Henio runs on 18 platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, email, and more.",
    "henio gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Henio via a local macOS server.",
    ...
    "Context files (.henio.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.henio/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Henio loads project context from .henio.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — henio tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.henio/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "henio chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "henio plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "henio acp runs Henio as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "HENIO_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "henio login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "henio status --deep runs deeper diagnostic checks across all components.",

    # --- Hidden Gems & Power-User Tricks ---
    "BOOT.md at ~/.henio/BOOT.md runs automatically on every gateway start — use it for startup checks.",
    "Cron jobs can attach a Python script (--script) whose stdout is injected into the prompt as context.",
    "Cron scripts live in ~/.henio/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Henio your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.henio/SOUL.md to customize.",
    ...
    "Before a gateway session reset, Henio auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.henio/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at HENIO_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "HENIO_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in HENIO_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.henio/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'henio profile create ls' would be rejected.",
    "henio profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "HENIO_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via henio tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.henio/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.henio/skills/.hub/quarantine/ holds skills pending security review.",
]
