...
Add, remove, or reorder entries here — both `henio setup` and
`henio` provider-selection will pick up the change automatically.
"""
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `henio model`
    flow without maintaining a separate static list.
    """
    from henio_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
def check_nous_free_tier() -> bool:
    ...
    try:
        from henio_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    for pid in _PROVIDER_ORDER:
        ...
        has_creds = False
        try:
            from henio_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Henio' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``henio_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Henio should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    try:
        from henio_cli.auth import resolve_api_key_provider_credentials
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from henio_cli.copilot_auth import copilot_request_headers
        return copilot_request_headers(is_agent_turn=True)
    except ImportError:
        return {
            "Editor-Version": COPILOT_EDITOR_VERSION,
            "User-Agent": "HenioAgent/1.0",
...
def _fetch_ai_gateway_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    if not base_url:
        from henio_constants import AI_GATEWAY_BASE_URL
    ...
    *,
    ...
    if normalized == "custom":
        ...
        message = (
            ...
            f"Henio will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
