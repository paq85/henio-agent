...
from henio_cli import auth as auth_mod
...
def _copilot_runtime_api_mode(model_cfg: Dict[str, Any], api_key: str) -> str:
    ...
    try:
        from henio_cli.models import copilot_model_api_mode
    ...
    *,
    ...
    if provider == "openai-codex":
        ...
        if configured_mode and _provider_supports_explicit_api_mode(provider, configured_provider):
            ...
            from henio_cli.models import opencode_model_api_mode
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    env_provider = os.getenv("HENIO_INFERENCE_PROVIDER", "").strip().lower()
    if env_provider:
        return env_provider
    ...
    *,
    ...
    if pool and pool.has_credentials():
        ...
        if entry is not None:
            pool_source = str(getattr(entry, "source", "") or "").strip().lower()
            pool_api_key = (
                getattr(entry, "runtime_api_key", None)
                or getattr(entry, "access_token", "")
            )
        # For Codex, auto-seeded pool entries are just a discovery mechanism for
        # shared CLI credentials. Runtime use should still go through
        # resolve_codex_runtime_credentials() so refresh and migration behavior
        # stays authoritative. Manual pool entries remain eligible here.
        if provider == "openai-codex" and entry is not None and pool_api_key:
            if not (pool_source == "manual" or pool_source.startswith("manual:")):
                logger.debug(
                    "Codex pool entry from source '%s' is auto-seeded; using runtime resolver instead",
                    pool_source or "<unknown>",
                )
                pool_api_key = ""
        ...
        # non-runtime contexts like `henio auth list`).  If the key is
        # expired, clear pool_api_key so we fall through to
        # resolve_nous_runtime_credentials() which handles refresh + mint.
        if provider == "nous" and entry is not None and pool_api_key:
            min_ttl = max(60, int(os.getenv("HENIO_NOUS_MIN_KEY_TTL_SECONDS", "1800")))
            nous_state = {
    ...
    if provider == "openai-codex":
        try:
            ...
            return {
                ...
                "base_url": creds.get("base_url", "").rstrip("/"),
                "api_key": creds.get("api_key", ""),
                "source": creds.get("source", "henio-auth-store"),
                "last_refresh": creds.get("last_refresh"),
                "requested_provider": requested_provider,
            }
        except AuthError:
