...
- ``henio_cli.providers``        -- canonical provider identity + overlays
- ``henio_cli.model_normalize``  -- per-provider name formatting
...
from henio_cli.providers import (
...
_HENIO_MODEL_WARNING = (
    "Nous Research Henio 3 & 4 models are NOT agentic and are not designed "
    "for use with Henio Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)


def _check_henio_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* looks like a Henio LLM model."""
    if "henio" in model_name.lower():
        return _HENIO_MODEL_WARNING
    return ""
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'henio model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'henio doctor' — config issues detected:"
    ...
    henio_warn = _check_henio_model_warning(new_model)
    if henio_warn:
        warnings.append(henio_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from henio_cli/models.py (OPENROUTER_MODELS,
    ...
    from henio_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS
    ...
    # Build curated model lists keyed by henio provider ID
    ...
    # --- 1. Check Henio-mapped providers ---
    for henio_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(henio_id)
        ...
        model_ids = curated.get(henio_id, [])
        total = len(model_ids)
    ...
    # --- 2. Check Henio-only providers (nous, openai-codex, copilot, opencode-go) ---
    from henio_cli.providers import HENIO_OVERLAYS
    from henio_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Henio provider ID.
    # HENIO_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Henio IDs ("copilot").
    _mdev_to_henio = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in HENIO_OVERLAYS.items():
        if pid in seen_slugs:
            continue

        # Resolve Henio slug — e.g. "github-copilot" → "copilot"
        henio_slug = _mdev_to_henio.get(pid, pid)
        if henio_slug in seen_slugs:
            continue
        ...
        if not has_creds:
            try:
                from henio_cli.auth import _load_auth_store
                store = _load_auth_store()
                providers_store = store.get("providers", {})
                pool_store = store.get("credential_pool", {})
                if store and (
                    pid in providers_store or henio_slug in providers_store
                    or pid in pool_store or henio_slug in pool_store
                ):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(henio_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", henio_slug, exc)
        ...
        # configured.
        if not has_creds and henio_slug == "anthropic":
            try:
                from agent.anthropic_adapter import (
                    read_claude_code_credentials,
                    read_henio_oauth_credentials,
                )
                henio_creds = read_henio_oauth_credentials()
                cc_creds = read_claude_code_credentials()
                if (henio_creds and henio_creds.get("accessToken")) or \
        ...
        # Use curated list — look up by Henio slug, fall back to overlay key
        model_ids = curated.get(henio_slug, []) or curated.get(pid, [])
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": henio_slug,
            "name": get_label(henio_slug),
            "is_current": henio_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "built-in" if henio_slug in PROVIDER_TO_MODELS_DEV else "henio",
        })
        seen_slugs.add(pid)
        seen_slugs.add(henio_slug)
