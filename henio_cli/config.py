"""
Configuration management for Henio Agent.

Config files are stored in ~/.henio/ for easy access:
- ~/.henio/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.henio/.env         - API keys and secrets

This module provides:
- henio config          - Show current configuration
- henio config edit     - Open config in editor
- henio config set      - Set a specific value
- henio config wizard   - Re-run setup wizard
"""
...
from henio_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("HENIO_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_henio_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Henio is running in package-manager-managed mode.

    Two signals: the HENIO_MANAGED env var (set by the systemd service),
    or a .managed marker file in HENIO_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade henio-agent"
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "henio update"


def format_managed_message(action: str = "modify this Henio installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("HENIO_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Henio installation is managed by NixOS "
            f"(HENIO_MANAGED={env_hint}).\n"
            "Edit the Henio NixOS service settings (currently `services.henio-agent.settings`) and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Henio installation is managed by Homebrew "
            f"(HENIO_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade henio-agent"
        )

    return (
        f"Cannot {action}: this Henio installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Henio Agent."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from HENIO_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, henio_bin
    or None if container mode is not active, we're already inside the
    container, or HENIO_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("HENIO_DEV") == "1":
        return None

    if _is_inside_container():
        return None

    container_mode_file = get_henio_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "henio-agent")
    exec_user = info.get("exec_user", "henio")
    henio_bin = info.get("henio_bin", "/data/current-package/bin/henio")
...
# Re-export from henio_constants — canonical definition lives there.
from henio_constants import get_henio_home  # noqa: F811,E402
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the henio group can
    share state with the gateway service.

    The mode can be overridden via the HENIO_HOME_MODE environment variable
    (e.g. HENIO_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse HENIO_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("HENIO_HOME_MODE", "").strip()
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into HENIO_HOME if the user doesn't have one yet."""
...
def ensure_henio_home():
    """Ensure ~/.henio directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_henio_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_henio_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"HENIO_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "terminal": {
        ...
        # docker_env lets you specify exact key-value pairs — useful when Henio
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Henio sends a stable profile-scoped userId to Camofox
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.henio/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # always goes to ~/.henio/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
    },

    # Honcho AI-native memory -- reads ~/.honcho/config.json as single source of truth.
    # This section is only needed for henio-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Henio Agent*" header.
    ...
    # Logging — controls file logging to ~/.henio/logs/.
...
# selection step (Henio Portal / OpenRouter / Custom endpoint), so this
...
OPTIONAL_ENV_VARS = {
    ...
    "NOUS_BASE_URL": {
        "description": "Henio Portal base URL override",
        "prompt": "Henio Portal base URL (leave empty for default)",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Henio auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @henio:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. HENIO_BOT)",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'henio-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    # HENIO_TOOL_PROGRESS and HENIO_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.henio.config`` entries, then checks
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'henio setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Henio won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'henio doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("HENIO_TOOL_PROGRESS")
            old_mode = get_env_value("HENIO_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from HENIO_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from HENIO_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("HENIO_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from HENIO_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: henio config set <key> <value>")
    ...
    # metadata.henio.config in their SKILL.md frontmatter.
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.henio/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.henio/config.yaml."""
    import copy
    ensure_henio_home()
    config_path = get_config_path()
...
#   openai-codex (OAuth — henio auth) — OpenAI Codex
#   nous         (OAuth — henio auth) — Henio Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.henio/config.yaml."""
    if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.henio/.env."""
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Henio env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.henio/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.henio/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.henio/.env and os.environ.
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.henio/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Henio Configuration                     │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  henio config edit      # Edit config file", Colors.DIM))
    print(color("  henio config set <key> <value>", Colors.DIM))
    print(color("  henio setup            # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: henio config set <key> <value>")
            print()
            print("Examples:")
            print("  henio config set model anthropic/claude-sonnet-4")
            print("  henio config set terminal.backend docker")
            print("  henio config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'henio config migrate' to add them")
        ...
        print("  henio config            Show current configuration")
        print("  henio config edit       Open config in editor")
        print("  henio config set <key> <value>   Set a config value")
        print("  henio config check      Check for missing/outdated config")
        print("  henio config migrate    Update config with new options")
        print("  henio config path       Show config file path")
        print("  henio config env-path   Show .env file path")
        sys.exit(1)
