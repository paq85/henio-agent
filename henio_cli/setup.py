"""
Interactive setup wizard for Henio Agent.
...
_DOCS_BASE = "https://paq85.github.io/henio-agent/docs"
...
def _setup_copilot_reasoning_selection(
    ...
    *,
    ...
    from henio_cli.models import github_model_reasoning_efforts, normalize_copilot_model_id
...
# display_henio_home imported lazily at call sites (stale-module safety during henio update)

from henio_cli.colors import Colors, color
...
from henio_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Henio Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Henio using environment variables or config commands:")
    print_info("  henio config set model.provider custom")
    print_info("  henio config set model.base_url http://localhost:8080/v1")
    print_info("  henio config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'henio setup' in an interactive terminal to use the full wizard.")
    print()
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0) -> int:
    ...
    from henio_cli.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from henio_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'henio setup')")


def _print_setup_summary(config: dict, henio_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'henio setup' to configure"))
    ...
    if subscription_features.web.managed_by_nous:
        tool_status.append(("Web Search & Extract (Henio subscription)", True, None))
    ...
    if subscription_features.browser.managed_by_nous:
        tool_status.append(("Browser Automation (Henio subscription)", True, None))
    ...
    if subscription_features.image_gen.managed_by_nous:
        tool_status.append(("Image Generation (Henio subscription)", True, None))
    ...
    if subscription_features.tts.managed_by_nous:
        tool_status.append(("Text-to-Speech (OpenAI via Henio subscription)", True, None))
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'henio setup tts'"))
    else:
        tool_status.append(("Text-to-Speech (Edge TTS)", True, None))

    if subscription_features.modal.managed_by_nous:
        tool_status.append(("Modal Execution (Henio subscription)", True, None))
    elif config.get("terminal", {}).get("backend") == "modal":
        if subscription_features.modal.direct_override:
            tool_status.append(("Modal Execution (direct Modal)", True, None))
        else:
            tool_status.append(("Modal Execution", False, "run 'henio setup terminal'"))
    elif managed_nous_tools_enabled() and subscription_features.nous_auth_present:
        tool_status.append(("Modal Execution (optional via Henio subscription)", True, None))
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'henio setup tools' to configure them,"
        )
        from henio_constants import display_henio_home as _dhh
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {henio_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('henio setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('henio setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('henio setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('henio setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('henio setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('henio config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('henio config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('henio config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('henio', Colors.GREEN)}              Start chatting")
    print(f"   {color('henio gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('henio doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `henio tools` and `henio setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``henio model``)
    ...
    provider added to ``henio model`` is automatically available here.
    ...
    # Delegate to the shared henio model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from henio_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: henio model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from henio_cli.auth_commands import auth_add_command
            ...
            print_info(
                "Henio can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Henio can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'henio setup' or configure AUXILIARY_VISION_* settings")
    ...
    if selected_provider == "nous" and nous_subscription_selected:
        ...
        if "tts" in changed_defaults:
            print_success("TTS provider set to: OpenAI TTS via your Henio subscription")
        else:
...
def _setup_tts_provider(config: dict):
    ...
    if managed_nous_tools_enabled() and subscription_features.nous_auth_present:
        choices.append("Henio Subscription (managed OpenAI TTS, billed to your subscription)")
    ...
    if selected == "nous-openai":
        selected = "openai"
        print_info("OpenAI TTS will use the managed Henio gateway and bill to your subscription.")
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.henio/.env."
            )
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'henio setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Henio runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("  When using Henio via Telegram/Discord, this is where")
        print_info(
        ...
        if use_managed_modal:
            ...
            print_info("Modal execution will use the managed Henio gateway and bill to your subscription.")
...
def _apply_default_agent_settings(config: dict):
    ...
    save_env_value("HENIO_MAX_ITERATIONS", "90")
    ...
    print_info("  Run `henio setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    # ── Max Iterations ──
    current_max = get_env_value("HENIO_MAX_ITERATIONS") or str(
    ...
    try:
        ...
        if max_iter > 0:
            save_env_value("HENIO_MAX_ITERATIONS", str(max_iter))
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Henio delivers cron job results,")
...
def _setup_slack():
    ...
    print()
    print_info("   Full guide: https://paq85.github.io/henio-agent/docs/user-guide/messaging/slack/")
    print()
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Henio delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where Henio delivers cron job results and notifications.")
...
def _setup_whatsapp():
    ...
    print_info("Requires Node.js. Run 'henio whatsapp' for guided setup.")
    print()
    if prompt_yes_no("Enable WhatsApp now?", True):
        save_env_value("WHATSAPP_ENABLED", "true")
        print_success("WhatsApp enabled")
        print_info("Run 'henio whatsapp' to choose your mode (separate bot number")
        print_info("or personal self-chat) and pair via QR code.")


def _setup_weixin():
    """Configure Weixin (personal WeChat) via iLink Bot API QR login."""
    from henio_cli.gateway import _setup_weixin as _gateway_setup_weixin
    _gateway_setup_weixin()


def _setup_signal():
    """Configure Signal via gateway setup."""
    from henio_cli.gateway import _setup_signal as _gateway_setup_signal
    _gateway_setup_signal()


def _setup_email():
    """Configure Email via gateway setup."""
    from henio_cli.gateway import _setup_email as _gateway_setup_email
...
def _setup_feishu():
    ...
    from henio_cli.gateway import _setup_feishu as _gateway_setup_feishu
    _gateway_setup_feishu()


def _setup_wecom():
    """Configure WeCom (Enterprise WeChat) via gateway setup."""
    from henio_cli.gateway import _setup_wecom as _gateway_setup_wecom
    _gateway_setup_wecom()


def _setup_wecom_callback():
    """Configure WeCom Callback (self-built app) via gateway setup."""
    from henio_cli.gateway import _setup_wecom_callback as _gw_setup
    _gw_setup()
...
def _setup_bluebubbles():
    ...
    print_info("Connects Henio to iMessage via BlueBubbles — a free, open-source")
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://paq85.github.io/henio-agent/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://paq85.github.io/henio-agent/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  henio config edit")
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Henio from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'henio setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     henio config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    print_info("  You can try manually: henio gateway install")
            else:
                print_info("  You can install later: henio gateway install")
                if _is_linux:
                    print_info("  Or as a boot-time service: sudo henio gateway install --system")
                print_info("  Or run in foreground:  henio gateway")
        else:
            print_info("Start the gateway to bring your bots online:")
            print_info("   henio gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `henio setup tools` and `henio tools` use the same flow:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      henio setup           — full or quick (auto-detected)
      henio setup model     — just model/provider
      henio setup tts       — just text-to-speech
      henio setup terminal  — just terminal backend
      henio setup gateway   — just messaging platforms
      henio setup tools     — just tool configuration
      henio setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Henio Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Henio Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Henio Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Henio configured.")
        print()
        ...
        if choice == 0:
            # Quick setup
            _run_quick_setup(config, henio_home)
            return
        elif choice == 1:
            # Full setup — fall through to run all sections
            pass
        elif choice == 7:
            print_info("Exiting. Run 'henio setup' again when ready.")
            return
        elif 2 <= choice <= 6:
            ...
            if section:
                ...
                func(config)
                save_config(config)
                _print_setup_summary(config, henio_home)
            return
    else:
        # ── First-Time Setup ──
        print()
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, henio_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {henio_home}")
...
def _resolve_henio_chat_argv() -> Optional[list[str]]:
    """Resolve argv for launching ``henio chat`` in a fresh process."""
    henio_bin = shutil.which("henio")
    if henio_bin:
        return [henio_bin, "chat"]

    try:
        if importlib.util.find_spec("henio_cli") is not None:
            return [sys.executable, "-m", "henio_cli.main", "chat"]
    except Exception:
        pass
...
def _offer_launch_chat():
    ...
    print()
    if not prompt_yes_no("Launch henio chat now?", True):
        return

    chat_argv = _resolve_henio_chat_argv()
    if not chat_argv:
        print_info("Could not relaunch Henio automatically. Run 'henio chat' manually.")
        return

    os.execvp(chat_argv[0], chat_argv)


def _run_first_time_quick_setup(config: dict, henio_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'henio setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    henio setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  henio setup gateway")
    print()
...
def _run_quick_setup(config: dict, henio_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'henio setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Henio to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'henio setup gateway'.")
