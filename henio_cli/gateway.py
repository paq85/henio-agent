"""
Gateway subcommand for the Henio CLI.

Handles: henio gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_henio_home is imported lazily at call sites to avoid ImportError
# when henio_constants is cached from a pre-update version during `henio update`.
from henio_cli.setup import (
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "henio-gateway*",
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``henio update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Henio profile are returned.
    """
    _exclude = exclude_pids or set()
    pids = [pid for pid in _get_service_pids() if pid not in _exclude]
    patterns = [
        "henio_cli.main gateway",
        "henio_cli.main --profile",
        "henio_cli.main -p",
        "henio_cli/main.py gateway",
        "henio_cli/main.py --profile",
        "henio_cli/main.py -p",
        "henio gateway",
        "gateway/run.py",
    ]
    current_home = str(get_henio_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"HENIO_HOME={current_home}" in command
            )

        if "--profile " in command or " -p " in command:
            return False
        if "HENIO_HOME=" in command and f"HENIO_HOME={current_home}" not in command:
            return False
        return True
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (HENIO_HOME-scoped).
...
from henio_constants import is_termux, is_wsl
...
_SERVICE_BASE = "henio-gateway"
SERVICE_DESCRIPTION = "Henio Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current HENIO_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (HENIO_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from henio_constants import get_default_henio_root
    home = get_henio_home().resolve()
    default = get_default_henio_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary HENIO_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(henio_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when HENIO_HOME is a named profile.

    For ``~/.henio/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        henio_home: Optional explicit HENIO_HOME path. Defaults to the current
            ``get_henio_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from henio_constants import get_default_henio_root
    home = Path(henio_home or str(get_henio_home())).resolve()
    default = get_default_henio_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this HENIO_HOME.

    Default ``~/.henio`` returns ``henio-gateway`` (backward compatible).
    Profile ``~/.henio/profiles/coder`` returns ``henio-gateway-coder``.
    Any other HENIO_HOME appends a short hash for uniqueness.
    """
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    henio gateway uninstall")
    print_info("    sudo henio gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            print_warning("  System service install requires sudo, so Henio can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo henio gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo henio gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo henio gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Henio often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.henio`` → ``ai.henio.gateway.plist`` (backward compatible).
    Profile ``~/.henio/profiles/coder`` → ``ai.henio.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.henio.gateway-{suffix}" if suffix else "ai.henio.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.henio/henio-agent  -> /home/alice/.henio/henio-agent
      /opt/henio                 -> /opt/henio  (kept as-is)
    """
...
def _henio_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current HENIO_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_henio_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.henio                    → /home/alice/.henio
      /root/.henio/profiles/coder     → /home/alice/.henio/profiles/coder
      /opt/custom-henio               → /opt/custom-henio  (kept as-is)
    """
    current_henio = get_henio_home().resolve()
    current_default = (Path.home() / ".henio").resolve()
    target_default = Path(target_home_dir) / ".henio"

    # Default ~/.henio → remap to target user's default
    if current_henio == current_default:
        return str(target_default)

    # Profile or subdir of ~/.henio → preserve the relative structure
    try:
        relative = current_henio.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.henio) — keep as-is
        return str(current_henio)
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        henio_home = _henio_home_for_target_user(home_dir)
        profile_arg = _profile_arg(henio_home)
...
User={username}
Group={group_name}
ExecStart={python_path} -m henio_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="HENIO_HOME={henio_home}"
Restart=on-failure
RestartSec=30
...
"""
    ...
    henio_home = str(get_henio_home().resolve())
    profile_arg = _profile_arg(henio_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Henio install")
    return True
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}henio gateway start{scope_flag}               # Start the service")
    print(f"  {'sudo ' if system else ''}henio gateway status{scope_flag}              # Check status")
...
def systemd_status(deep: bool = False, system: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}henio gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}henio gateway restart{scope_flag}   # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}henio gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.henio.gateway-{suffix}" if suffix else "ai.henio.gateway"
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Henio install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  henio gateway status              # Check status")
...
def launchd_stop():
    ...
    # `henio gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Henio install")
    else:
        print("⚠ Service definition is stale relative to the current Henio install")
        print("  Run: henio gateway start")
    ...
    if deep:
        log_file = get_henio_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Henio will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @henio:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. henio) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where Henio delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Henio agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Henio will use (e.g., henio@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Henio connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: henio pairing generate bluebubbles",
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_henio_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'henio pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: henio pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'henio gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from henio_cli.main import cmd_whatsapp
    import argparse
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Henio will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info("  3. Henio will store the returned account_id/token in ~/.henio/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `henio gateway setup`.")
        return
    ...
    import asyncio
    try:
        credentials = asyncio.run(qr_login(str(get_henio_home())))
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `henio pairing approve`.")
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"HenioAgent\"")
        ...
        if service_running:
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: henio gateway")
                except subprocess.CalledProcessError as e:
            ...
            if supports_systemd_services() or is_macos():
                ...
                if prompt_yes_no(f"  Install the gateway as a {platform_name} service?{wsl_note} (runs in background, starts on boot)", True):
                    ...
                    print_info("  Or run in foreground:  henio gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: henio gateway run")
                print_info("  For persistence:   tmux new -s henio 'henio gateway run'")
                ...
                if is_termux():
                    ...
                    print_info("  Run in foreground: henio gateway run")
                    print_info(f"  Or start it manually in the background (best effort): nohup henio gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
                else:
                    print_info("  Service install not supported on this platform.")
                    print_info("  Run in foreground: henio gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'henio gateway setup' when ready.")
...
def gateway_command(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: henio gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: henio gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s henio 'henio gateway run'")
                print()
            ...
            print()
            print("  henio gateway run                               # direct foreground")
            print("  tmux new -s henio 'henio gateway run'           # persistent via tmux")
            print("  nohup henio gateway run > ~/.henio/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: henio gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: henio gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: henio gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  henio gateway install")
                    print("  sudo henio gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  henio gateway run       # Run in foreground")
                if is_termux():
                    print("  nohup henio gateway run > ~/.henio/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s henio 'henio gateway run'           # persistent via tmux")
                    print("  nohup henio gateway run > ~/.henio/logs/gateway.log 2>&1 &  # background")
                else:
                    print("  henio gateway install   # Install as user service")
                    print("  sudo henio gateway install --system  # Install as boot-time system service")
