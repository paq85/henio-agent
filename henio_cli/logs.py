"""``henio logs`` — view and filter Henio log files.
...
Usage examples::
    ...
    henio logs                    # last 50 lines of agent.log
    henio logs -f                 # follow agent.log in real time
    henio logs errors             # last 50 lines of errors.log
    henio logs gateway -n 100    # last 100 lines of gateway.log
    henio logs --level WARNING    # only WARNING+ lines
    henio logs --session abc123   # filter by session ID substring
    henio logs --component tools  # only tool-related lines
    henio logs --since 1h         # lines from the last hour
    henio logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_henio_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Henio runs — try 'henio chat' first)")
        sys.exit(1)
    ...
    if component:
        from henio_logging import COMPONENT_PREFIXES
    ...
    if follow:
        print(f"--- {display_henio_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_henio_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_henio_home()}/logs/")
        return

    print(f"Log files in {display_henio_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'henio chat' to generate logs)")
