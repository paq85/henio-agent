"""
Henio MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Henio-specific extra)

Usage:
    henio mcp serve
    henio mcp serve --verbose
...
logger = logging.getLogger("henio.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using HENIO_HOME."""
    try:
        from henio_constants import get_henio_home
        return get_henio_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("HENIO_HOME", Path.home() / ".henio")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from henio_constants import get_henio_home
        directory_file = get_henio_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("HENIO_HOME", Path.home() / ".henio")
...
class EventBridge:
    ...
    def _poll_once(self, db):
        ...
        try:
            from henio_constants import get_henio_home
            db_file = get_henio_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("HENIO_HOME", Path.home() / ".henio")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Henio MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(
            "MCP server requires the 'mcp' package. "
            "Install with: pip install 'henio-agent[mcp]'"
        )

    mcp = FastMCP(
        "henio",
        instructions=(
            "Henio Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Henio MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
