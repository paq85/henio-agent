...
_loaded_env_paths = load_henio_dotenv(henio_home=_henio_home, project_env=_project_env)
...
class _SafeWriter:
    ...
    When henio-agent runs as a systemd service, Docker container, or headless
...
def _should_parallelize_tool_batch(tool_calls) -> bool:
    ...
    for tool_call in tool_calls:
        ...
        function_args = _parse_tool_call_args_dict(tool_call.function.arguments)
...
def _parse_tool_call_args_dict(raw_args: Any) -> dict | None:
    """Best-effort normalize tool-call arguments into a dict."""
...
def _canonicalize_tool_call_args(raw_args: Any) -> str:
    """Return a stable string form for tool-call argument deduping."""
    parsed = _parse_tool_call_args_dict(raw_args)
...
class AIAgent:
    ...
    def __init__(
        ...
        if (
            ...
            and not self._base_url_lower.startswith("acp://copilot")
            and (
        ...
        # both live under ~/.henio/logs/.  Idempotent, so gateway mode
        # (which creates a new AIAgent per message) won't duplicate handlers.
        from henio_logging import setup_logging, setup_verbose_logging
        setup_logging(henio_home=_henio_home)
        ...
        # Session logs go into ~/.henio/sessions/ alongside gateway sessions
        henio_home = get_henio_home()
        self.logs_dir = henio_home / "sessions"
        ...
        if self._session_db:
            try:
                self._session_db.create_session(
                    ...
                    source=self.platform or os.environ.get("HENIO_SESSION_SOURCE", "cli"),
                    model=self.model,
                    model_config={
        ...
        try:
            from henio_cli.config import load_config as _load_agent_config
            ...
            try:
                ...
                if not _mem_provider_name:
                    try:
                        ...
                        if _hcfg.enabled and (_hcfg.api_key or _hcfg.base_url):
                            ...
                            try:
                                from henio_cli.config import load_config as _lc, save_config as _sc
                                _cfg = _lc()
                ...
                if _mem_provider_name:
                    ...
                    if self._memory_manager.providers:
                        from henio_constants import get_henio_home as _ghh
                        _init_kwargs = {
                            "session_id": self.session_id,
                        ...
                        try:
                            ...
                            _init_kwargs["agent_workspace"] = "henio"
                        except Exception:
                            pass
        ...
        if _engine_name != "compressor":
            ...
            if _selected_engine is None:
                try:
                    from henio_cli.plugins import get_plugin_context_engine
        ...
        if (self.model or "").strip() and _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                f"Model {self.model} has a context window of {_ctx:,} tokens, "
                f"which is below the minimum {MINIMUM_CONTEXT_LENGTH:,} required "
                f"by Henio Agent.  Choose a model with at least "
    ...
    def switch_model(self, new_model, new_provider, api_key='', base_url='', api_mode=''):
        ...
        """
        import logging
        from henio_cli.providers import determine_api_mode
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``henio chat -q``. In that mode,
        ...
        try:
            ...
            if client is None or not aux_model:
                msg = (
                    ...
                    "Run `henio setup` or set OPENROUTER_API_KEY."
                )
    ...
    def _dump_api_request_debug(
        ...
        *,
        ...
        try:
            ...
            if env_var_enabled("HENIO_DUMP_REQUEST_STDOUT"):
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        if not self.skip_context_files:
            ...
            # mode).  The gateway process runs from the henio-agent install
        ...
        from henio_time import now as _henio_now
        now = _henio_now()
    ...
    def _deduplicate_tool_calls(tool_calls: list) -> list:
        ...
        unique: list = []
        for tc in tool_calls:
            key = (tc.function.name, _canonicalize_tool_call_args(tc.function.arguments))
            if key not in seen:
                seen.add(key)
                unique.append(tc)
            else:
    ...
    def _create_request_openai_client(self, *, reason: str) -> Any:
        ...
        if isinstance(primary_client, Mock) or not callable(getattr(primary_client, "close", None)):
    ...
    def _try_refresh_nous_client_credentials(self, *, force: bool = True) -> bool:
        ...
        try:
            ...
            creds = resolve_nous_runtime_credentials(
                min_key_ttl_seconds=max(60, int(os.getenv("HENIO_NOUS_MIN_KEY_TTL_SECONDS", "1800"))),
                timeout_seconds=float(os.getenv("HENIO_NOUS_TIMEOUT_SECONDS", "15")),
                force_mint=force,
            )
        except Exception as exc:
    ...
    def _interruptible_streaming_api_call(
        ...
        def _call_chat_completions():
            ...
            import httpx as _httpx
            _base_timeout = float(os.getenv("HENIO_API_TIMEOUT", 1800.0))
            _stream_read_timeout = float(os.getenv("HENIO_STREAM_READ_TIMEOUT", 120.0))
            # Local providers (Ollama, llama.cpp, vLLM) can take minutes for
            # prefill on large contexts before producing the first token.
            # Auto-increase the httpx read timeout unless the user explicitly
            # overrode HENIO_STREAM_READ_TIMEOUT.
        ...
        def _call():
            ...
            _max_stream_retries = int(os.getenv("HENIO_STREAM_RETRIES", 2))
        ...
        _stream_stale_timeout_base = float(os.getenv("HENIO_STREAM_STALE_TIMEOUT", 180.0))
        # Local providers (Ollama, oMLX, llama-cpp) can take 300+ seconds
        # for prefill on large contexts.  Disable the stale detector unless
        # the user explicitly set HENIO_STREAM_STALE_TIMEOUT.
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        api_kwargs = {
            ...
            "timeout": float(os.getenv("HENIO_API_TIMEOUT", 1800.0)),
        }
        if self._is_qwen_portal():
            api_kwargs["metadata"] = {
                "sessionId": self.session_id or "henio",
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        if "models.github.ai" in self._base_url_lower or "api.githubcopilot.com" in self._base_url_lower:
            try:
                from henio_cli.models import github_model_reasoning_efforts
    ...
    def _handle_max_iterations(self, messages: list, api_call_count: int) -> str:
        ...
        try:
            ...
            if _is_nous:
                summary_extra_body["tags"] = ["product=henio-agent"]
    ...
    def run_conversation(
        ...
        # ``henio logs --session <id>`` can filter a single conversation.
        from henio_logging import set_session_context
        ...
        # are reused.  The system prompt is Henio's territory; plugins
        ...
        while (api_call_count < self.max_iterations and self.iteration_budget.remaining > 0) or self._budget_grace_call:
            ...
            # cache prefix.  The system prompt is reserved for Henio internals.
            if effective_system:
                api_messages = [{"role": "system", "content": effective_system}] + api_messages
            ...
            while retry_count < max_retries:
                try:
                    ...
                    if env_var_enabled("HENIO_DUMP_REQUESTS"):
                    ...
                    if (
                        ...
                        _dhh = _dhh_fn()
                        print(f"{self.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Henio-managed OAuth/setup tokens")
                        print(f"{self.log_prefix}     • Check ANTHROPIC_API_KEY in {_dhh}/.env for API keys or legacy token values")
                        print(f"{self.log_prefix}     • For API keys: verify at https://console.anthropic.com/settings/keys")
                        print(f"{self.log_prefix}     • For Claude Code: run 'claude /login' to refresh, then retry")
                        print(f"{self.log_prefix}     • Legacy cleanup: henio config set ANTHROPIC_TOKEN \"\"")
                        print(f"{self.log_prefix}     • Clear stale keys: henio config set ANTHROPIC_API_KEY \"\"")
                    ...
                    if is_client_error:
                        ...
                        if classified.is_auth or classified.reason == FailoverReason.billing:
                            if _provider == "openai-codex" and status_code == 401:
                                ...
                                self._vprint(f"{self.log_prefix}      2. Then run `henio auth` to re-authenticate.", force=True)
                            else:
                                self._vprint(f"{self.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                                self._vprint(f"{self.log_prefix}      • Is the key valid? Run: henio setup", force=True)
