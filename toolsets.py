...
TOOLSETS = {
    ...
    # ==========================================================================
    # Full Henio toolsets (CLI + messaging platforms)
    #
    ...
    "henio-email": {
        "description": "Email bot toolset - interact with Henio via email (IMAP/SMTP)",
        "tools": _HENIO_CORE_TOOLS,
        "includes": []
    },
...
_PUBLIC_TOOLSET_ALIASES = {
    "henio-acp": "henio-acp",
    "henio-api-server": "henio-api-server",
    "henio-cli": "henio-cli",
    "henio-telegram": "henio-telegram",
    "henio-discord": "henio-discord",
    "henio-whatsapp": "henio-whatsapp",
    "henio-slack": "henio-slack",
    "henio-signal": "henio-signal",
    "henio-bluebubbles": "henio-bluebubbles",
    "henio-homeassistant": "henio-homeassistant",
    "henio-email": "henio-email",
    "henio-mattermost": "henio-mattermost",
    "henio-matrix": "henio-matrix",
    "henio-dingtalk": "henio-dingtalk",
    "henio-feishu": "henio-feishu",
...
_LEGACY_PUBLIC_TOOLSET_NAMES = {
    legacy: canonical for canonical, legacy in _PUBLIC_TOOLSET_ALIASES.items()
}


def _resolve_toolset_alias(name: str) -> str:
    """Resolve canonical public aliases to the legacy backing key."""
    return _PUBLIC_TOOLSET_ALIASES.get(name, name)


def _public_toolset_name(name: str) -> str:
    """Return the canonical public name for a backing toolset key."""
    return _LEGACY_PUBLIC_TOOLSET_NAMES.get(name, name)
...
def get_toolset(name: str) -> Optional[Dict[str, Any]]:
    ...
    return TOOLSETS.get(_resolve_toolset_alias(name))
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    name = _resolve_toolset_alias(name)
...
def get_all_toolsets() -> Dict[str, Dict[str, Any]]:
    ...
    for ts_name, ts_data in TOOLSETS.items():
        result[_public_toolset_name(ts_name)] = ts_data
...
def get_toolset_names() -> List[str]:
    ...
    """
    names = set(get_all_toolsets().keys())
...
def validate_toolset(name: str) -> bool:
    ...
    if name in {"all", "*"}:
        return True
    if _resolve_toolset_alias(name) in TOOLSETS:
        return True
...
def get_toolset_info(name: str) -> Dict[str, Any]:
    ...
    resolved_name = _public_toolset_name(_resolve_toolset_alias(name))
