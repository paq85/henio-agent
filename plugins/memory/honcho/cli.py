...
Handles: henio honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "henio" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Henio profiles and creates host blocks for any that don't
    have one yet. Inherits settings from the default host block.
    """
    try:
        from henio_cli.profiles import list_profiles
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'henio honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'henio honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `henio update` -- no output, no exceptions.
    """
    try:
        from henio_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Henio profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $HENIO_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_henio_home() / "honcho.json"
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Henio persistent cross-session memory.")
    ...
    henio_host = hosts.setdefault(_host_key(), {})
    ...
    if is_local:
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'henio honcho setup' again once you have a key.\n")
            return

    # --- 3. Identity ---
    current_peer = henio_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        henio_host["peerName"] = new_peer

    current_ai = henio_host.get("aiPeer") or cfg.get("aiPeer", "henio")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        henio_host["aiPeer"] = new_ai

    current_workspace = henio_host.get("workspace") or cfg.get("workspace", "henio")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        henio_host["workspace"] = new_workspace

    # --- 4. Observation mode ---
    current_obs = henio_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in ("unified", "directional"):
        henio_host["observationMode"] = new_obs
    else:
        henio_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(henio_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        henio_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        henio_host["writeFrequency"] = new_wf if new_wf in ("async", "turn", "session") else "async"

    # --- 6. Recall mode ---
    _raw_recall = henio_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in ("hybrid", "context", "tools"):
        henio_host["recallMode"] = new_recall

    # --- 7. Session strategy ---
    current_strat = henio_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-directory")
    print("\n  Session strategy:")
    ...
    if new_strat in ("per-session", "per-repo", "per-directory", "global"):
        henio_host["sessionStrategy"] = new_strat

    henio_host["enabled"] = True
    henio_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        henio_config = load_config()
        henio_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(henio_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: henio config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    henio honcho status     -- show full config")
    print("    henio honcho mode       -- change recall/observation mode")
    print("    henio honcho tokens     -- tune context and dialectic budgets")
    print("    henio honcho peer       -- update peer names")
    print("    henio honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Henio profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: henio honcho setup\n")
        return
    ...
    if not cfg:
        ...
        print("  Run 'henio honcho setup' to configure.\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: henio honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        henio = hosts.get(_host_key(), {})
        user = henio.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = henio.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = henio.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = henio.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Henio' identity in Honcho. Seed with 'henio honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: henio honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = henio.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = henio.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = henio.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Henio asks Honcho's AI peer a question")
        ...
        print("\n  Set with: henio honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'henio honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'henio honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    henio honcho identity --show        — show both peer representations")
        print("    henio honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'henio memory setup'...\n")
        from henio_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``henio honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``henio honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to henio memory setup)",
    )
