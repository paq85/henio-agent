...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/henio>
...
```bash
henio honcho setup    # full interactive wizard (cloud or local)
henio memory setup    # generic picker, also works
```

Or manually:
```bash
henio config set memory.provider honcho
echo "HONCHO_API_KEY=your-key" >> ~/.henio/.env
```
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$HENIO_HOME/honcho.json` | Profile-local (isolated Henio instances) |
| 2 | `~/.henio/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Henio profile: `henio` (default) or `henio.<profile>`.
...
Host key derivation: `HENIO_HONCHO_HOST` env > active profile (`henio.<profile>`) > `"henio"`.
...
| `HENIO_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `henio honcho setup` | Full interactive setup wizard |
| `henio honcho status` | Show resolved config for active profile |
| `henio honcho enable` / `disable` | Toggle Honcho for active profile |
| `henio honcho mode <mode>` | Change recall or observation mode |
| `henio honcho peer --user <name>` | Update user peer name |
| `henio honcho peer --ai <name>` | Update AI peer name |
| `henio honcho tokens --context <N>` | Set context token budget |
| `henio honcho tokens --dialectic <N>` | Set dialectic max chars |
| `henio honcho map <name>` | Map current directory to a session name |
| `henio honcho sync` | Create host blocks for all Henio profiles |
