...
Resolution order for config file:
  1. $HENIO_HOME/honcho.json  (instance-local, enables isolated Henio instances)
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Henio profile.

    Resolution order:
      1. HENIO_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``henio.<profile>``
      3. Fallback: ``"henio"`` (default profile)
    """
    explicit = os.environ.get("HENIO_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $HENIO_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.henio/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_henio_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = Path.home() / ".henio" / "honcho.json"
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into Henio system prompt
    ...
    # True when Honcho was explicitly configured for this host (hosts.henio
    ...
    def from_global_config(
        ...
        Resolution: $HENIO_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active Henio profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.henio block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. Henio session title (from /title command)
          3. per-session strategy — Henio session_id ({timestamp}_{hex})
        ...
        # per-session: inherit Henio session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'henio honcho setup' or set HONCHO_API_KEY. "
    ...
    if not resolved_base_url:
        try:
            ...
            henio_cfg = load_config()
            honcho_cfg = henio_cfg.get("honcho", {})
