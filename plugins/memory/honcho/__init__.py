...
Config: Uses the existing Honcho config chain:
  1. $HENIO_HOME/honcho.json (profile-scoped)
...
class HonchoMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, henio_home):
        """Write config to $HENIO_HOME/honcho.json (Honcho SDK native format)."""
        import json
        from pathlib import Path
        config_path = Path(henio_home) / "honcho.json"
        existing = {}
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            # Preserve explicit peerName selections only when we have a real
            # HonchoClientConfig instance that represents the user's saved
            # config. Some tests patch from_global_config() with a bare mock to
            # verify user_id threading; those mocks have no provenance about
            # whether peer_name was explicitly user-chosen, so gateway user_id
            # wins in that compatibility path.
            _gw_user_id = kwargs.get("user_id")
            _peer_name_is_explicit = isinstance(cfg, HonchoClientConfig) and bool(getattr(cfg, "peer_name", None))
            if _gw_user_id and not _peer_name_is_explicit:
    ...
    def _do_session_init(self, cfg, session_id: str, **kwargs) -> None:
        ...
        try:
            if not session.messages:
                ...
                mem_dir = str(get_henio_home() / "memories")
    ...
    def _ensure_session(self) -> bool:
        ...
        try:
            self._do_session_init(
                self._config,
                self._lazy_init_session_id or "henio-default",
