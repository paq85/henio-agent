...
Config file: `$HENIO_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `henio` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `henio-{identity}` → `henio-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Henio profile:

```json
{
  "container_tag": "henio-{identity}"
}
```

For a profile named `coder`, this resolves to `henio-coder`. The default profile resolves to `henio-default`. Without `{identity}`, all profiles share the same container.
