...
def _load_supermemory_config(henio_home: str) -> dict:
    config = _default_config()
    config_path = Path(henio_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, henio_home: str) -> None:
    config_path = Path(henio_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `henio memory setup`.
        # All other options are documented for $HENIO_HOME/supermemory.json
    ...
    def save_config(self, values, henio_home):
        ...
        _save_supermemory_config(sanitized, henio_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from henio_constants import get_henio_home
        self._henio_home = kwargs.get("henio_home") or str(get_henio_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._henio_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "henio", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "henio_memory", "target": target, "type": "explicit_memory"},
