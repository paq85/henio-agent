# nix/nixosModules.nix — NixOS module for henio-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: henio runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $HENIO_HOME/.env
# and read by henio at startup — no container recreation needed for env changes.
#
# Tool resolution: the henio wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.henio-agent;
    henio-agent = inputs.self.packages.${pkgs.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-henio-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "henio-config-attrs";
      description = "Henio YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "henio-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "henio-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'HENIO_DOC_EOF'\n${value}\nHENIO_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "henio-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/henio";
    ...
    # henio user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "henio-container-entrypoint" ''
      set -eu

      HENIO_UID="''${HENIO_UID:?HENIO_UID must be set}"
      HENIO_GID="''${HENIO_GID:?HENIO_GID must be set}"

      # ── Group: ensure a group with GID=$HENIO_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$HENIO_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="henio"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$HENIO_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$HENIO_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$HENIO_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$HENIO_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="henio"
        TARGET_HOME="/home/henio"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$HENIO_UID" -g "$HENIO_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$HENIO_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$HENIO_UID:$HENIO_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure HENIO_HOME is owned by the target user
      if [ -n "''${HENIO_HOME:-}" ] && [ -d "$HENIO_HOME" ]; then
        chown -R "$HENIO_UID:$HENIO_GID" "$HENIO_HOME"
      fi
      ...
      if [ ! -f /var/lib/henio-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        echo "First boot: provisioning agent tools..."
        apt-get update -qq
        apt-get install -y -qq sudo nodejs npm curl
        touch /var/lib/henio-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/henio ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/henio
        chmod 0440 /etc/sudoers.d/henio
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$HENIO_UID" --regid="$HENIO_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $HENIO_HOME/.env.
    ...
    # Default: /var/lib/henio/workspace → /data/workspace.
  ...
  in {
    options.services.henio-agent = with lib; {
      enable = mkEnableOption "Henio Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = henio-agent;
        description = "The henio-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/henio";
        description = "State directory. Contains .henio/ subdir (HENIO_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Henio config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $HENIO_HOME/.env at activation time.
          Henio reads this file on every startup via load_henio_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $HENIO_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $HENIO_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `henio gateway`.";
      };
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the henio CLI to environment.systemPackages and export
          HENIO_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.henio symlink to the service
            stateDir. These users are automatically added to the henio group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.henio-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the henio CLI to system PATH and export HENIO_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.henio/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ cfg.package ];
        environment.variables.HENIO_HOME = "${cfg.stateDir}/.henio";
      })
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.henio-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed henio
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure henio is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.henio        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.henio/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.henio/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.henio/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.henio/memories 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."henio-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.henio
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.henio ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.henio ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.henio -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories; do
            mkdir -p "${cfg.stateDir}/.henio/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.henio/$_subdir"
            chmod 2770 "${cfg.stateDir}/.henio/$_subdir"
            find "${cfg.stateDir}/.henio/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.henio/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.henio/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.henio/config.yaml
            chmod 0640 ${cfg.stateDir}/.henio/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.henio/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.henio/.managed
          chmod 0644 ${cfg.stateDir}/.henio/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.henio/.container-mode <<'HENIO_CONTAINER_MODE_EOF'
# Written by NixOS activation script. Do not edit manually.
backend=${cfg.container.backend}
container_name=${containerName}
exec_user=${cfg.user}
henio_bin=${containerDataDir}/current-package/bin/henio
HENIO_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.henio/.container-mode
            chmod 0644 ${cfg.stateDir}/.henio/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.henio/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.henio";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.henio" ]; then
                  rm -f "${symlinkPath}"
                  echo "henio-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.henio -> stateDir/.henio for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.henio";
                target = "${cfg.stateDir}/.henio";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "henio-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.henio/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.henio/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.henio/auth.json
              fi
            ''}
          ''}
          ...
          # Henio reads $HENIO_HOME/.env at startup via load_henio_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.henio/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'HENIO_NIX_ENV_EOF'
${envFileContent}
HENIO_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.henio-agent = {
          description = "Henio Agent Gateway";
          ...
          serviceConfig = {
            ...
            # $HENIO_HOME/.env by the activation script. load_henio_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${cfg.package}/bin/henio"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the henio group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.henio-agent = {
          description = "Henio Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              HENIO_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              HENIO_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env HENIO_UID="$HENIO_UID" \
                --env HENIO_GID="$HENIO_GID" \
                --env HENIO_HOME=${containerDataDir}/.henio \
                --env HENIO_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/henio gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
