...
{ inputs, ... }: {
  perSystem = { pkgs, system, lib, ... }:
    let
      henio-agent = inputs.self.packages.${system}.default;
      ...
      configKeys = pkgs.runCommand "henio-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${henioVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = lib.optionalAttrs pkgs.stdenv.hostPlatform.isLinux {
        ...
        package-contents = pkgs.runCommand "henio-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${henio-agent}/bin/henio || (echo "FAIL: henio binary missing"; exit 1)
          test -x ${henio-agent}/bin/henio-agent || (echo "FAIL: henio-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${henio-agent}/bin/henio version 2>&1 | grep -qi "henio" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "henio-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in henio henio-agent henio-acp; do
            test -x ${henio-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "henio-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking henio --help ==="
          ${henio-agent}/bin/henio --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${henio-agent}/bin/henio --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "henio-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${henio-agent}/share/henio-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${henio-agent}/share/henio-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "HENIO_BUNDLED_SKILLS" ${henio-agent}/bin/henio || \
            (echo "FAIL: HENIO_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: HENIO_BUNDLED_SKILLS set in wrapper"
        ...
        # Verify HENIO_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "henio-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(HENIO_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking HENIO_MANAGED guards ==="
          check_blocked "config set" ${henio-agent}/bin/henio config set model foo
          check_blocked "config edit" ${henio-agent}/bin/henio config edit
        ...
        in pkgs.runCommand "henio-config-roundtrip" {
          ...
          merge_and_load() {
            local henio_home="$1"
            export HENIO_HOME="$henio_home"
            ${configMergeScript} ${nixSettings} "$henio_home/config.yaml"
            ${henioVenv}/bin/python3 -c '
import json, sys
