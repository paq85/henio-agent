...
Training tasks (3):
    1. Create ~/greeting.txt with "Hello from Henio Agent"
...
TRAIN_TASKS = [
    {
        "prompt": "Create a file at ~/greeting.txt containing exactly the text: Hello from Henio Agent",
        "verify_path": "~/greeting.txt",
        "expected_content": "Hello from Henio Agent",
    },
    {
...
class TerminalTestEnv(HenioAgentBaseEnv):
    ...
    def config_init(cls) -> Tuple[TerminalTestEnvConfig, List[APIServerConfig]]:
        ...
        Claude for inference. API keys loaded from ~/henio-agent/.env.
        """
