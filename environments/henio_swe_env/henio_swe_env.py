"""
HenioSweEnv -- SWE-Bench Style Environment with Modal Sandboxes
...
Usage:
    ...
    vllm serve YourModel --tool-parser henio
    run-api
    python environments/henio_swe_env.py serve \\
    ...
    # Phase 2: VLLM server type (full-fidelity environment run)
    python environments/henio_swe_env.py serve \\
        --openai.base_url http://localhost:8000/v1 \\
        --openai.model_name YourModel \\
        --openai.server_type vllm \\
        --env.tool_call_parser henio \\
        --env.terminal_backend modal
"""
...
class HenioSweEnv(HenioAgentBaseEnv):
    ...
    name = "henio-swe"
    env_config_cls = HenioSweEnvConfig

    @classmethod
    def config_init(cls) -> Tuple[HenioSweEnvConfig, List[APIServerConfig]]:
        """
        ...
        env_config = HenioSweEnvConfig(
            ...
            # Atropos settings
            group_size=4,
            tokenizer_name="NousResearch/DeepHenio-3-Llama-3-3B-Preview",
            tool_call_parser="henio",
            steps_per_eval=50,
            total_steps=500,
            use_wandb=True,
            wandb_name="henio-swe",
        )

        server_configs = [
            APIServerConfig(
                base_url="http://localhost:8000/v1",
                model_name="NousResearch/DeepHenio-3-Llama-3-3B-Preview",
