...
The evaluate flow:
    ...
    2. evaluate()  -- Iterates over all runs sequentially through:
        a. rollout_and_score_eval()  -- Per-run agent loop
            ...
            - Runs HenioAgentLoop with terminal tool only
...
Requires: pip install henio-agent[yc-bench]
"""
...
from environments.agent_loop import HenioAgentLoop
...
class YCBenchEvalConfig(HenioAgentEnvConfig):
    """
    Configuration for the YC-Bench evaluation environment.

    Extends HenioAgentEnvConfig with YC-Bench-specific settings for
...
class YCBenchEvalEnv(HenioAgentBaseEnv):
    """
    ...
    a competing built-in agent loop). The HenioAgentLoop then drives the
    ...
    async def rollout_and_score_eval(self, eval_item: Dict[str, Any]) -> Dict:
        ...
        3. Runs HenioAgentLoop with terminal tool
        ...
        try:
            ...
            # LiteLLM), which would compete with our HenioAgentLoop.
