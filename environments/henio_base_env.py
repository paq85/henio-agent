"""
HenioAgentBaseEnv -- Abstract Base Environment for Henio-Agent + Atropos

Provides the Atropos integration plumbing that all henio-agent environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via HenioAgentLoop
...
# Ensure the henio-agent repo root is on sys.path so that imports like
...
# Load API keys from henio-agent/.env so all environments can access them
...
# Import henio-agent toolset infrastructure
...
class HenioAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for henio-agent Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of henio toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: henio, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
    ...
    default_result_size_chars: int = Field(
        ...
        "to sandbox. Results exceeding this are written to /tmp/henio-results/ "
...
class HenioAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for henio-agent Atropos integration.
    ...
    - Phase 1 (OpenAI server type): Uses server.chat_completion() directly.
      ...
      tokens. Good for SFT data gen, verifier testing, evaluation.
        ...
        - Phase 2 (VLLM server type): Uses ManagedServer for exact token IDs + logprobs
            ...
            from raw output. Full-fidelity evaluation and trajectory capability.
    ...
    name: Optional[str] = "henio-agent"
    env_config_cls = HenioAgentEnvConfig

    def __init__(
        self,
        config: HenioAgentEnvConfig,
        ...
        # Set terminal environment variables so henio tools pick them up.
        ...
        if config.distribution:
            ...
            if group_toolsets is None:
                logger.warning(
                    ...
                    "Set explicit enabled_toolsets for controlled environment runs."
                )
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY henio-agent tool (terminal, file, web,
