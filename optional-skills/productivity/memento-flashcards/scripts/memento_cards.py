...
Data file: $HENIO_HOME/skills/productivity/memento-flashcards/data/cards.json
"""
...
_HENIO_HOME = Path(os.environ.get("HENIO_HOME", Path.home() / ".henio"))
DATA_DIR = _HENIO_HOME / "skills" / "productivity" / "memento-flashcards" / "data"
...
def cmd_stats(args: argparse.Namespace) -> None:
    ...
    due_now = 0
    for card in data["cards"]:
        if card["status"] != "retired" and _parse_iso(card["next_review_at"]) <= now:
            due_now += 1
...
def main() -> None:
    ...
    p_delete = sub.add_parser("delete", help="Delete one card")
    p_delete.add_argument("--id", required=True)

    p_delete_collection = sub.add_parser("delete-collection", help="Delete all cards in a collection")
    p_delete_collection.add_argument("--collection", required=True)
