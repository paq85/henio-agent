#!/usr/bin/env python3
"""OpenClaw -> Henio migration helper.

This script migrates the parts of an OpenClaw user footprint that map cleanly
into Henio, archives selected unmapped docs for manual review, and reports
...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    "soul": {"label": "SOUL.md", "description": "Import the OpenClaw persona file into Henio."},
    "workspace-agents": {"label": "Workspace instructions", "description": "Copy workspace instructions into a chosen workspace."},
    "memory": {"label": "MEMORY.md", "description": "Import long-term memory entries into Henio memories."},
    "user-profile": {"label": "USER.md", "description": "Import user profile entries into Henio memories."},
    "messaging-settings": {"label": "Messaging settings", "description": "Import compatible messaging settings into .env."},
    "secret-settings": {"label": "Allowlisted secrets", "description": "Import a small allowlist of compatible secrets when enabled."},
    "command-allowlist": {"label": "Command allowlist", "description": "Merge exec approval patterns into config.yaml."},
    "skills": {"label": "User skills", "description": "Copy OpenClaw skills into skills/openclaw-imports/."},
    "tts-assets": {"label": "TTS assets", "description": "Copy compatible workspace TTS assets."},
    "discord-settings": {"label": "Discord settings", "description": "Import Discord bot token and allowlist into .env."},
    "slack-settings": {"label": "Slack settings", "description": "Import Slack bot/app tokens and allowlist into .env."},
    "whatsapp-settings": {"label": "WhatsApp settings", "description": "Import WhatsApp allowlist into .env."},
    "signal-settings": {"label": "Signal settings", "description": "Import Signal account, HTTP URL, and allowlist into .env."},
    "provider-keys": {"label": "Provider API keys", "description": "Import provider keys into .env when enabled."},
    "model-config": {"label": "Default model", "description": "Import the default model setting into config.yaml."},
    "tts-config": {"label": "TTS configuration", "description": "Import TTS provider and voice settings into config.yaml."},
    "shared-skills": {"label": "Shared skills", "description": "Copy shared OpenClaw skills into Henio."},
    "daily-memory": {"label": "Daily memory files", "description": "Merge workspace/memory entries into MEMORY.md."},
    "archive": {"label": "Archive unmapped docs", "description": "Archive compatible-but-unmapped docs for manual review."},
    "mcp-servers": {"label": "MCP servers", "description": "Import MCP server definitions from config."},
    "plugins-config": {"label": "Plugins configuration", "description": "Archive plugins config for manual review."},
    "cron-jobs": {"label": "Cron / scheduled tasks", "description": "Archive cron config or store for manual recreation via `henio cron`."},
    "hooks-config": {"label": "Hooks and webhooks", "description": "Archive hook configuration."},
    "agent-config": {"label": "Agent defaults and multi-agent setup", "description": "Import agent defaults and archive multi-agent specifics."},
    "gateway-config": {"label": "Gateway configuration", "description": "Archive or extract compatible gateway settings."},
    "session-config": {"label": "Session configuration", "description": "Import session reset policies."},
    "full-providers": {"label": "Full model provider definitions", "description": "Import custom provider definitions when possible."},
    "deep-channels": {"label": "Deep channel configuration", "description": "Import or archive advanced channel settings."},
    "browser-config": {"label": "Browser configuration", "description": "Import browser settings into config.yaml."},
    "tools-config": {"label": "Tools configuration", "description": "Import tool settings into config.yaml."},
    "approvals-config": {"label": "Approval rules", "description": "Import approval mode into config.yaml."},
    "memory-backend": {"label": "Memory backend configuration", "description": "Archive memory backend settings for manual review."},
    "skills-config": {"label": "Skills registry configuration", "description": "Archive per-skill registry settings."},
    "ui-identity": {"label": "UI and identity settings", "description": "Archive OpenClaw UI/identity settings."},
    "logging-config": {"label": "Logging and diagnostics", "description": "Archive logging and diagnostics settings."},
}

MIGRATION_PRESETS: Dict[str, set[str]] = {
    "user-data": {
        "soul", "workspace-agents", "memory", "user-profile", "messaging-settings",
        "command-allowlist", "skills", "tts-assets", "discord-settings",
        "slack-settings", "whatsapp-settings", "signal-settings", "model-config",
        "tts-config", "shared-skills", "daily-memory", "archive", "mcp-servers",
        "agent-config", "session-config", "browser-config", "tools-config",
        "approvals-config", "deep-channels", "full-providers", "plugins-config",
        "cron-jobs", "hooks-config", "memory-backend", "skills-config",
        "ui-identity", "logging-config", "gateway-config",
    },
    "full": set(MIGRATION_OPTION_METADATA),
}

_REBRAND_PATTERNS: List[Tuple[re.Pattern[str], str]] = [
    (re.compile(r"\bOpen[\s-]?Claw\b", re.IGNORECASE), "Henio"),
    (re.compile(r"\bClawdBot\b", re.IGNORECASE), "Henio"),
    (re.compile(r"\bMoltBot\b", re.IGNORECASE), "Henio"),
]
...
def resolve_selected_options(include: Optional[Sequence[str]] = None, exclude: Optional[Sequence[str]] = None, preset: Optional[str] = None) -> set[str]:
    ...
    if preset_name and preset_name not in MIGRATION_PRESETS:
        raise ValueError("Unknown migration preset: " + preset_name + ". Valid presets: " + ", ".join(sorted(MIGRATION_PRESETS)))
    unknown = (set(include_values) - {"all"} - valid) | (set(exclude_values) - {"all"} - valid)
    if unknown:
        raise ValueError("Unknown migration option(s): " + ", ".join(sorted(unknown)) + ". Valid options: " + ", ".join(sorted(valid)))
    if preset_name:
...
def dump_yaml_file(path: Path, data: Dict[str, Any]) -> None:
    if yaml is None:
        raise RuntimeError("PyYAML is required to update Henio config.yaml")
    ensure_parent(path)
...
def extract_markdown_entries(text: str) -> List[str]:
    ...
    def context_prefix() -> str:
        filtered = [heading for heading in headings if heading and not re.search(r"\b(MEMORY|USER|SOUL|AGENTS|TOOLS|IDENTITY)\.md\b", heading, re.I)]
    ...
    def flush_paragraph() -> None:
        ...
        entries.append(f"{prefix}: {text_block}" if prefix else text_block)
...
def merge_entries(existing: Sequence[str], incoming: Sequence[str], limit: int) -> Tuple[List[str], Dict[str, int], List[str]]:
...
def write_report(output_dir: Path, report: Dict[str, Any]) -> None:
    ...
    lines = [
        "# OpenClaw -> Henio Migration Report",
        "",
...
class Migrator:
    def __init__(
        ...
        if self.skill_conflict_mode not in SKILL_CONFLICT_MODES:
            raise ValueError("Unknown skill conflict mode: " + self.skill_conflict_mode)
        self.timestamp = datetime.now().strftime("%Y%m%dT%H%M%S")
        self.output_dir = output_dir or (target_root / "migration" / "openclaw" / self.timestamp if execute else None)
    ...
    def record(self, kind: str, source: Optional[Any], destination: Optional[Any], status: str, reason: str = "", **details: Any) -> None:
    ...
    def copy_file(self, source: Path, destination: Path, kind: str, transform: Optional[Any] = None) -> None:
        ...
        if self.execute:
            ...
            ensure_parent(destination)
            if transform:
                content = transform(read_text(source))
    ...
    def build_report(self) -> Dict[str, Any]:
        summary: Dict[str, int] = {"migrated": 0, "archived": 0, "skipped": 0, "conflict": 0, "error": 0}
        ...
        report = {
            ...
            "selection": {
                ...
                "preset": self.preset_name or None,
                "skill_conflict_mode": self.skill_conflict_mode,
                "available": [{"id": option_id, **meta} for option_id, meta in MIGRATION_OPTION_METADATA.items()],
                "presets": [{"id": preset_id, "selected": sorted(option_ids)} for preset_id, option_ids in MIGRATION_PRESETS.items()],
            },
            "summary": summary,
    ...
    def merge_env_values(self, additions: Dict[str, str], kind: str, source: Path) -> None:
        ...
        if self.execute:
            ...
            self.record(kind, source, destination, "migrated", backup=str(backup_path) if backup_path else "", added_keys=sorted(added.keys()), conflicting_keys=conflicts)
        else:
            self.record(kind, source, destination, "migrated", "Would merge env values", added_keys=sorted(added.keys()), conflicting_keys=conflicts)
    ...
    def migrate_workspace_agents(self) -> None:
        source = self.source_candidate(f"workspace/{WORKSPACE_INSTRUCTIONS_FILENAME}", f"workspace.default/{WORKSPACE_INSTRUCTIONS_FILENAME}")
        if source is None:
    ...
    def migrate_memory(self, source: Optional[Path], destination: Path, limit: int, kind: str) -> None:
        ...
        if self.execute:
            ...
            self.record(kind, source, destination, "migrated", backup=str(backup_path) if backup_path else "", overflow_preview=overflowed[:5], **details)
        else:
    ...
    def migrate_messaging_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        workspace = (config.get("agents", {}) or {}).get("defaults", {}).get("workspace")
        if isinstance(workspace, str) and workspace.strip():
            ...
            try:
                ...
                try:
                    inside_source = str(Path(ws_path).resolve()).startswith(str(self.source_root.resolve()))
                except Exception:
                    inside_source = False
        ...
        if additions:
            self.merge_env_values(additions, "messaging-settings", source_path)
        else:
            self.record("messaging-settings", source_path, self.target_root / ".env", "skipped", "No Henio-compatible messaging settings found")
    ...
    def handle_secret_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if self.migrate_secrets:
            ...
            return
        self.record("secret-settings", self.source_root / "openclaw.json", self.target_root / ".env", "skipped", "Secret migration disabled. Re-run with --migrate-secrets to import allowlisted secrets.", supported_targets=sorted(SUPPORTED_SECRET_TARGETS))

    def migrate_secret_settings(self, config: Dict[str, Any]) -> None:
        additions: Dict[str, str] = {}
        tg_cfg = config.get("channels", {}).get("telegram", {})
        telegram_token = tg_cfg.get("botToken") if isinstance(tg_cfg, dict) else None
        if isinstance(telegram_token, str) and telegram_token.strip():
            additions["TELEGRAM_BOT_TOKEN"] = telegram_token.strip()
        if additions:
            self.merge_env_values(additions, "secret-settings", self.source_root / "openclaw.json")
        else:
            self.record("secret-settings", self.source_root / "openclaw.json", self.target_root / ".env", "skipped", "No allowlisted Henio-compatible secrets found", supported_targets=sorted(SUPPORTED_SECRET_TARGETS))
    ...
    def migrate_discord_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(discord, dict):
            ...
            allow_from = discord.get("allowFrom") or []
            if isinstance(allow_from, list):
    ...
    def migrate_slack_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(slack, dict):
            ...
            allow_from = slack.get("allowFrom") or []
            if isinstance(allow_from, list):
                users = [str(u).strip() for u in allow_from if str(u).strip()]
                if users:
    ...
    def migrate_whatsapp_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(whatsapp, dict):
            allow_from = whatsapp.get("allowFrom") or []
            if isinstance(allow_from, list):
                users = [str(u).strip() for u in allow_from if str(u).strip()]
                if users:
    ...
    def migrate_signal_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(signal, dict):
            ...
            allow_from = signal.get("allowFrom") or []
            if isinstance(allow_from, list):
                users = [str(u).strip() for u in allow_from if str(u).strip()]
                if users:
    ...
    def handle_provider_keys(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if not self.migrate_secrets:
            self.record("provider-keys", self.source_root / "openclaw.json", self.target_root / ".env", "skipped", "Secret migration disabled. Re-run with --migrate-secrets to import provider API keys.", supported_targets=sorted(SUPPORTED_SECRET_TARGETS))
            return
            ...
            for provider_name, provider_cfg in providers.items():
                ...
                if not env_var:
                    ...
                    if name_lower == "openrouter":
                        ...
                        env_var = "ANTHROPIC_API_KEY"
                if env_var:
                    secret_additions[env_var] = api_key.strip()
        if secret_additions:
            self.merge_env_values(secret_additions, "provider-keys", self.source_root / "openclaw.json")
        else:
            self.record("provider-keys", self.source_root / "openclaw.json", self.target_root / ".env", "skipped", "No provider API keys found", supported_targets=sorted(SUPPORTED_SECRET_TARGETS))
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if yaml is None:
            ...
            return
        henio_config = load_yaml_file(destination)
        current_model = henio_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = henio_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str.strip()
                henio_config["model"] = existing_model
            else:
                henio_config["model"] = {"default": model_str.strip()}
            dump_yaml_file(destination, henio_config)
            self.record("model-config", source_path, destination, "migrated", backup=str(backup_path) if backup_path else "", model=model_str.strip())
        else:
            self.record("model-config", source_path, destination, "migrated", "Would set model", model=model_str.strip())
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        elevenlabs = tts.get("elevenlabs") or {}
        if isinstance(elevenlabs, dict):
            settings: Dict[str, str] = {}
            voice_id = elevenlabs.get("voiceId")
            model_id = elevenlabs.get("modelId")
            if isinstance(voice_id, str) and voice_id.strip():
                settings["voice_id"] = voice_id.strip()
            if isinstance(model_id, str) and model_id.strip():
                settings["model_id"] = model_id.strip()
            if settings:
                tts_data["elevenlabs"] = settings
        openai_tts = tts.get("openai") or {}
        if isinstance(openai_tts, dict):
            settings: Dict[str, str] = {}
            model = openai_tts.get("model") or openai_tts.get("modelId")
            voice = openai_tts.get("voice")
            if isinstance(model, str) and model.strip():
                settings["model"] = model.strip()
            if isinstance(voice, str) and voice.strip():
                settings["voice"] = voice.strip()
            if settings:
                tts_data["openai"] = settings
        edge_tts = tts.get("edge") or tts.get("microsoft") or {}
        if isinstance(edge_tts, dict):
            voice = edge_tts.get("voice")
            if isinstance(voice, str) and voice.strip():
                tts_data["edge"] = {"voice": voice.strip()}
        if not tts_data:
            self.record("tts-config", source_path, destination, "skipped", "No compatible TTS settings found")
            return
        henio_config = load_yaml_file(destination)
        existing_tts = henio_config.get("tts", {}) if isinstance(henio_config.get("tts"), dict) else {}
        if self.execute:
            backup_path = self.maybe_backup(destination)
        ...
        for skill_dir in skill_dirs:
            ...
            if destination.exists():
                if self.skill_conflict_mode == "skip":
                    self.record("skill" if kind_label == "skills" else kind_label, skill_dir, destination, "conflict", "Destination skill already exists")
                    continue
            ...
            if self.execute:
                ...
                self.record("skill" if kind_label == "skills" else kind_label, skill_dir, final_destination, "migrated", **details)
            else:
                if final_destination != destination:
                    self.record("skill" if kind_label == "skills" else kind_label, skill_dir, final_destination, "migrated", f"Would copy {desc} directory under a renamed folder", renamed_from=str(destination))
                else:
                    self.record("skill" if kind_label == "skills" else kind_label, skill_dir, final_destination, "migrated", f"Would copy {desc} directory")
    ...
    def migrate_skills(self) -> None:
        ...
        if not source_root or not source_root.exists():
            ...
            return
        self._import_skill_directory(source_root, "skills", "skill")

    def migrate_shared_skills(self) -> None:
        skill_sources = [
            (self.source_root / "skills", "shared-skills", "managed skills"),
            (Path.home() / ".agents" / "skills", "shared-skills", "personal cross-project skills"),
            (self.source_root / "workspace" / ".agents" / "skills", "shared-skills", "project-level shared skills"),
            (self.source_root / "workspace.default" / ".agents" / "skills", "shared-skills", "project-level shared skills"),
        ]
        found_any = False
    ...
    def migrate_daily_memory(self) -> None:
        ...
        for md_file in md_files:
            all_incoming.extend(extract_markdown_entries(read_text(md_file)))
        if not all_incoming:
        ...
        if self.execute:
            ...
            ensure_parent(destination)
            destination.write_text(ENTRY_DELIMITER.join(merged) + ("\n" if merged else ""), encoding="utf-8")
            self.record("daily-memory", source_dir, destination, "migrated", backup=str(backup_path) if backup_path else "", overflow_preview=overflowed[:5], **details)
        else:
    ...
    def migrate_command_allowlist(self) -> None:
        ...
        if not destination.exists():
            self.record("command-allowlist", source, destination, "skipped", "Henio config.yaml does not exist yet")
            return
        ...
        if self.execute:
            ...
            self.record("command-allowlist", source, destination, "migrated", backup=str(backup_path) if backup_path else "", added_patterns=added)
        else:
    ...
    def archive_docs(self) -> None:
        ...
        for candidate in candidates:
            if candidate:
                self.archive_path(candidate, reason="No direct Henio destination; archived for manual review")
    ...
    def migrate_cron_jobs(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if cron:
            ...
            if self.archive_dir:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived", "Cron config archived. Use 'henio cron' to recreate jobs manually.")
        if cron_store.is_dir() and self.archive_dir:
            ...
            self.record("cron-jobs", str(cron_store), str(dest_cron), "archived", "Cron job store archived")
        if not found_any:
    ...
    def generate_migration_notes(self) -> None:
        if not self.output_dir:
            return
        notes = [
            "# OpenClaw -> Henio Migration Notes",
            "",
            "This document lists items that require manual attention after migration.",
            "",
            "## PM2 / External Processes",
            "",
            "Your PM2 processes (Discord bots, Telegram bots, etc.) are NOT affected by this migration.",
            "No action needed for PM2-managed processes.",
            "",
        ]
        archived = [item for item in self.items if item.status == "archived"]
        if archived:
            notes.extend([
                "## Archived Items (Manual Review Needed)",
                "",
                "These OpenClaw configurations were archived because they don't have a direct 1:1 mapping in Henio.",
                "",
            ])
            for item in archived:
                notes.append(f"- **{item.kind}**: `{item.destination}` -- {item.reason}")
            notes.append("")
        conflicts = [item for item in self.items if item.status == "conflict"]
        if conflicts:
            notes.extend([
                "## Conflicts (Existing Henio Config Not Overwritten)",
                "",
                "These items already existed in your Henio config. Re-run with `--overwrite` to force, or merge manually:",
                "",
            ])
            for item in conflicts:
                notes.append(f"- **{item.kind}**: {item.reason}")
            notes.append("")
        has_cron_config_archive = any(item.kind == "cron-jobs" and item.status == "archived" and item.destination and item.destination.endswith("cron-config.json") for item in self.items)
        has_cron_store_archive = any(item.kind == "cron-jobs" and item.status == "archived" and item.destination and item.destination.endswith("cron-store") for item in self.items)
        notes.extend([
            "## IMPORTANT: Archive the OpenClaw Directory",
            "",
            "After migration, your OpenClaw directory still exists on disk with workspace state files.",
            "If Henio discovers these directories, it may read or write stale state instead of Henio state.",
            "",
            "## Henio-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `henio setup` to configure any remaining settings",
            "- Review `~/.henio/config.yaml` for any adjustments",
        ])
        if has_cron_config_archive:
            notes.append("- Run `henio cron` to recreate scheduled tasks (see archive/cron-config.json)")
        elif has_cron_store_archive:
            notes.append("- Run `henio cron` to recreate scheduled tasks (see archived cron-store)")
        has_skills = any(item.kind in ("skill", "skills") and item.status == "migrated" for item in self.items)
        if has_skills:
            notes.extend([
                "",
                "## Imported Skills",
                "",
                "Imported skills require a new session to take effect. After migration, restart your agent or start a new chat session, then run `/skills` to verify they loaded correctly.",
            ])
    ...
    def migrate(self) -> Dict[str, Any]:
        ...
        self.run_if_selected("memory", lambda: self.migrate_memory(self.source_candidate("workspace/MEMORY.md", "workspace.default/MEMORY.md"), self.target_root / "memories" / "MEMORY.md", self.memory_limit, kind="memory"))
        self.run_if_selected("user-profile", lambda: self.migrate_memory(self.source_candidate("workspace/USER.md", "workspace.default/USER.md"), self.target_root / "memories" / "USER.md", self.user_limit, kind="user-profile"))
        ...
        self.run_if_selected("tts-assets", lambda: self.record("tts-assets", None, self.target_root / "tts", "skipped", "Source directory not found"))
        self.run_if_selected("archive", self.archive_docs)
        for option_id in (
            "mcp-servers", "plugins-config", "cron-jobs", "hooks-config", "agent-config", "gateway-config",
            "session-config", "full-providers", "deep-channels", "browser-config", "tools-config",
            "approvals-config", "memory-backend", "skills-config", "ui-identity", "logging-config",
        ):
            if option_id == "cron-jobs":
                self.run_if_selected(option_id, lambda cfg=config: self.migrate_cron_jobs(cfg))
            else:
                self.run_if_selected(option_id, lambda opt=option_id: self.record(opt, None, None, "skipped", "No compatible data found"))
        self.generate_migration_notes()
        return self.build_report()


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate OpenClaw user state into Henio.")
    parser.add_argument("--source", default=str(Path.home() / ".openclaw"), help="OpenClaw home directory")
    parser.add_argument("--target", default=str(Path.home() / ".henio"), help="Henio home directory")
    ...
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing targets after backing them up")
    parser.add_argument("--migrate-secrets", action="store_true", help="Import an allowlist of compatible secrets into the target env file")
    parser.add_argument("--skill-conflict", choices=sorted(SKILL_CONFLICT_MODES), default="skip", help="How to handle imported skill directory conflicts")
    parser.add_argument("--preset", choices=sorted(MIGRATION_PRESETS), help="Apply a named migration preset")
    parser.add_argument("--include", action="append", default=[], help="Comma-separated migration option ids to include")
    parser.add_argument("--exclude", action="append", default=[], help="Comma-separated migration option ids to skip")
