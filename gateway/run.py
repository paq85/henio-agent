...
# Resolve Henio home directory (respects HENIO_HOME override)
from henio_constants import get_henio_home
from utils import atomic_yaml_write, is_truthy_value
_henio_home = get_henio_home()

# Load environment variables from ~/.henio/.env first.
# User-managed env files should override stale shell exports on restart.
from dotenv import load_dotenv  # backward-compat for tests that monkeypatch this symbol
from henio_cli.env_loader import load_henio_dotenv
_env_path = _henio_home / '.env'
load_henio_dotenv(henio_home=_henio_home, project_env=Path(__file__).resolve().parents[1] / '.env')

# Bridge config.yaml values into the environment so os.getenv() picks them up.
# config.yaml is authoritative for terminal settings — overrides .env.
_config_path = _henio_home / 'config.yaml'
if _config_path.exists():
    try:
        ...
        from henio_cli.config import _expand_env_vars
        ...
        if _agent_cfg and isinstance(_agent_cfg, dict):
            if "max_turns" in _agent_cfg:
                os.environ["HENIO_MAX_ITERATIONS"] = str(_agent_cfg["max_turns"])
            # Bridge agent.gateway_timeout → HENIO_AGENT_TIMEOUT env var.
            # Env var from .env takes precedence (already in os.environ).
            if "gateway_timeout" in _agent_cfg and "HENIO_AGENT_TIMEOUT" not in os.environ:
                os.environ["HENIO_AGENT_TIMEOUT"] = str(_agent_cfg["gateway_timeout"])
            if "gateway_timeout_warning" in _agent_cfg and "HENIO_AGENT_TIMEOUT_WARNING" not in os.environ:
                os.environ["HENIO_AGENT_TIMEOUT_WARNING"] = str(_agent_cfg["gateway_timeout_warning"])
            if "restart_drain_timeout" in _agent_cfg and "HENIO_RESTART_DRAIN_TIMEOUT" not in os.environ:
                os.environ["HENIO_RESTART_DRAIN_TIMEOUT"] = str(_agent_cfg["restart_drain_timeout"])
        _display_cfg = _cfg.get("display", {})
        if _display_cfg and isinstance(_display_cfg, dict):
            if "busy_input_mode" in _display_cfg and "HENIO_GATEWAY_BUSY_INPUT_MODE" not in os.environ:
                os.environ["HENIO_GATEWAY_BUSY_INPUT_MODE"] = str(_display_cfg["busy_input_mode"])
        # Timezone: bridge config.yaml → HENIO_TIMEZONE env var.
        # HENIO_TIMEZONE from .env takes precedence (already in os.environ).
        _tz_cfg = _cfg.get("timezone", "")
        if _tz_cfg and isinstance(_tz_cfg, str) and "HENIO_TIMEZONE" not in os.environ:
            os.environ["HENIO_TIMEZONE"] = _tz_cfg.strip()
        # Security settings
        _security_cfg = _cfg.get("security", {})
        if isinstance(_security_cfg, dict):
            _redact = _security_cfg.get("redact_secrets")
            if _redact is not None:
                os.environ["HENIO_REDACT_SECRETS"] = str(_redact).lower()
    except Exception:
        pass  # Non-fatal; gateway can still run with .env values

# Apply IPv4 preference if configured (before any HTTP clients are created).
try:
    from henio_constants import apply_ipv4_preference
...
try:
    from henio_cli.config import print_config_warnings
    print_config_warnings()
except Exception:
    pass

# Gateway runs in quiet mode - suppress debug output and use cwd directly (no temp dirs)
os.environ["HENIO_QUIET"] = "1"

# Enable interactive exec approval for dangerous commands on messaging platforms
os.environ["HENIO_EXEC_ASK"] = "1"
...
def _expand_whatsapp_auth_aliases(identifier: str) -> set:
    ...
    session_dir = _henio_home / "whatsapp" / "session"
    resolved = set()
    queue = [normalized]
...
def _resolve_runtime_agent_kwargs() -> dict:
    ...
    from henio_cli.runtime_provider import (
    ...
    try:
        runtime = resolve_runtime_provider(
            requested=os.getenv("HENIO_INFERENCE_PROVIDER"),
        )
            ...
            for skill_md in skills_dir.rglob("SKILL.md"):
                ...
                if name == normalized and name in disabled:
                    return (
                        ...
                        f"Enable it with: `henio skills config`"
                    )
            ...
            for skill_md in optional_dir.rglob("SKILL.md"):
                ...
                if name == normalized:
                    ...
                    return (
                        ...
                        f"Install it with: `henio skills install {install_path}`"
                    )
    except Exception:
        pass
    return None
...
def _load_gateway_config() -> dict:
    """Load and parse ~/.henio/config.yaml, returning {} on any error."""
    try:
        config_path = _henio_home / 'config.yaml'
        if config_path.exists():
            import yaml
            with open(config_path, 'r', encoding='utf-8') as f:
                return yaml.safe_load(f) or {}
    except Exception:
        logger.debug("Could not load gateway config from %s", _henio_home / 'config.yaml')
    return {}
...
def _resolve_henio_bin() -> Optional[list[str]]:
    """Resolve the Henio update command as argv parts.

    Tries in order:
    1. ``shutil.which("henio")`` — standard PATH lookup
    2. ``sys.executable -m henio_cli.main`` — fallback when Henio is running
       from a venv/module invocation and the ``henio`` shim is not on PATH

    Returns argv parts ready for quoting/joining, or ``None`` if neither works.
    """
    import shutil

    henio_bin = shutil.which("henio")
    if henio_bin:
        return [henio_bin]
    ...
    try:
        ...
        if importlib.util.find_spec("henio_cli") is not None:
            return [sys.executable, "-m", "henio_cli.main"]
    except Exception:
        pass
...
class GatewayRunner:
    ...
    def __init__(self, config: Optional[GatewayConfig] = None):
        ...
        pairing_dir = _henio_home / "platforms" / "pairing"
        legacy_pairing_dir = _henio_home / "pairing"
        if legacy_pairing_dir.exists():
            pairing_dir = legacy_pairing_dir
        self.pairing_store = PairingStore(pairing_dir=pairing_dir)
    ...
    def _has_setup_skill(self) -> bool:
        """Check if the henio-agent-setup skill is installed."""
        try:
            from tools.skill_manager_tool import _find_skill
            return _find_skill("henio-agent-setup") is not None
        except Exception:
            return False

    # -- Voice mode persistence ------------------------------------------

    _VOICE_MODE_PATH = _henio_home / "gateway_voice_mode.json"
    ...
    def _resolve_turn_agent_config(self, user_message: str, model: str, runtime_kwargs: dict) -> dict:
        ...
        from henio_cli.models import resolve_fast_mode_overrides
    ...
    def _load_prefill_messages() -> List[Dict[str, Any]]:
        ...
        Checks HENIO_PREFILL_MESSAGES_FILE env var first, then falls back to
        the prefill_messages_file key in ~/.henio/config.yaml.
        Relative paths are resolved from ~/.henio/.
        """
        import json as _json
        file_path = os.getenv("HENIO_PREFILL_MESSAGES_FILE", "")
        if not file_path:
            try:
                import yaml as _y
                cfg_path = _henio_home / "config.yaml"
                if cfg_path.exists():
    ...
    def _load_ephemeral_system_prompt() -> str:
        ...
        Checks HENIO_EPHEMERAL_SYSTEM_PROMPT env var first, then falls back to
        agent.system_prompt in ~/.henio/config.yaml.
        """
        prompt = os.getenv("HENIO_EPHEMERAL_SYSTEM_PROMPT", "")
        if prompt:
            return prompt
        try:
            import yaml as _y
    ...
    def _load_reasoning_config() -> dict | None:
        ...
        default (medium).
        """
        from henio_constants import parse_reasoning_effort
        effort = ""
        try:
            import yaml as _y
            cfg_path = _henio_home / "config.yaml"
    ...
    def _load_busy_input_mode() -> str:
        ...
        mode = os.getenv("HENIO_GATEWAY_BUSY_INPUT_MODE", "").strip().lower()
        if not mode:
            try:
                import yaml as _y
                cfg_path = _henio_home / "config.yaml"
    ...
    def _load_restart_drain_timeout() -> float:
        ...
        raw = os.getenv("HENIO_RESTART_DRAIN_TIMEOUT", "").strip()
        if not raw:
            try:
                import yaml as _y
                cfg_path = _henio_home / "config.yaml"
    ...
    def _load_background_notifications_mode() -> str:
        ...
        """
        mode = os.getenv("HENIO_BACKGROUND_NOTIFICATIONS", "")
        if not mode:
            try:
                import yaml as _y
                cfg_path = _henio_home / "config.yaml"
    ...
    async def _drain_active_agents(self, timeout: float) -> tuple[Dict[str, Any], bool]:
        ...
        if not getattr(self, "_running_agents", {}):
        ...
        while getattr(self, "_running_agents", {}) and asyncio.get_running_loop().time() < deadline:
            _maybe_update_status()
            await asyncio.sleep(0.1)
        timed_out = bool(getattr(self, "_running_agents", {}))
        _maybe_update_status(force=True)
    ...
    def _finalize_shutdown_agents(self, active_agents: Dict[str, Any]) -> None:
        for agent in active_agents.values():
            try:
                from henio_cli.plugins import invoke_hook as _invoke_hook
                _invoke_hook(
                    "on_session_finalize",
    ...
    async def _launch_detached_restart_command(self) -> None:
        ...
        henio_cmd = _resolve_henio_bin()
        if not henio_cmd:
            logger.error("Could not locate henio binary for detached /restart")
            return

        current_pid = os.getpid()
        cmd = " ".join(shlex.quote(part) for part in henio_cmd)
        shell_cmd = (
    ...
    async def start(self) -> bool:
        ...
        """
        logger.info("Starting Henio Gateway...")
        logger.info("Session storage: %s", self.config.sessions_dir)
        try:
            from henio_cli.profiles import get_active_profile_name
        ...
        if not _any_allowlist and not _allow_all:
            logger.warning(
                ...
                "Set GATEWAY_ALLOW_ALL_USERS=true in ~/.henio/.env to allow open access, "
        ...
        if not notified and any(
            path.exists()
            for path in (
                _henio_home / ".update_pending.json",
                _henio_home / ".update_pending.claimed.json",
            )
        ):
    ...
    async def stop(
        ...
        *,
        ...
        if existing_stop_task is not None and inspect.isawaitable(existing_stop_task):
        ...
        async def _stop_impl() -> None:
            ...
            raw_timeout = getattr(self, "_restart_drain_timeout", 0.0)
            try:
            ...
            if timed_out:
                ...
                while getattr(self, "_running_agents", {}) and asyncio.get_running_loop().time() < interrupt_deadline:
                    self._update_runtime_status("draining")
                    await asyncio.sleep(0.1)

            if self._restart_requested and self._restart_detached:
                try:
                    await GatewayRunner._launch_detached_restart_command(self)
                except Exception as e:
            ...
            for _task in list(getattr(self, "_background_tasks", set()) or set()):
                if _task is getattr(self, "_stop_task", None):
                    continue
                _task.cancel()
            if isinstance(getattr(self, "_background_tasks", None), set):
                self._background_tasks.clear()

            if isinstance(getattr(self, "adapters", None), dict):
                self.adapters.clear()
            if isinstance(getattr(self, "_running_agents", None), dict):
                self._running_agents.clear()
            if isinstance(getattr(self, "_pending_messages", None), dict):
                self._pending_messages.clear()
            if isinstance(getattr(self, "_pending_approvals", None), dict):
            ...
            self._draining = False
            self._update_runtime_status("stopped", getattr(self, "_exit_reason", None))
    ...
    def _create_adapter(
        ...
        if platform == Platform.TELEGRAM:
            ...
            if not check_slack_requirements():
                logger.warning("Slack: slack-bolt not installed. Run: pip install 'henio-agent[slack]'")
                return None
    ...
    async def _handle_message(self, event: MessageEvent) -> Optional[str]:
        ...
        if getattr(self, "_running_agents", None) is None:
            self._running_agents = {}
        if getattr(self, "_running_agents_ts", None) is None:
            self._running_agents_ts = {}
        if getattr(self, "_pending_messages", None) is None:
            self._pending_messages = {}
        if getattr(self, "_pending_approvals", None) is None:
            self._pending_approvals = {}
        if not hasattr(self, "_draining"):
            self._draining = False
        if not hasattr(self, "_restart_requested"):
            self._restart_requested = False
        if not hasattr(self, "_busy_input_mode"):
        ...
        if getattr(event, "internal", False):
            ...
            if source.user_id is None and not authorized:
                    ...
                    if code:
                        ...
                        if adapter:
                            await adapter.send(
                                ...
                                f"`henio pairing approve {platform_name} {code}`"
                            )
                    else:
                        adapter = self.adapters.get(source.platform)
                        if adapter:
        ...
        if _update_prompts.get(_quick_key):
            ...
            if cmd in ("approve", "yes"):
                ...
                response_text = raw
            if response_text:
                response_path = _henio_home / ".update_response"
                try:
        ...
        _raw_stale_timeout = float(os.getenv("HENIO_AGENT_TIMEOUT", 1800))
        ...
        if _quick_key in self._running_agents:
            ...
            from henio_cli.commands import resolve_command as _resolve_cmd_inner
        ...
        # in henio_cli/commands.py — no hardcoded set to maintain here.
        from henio_cli.commands import GATEWAY_KNOWN_COMMANDS, resolve_command as _resolve_cmd
        ...
        if command:
            try:
                from henio_cli.plugins import get_plugin_command_handler
                # Normalize underscores to hyphens so Telegram's underscored
                # autocomplete form matches plugin commands registered with
                # hyphens. See henio_cli/commands.py:_build_telegram_menu.
                    ...
                    if _stt_adapter:
                        try:
                            _stt_msg = (
                                ...
                                "(`pip install faster-whisper` in the Henio venv) "
                                "and set `stt.enabled: true` in config.yaml, "
                                "then /restart the gateway."
                            )
                            if self._has_setup_skill():
                                _stt_msg += "\n\nFor full setup instructions, type: `/skill henio-agent-setup`"
    ...
    async def _handle_message_with_agent(self, event, source, _quick_key: str):
        ...
        if history and len(history) >= 4:
            ...
            _hyg_data = {}
            try:
                _hyg_cfg_path = _henio_home / "config.yaml"
            ...
            if not os.getenv(env_key):
                ...
                if adapter:
                    await adapter.send(
                        ...
                        f"A home channel is where Henio delivers cron job results "
    ...
    async def _handle_reset_command(self, event: MessageEvent) -> str:
        ...
        try:
            from henio_cli.tips import get_random_tip
            _tip_line = f"\n✦ Tip: {get_random_tip()}"
        except Exception:
            _tip_line = ""
    ...
    async def _handle_profile_command(self, event: MessageEvent) -> str:
        ...
        from henio_constants import get_henio_home, display_henio_home
        from pathlib import Path

        home = get_henio_home()
        display = display_henio_home()

        # Detect profile name from HENIO_HOME path
        # Profile paths look like: ~/.henio/profiles/<name>
        profiles_parent = Path.home() / ".henio" / "profiles"
        try:
    ...
    async def _handle_help_command(self, event: MessageEvent) -> str:
        ...
        from henio_cli.commands import gateway_help_lines
        lines = [
            "📖 **Henio Commands**\n",
            *gateway_help_lines(),
        ]
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        """
        import yaml
        from henio_cli.model_switch import (
            switch_model as _switch_model, parse_model_flags,
            list_authenticated_providers,
        )
        from henio_cli.providers import get_label
        ...
        if persist_global:
            try:
                ...
                from henio_cli.config import save_config
                save_config(cfg)
            except Exception as e:
    ...
    async def _handle_provider_command(self, event: MessageEvent) -> str:
        ...
        if current_provider == "auto":
            try:
                from henio_cli.auth import resolve_provider as _resolve_provider
        ...
        lines.append("Setup: `henio setup`")
        return "\n".join(lines)
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        if not personalities:
            return "No personalities configured in `~/.henio/config.yaml`"
        ...
        try:
            ...
            if "pynacl" in err_lower or "nacl" in err_lower or "davey" in err_lower:
                return (
                    ...
                    "Install or reinstall Henio with the messaging extra, e.g. "
                    "`pip install henio-agent[messaging]`."
                )
    ...
    async def _send_voice_reply(self, event: MessageEvent, text: str) -> None:
        ...
        try:
            ...
            audio_path = os.path.join(
                tempfile.gettempdir(), "henio_voice",
    ...
    async def _handle_rollback_command(self, event: MessageEvent) -> str:
        ...
        try:
            import yaml as _y
            _cfg_path = _henio_home / "config.yaml"
            if _cfg_path.exists():
    ...
    async def _run_background_task(
        ...
        try:
            ...
            from henio_cli.tools_config import _get_platform_tools
            enabled_toolsets = sorted(_get_platform_tools(user_config, platform_key))

            pr = self._provider_routing
            max_iterations = int(os.getenv("HENIO_MAX_ITERATIONS", "90"))
    ...
    async def _handle_fast_command(self, event: MessageEvent) -> str:
        ...
        import yaml
        from henio_cli.models import model_supports_fast_mode
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update Henio Agent to the latest version.

        Spawns ``henio update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``henio update`` may trigger. Marker
        ...
        import subprocess
        from datetime import datetime
        from henio_cli.config import is_managed, format_managed_message

        # Block non-messaging platforms (API server, webhooks, ACP)
        platform = event.source.platform
        if platform not in self._UPDATE_ALLOWED_PLATFORMS:
            return "✗ /update is only available from messaging platforms. Run `henio update` from the terminal."

        if is_managed():
            return f"✗ {format_managed_message('update Henio Agent')}"
        ...
        if not henio_cmd:
            return (
                "✗ Could not locate the `henio` command. "
                "Henio is running, but the update command could not find the "
                "executable on PATH or via the current Python interpreter. "
                "Try running `henio update` manually in your terminal."
            )

        pending_path = _henio_home / ".update_pending.json"
        output_path = _henio_home / ".update_output.txt"
        exit_code_path = _henio_home / ".update_exit_code"
        session_key = self._session_key_for_source(event.source)
        pending = {
        ...
        # Spawn `henio update --gateway` detached so it survives gateway restart.
        ...
        henio_cmd_str = " ".join(shlex.quote(part) for part in henio_cmd)
        update_cmd = (
            f"PYTHONUNBUFFERED=1 {henio_cmd_str} update --gateway"
        ...
        self._schedule_update_notification_watch()
        return "⚕ Starting Henio update… I'll stream progress here."
    ...
    async def _watch_update_progress(
        ...
        """Watch ``henio update --gateway``, streaming output + forwarding prompts.
        ...
        pending_path = _henio_home / ".update_pending.json"
        claimed_path = _henio_home / ".update_pending.claimed.json"
        output_path = _henio_home / ".update_output.txt"
        exit_code_path = _henio_home / ".update_exit_code"
        prompt_path = _henio_home / ".update_prompt.json"
        ...
        while loop.time() < deadline:
            ...
            if exit_code_path.exists():
                ...
                try:
                    ...
                    if exit_code == 0:
                        await adapter.send(chat_id, "✅ Henio update finished.")
                    else:
                        await adapter.send(chat_id, "❌ Henio update failed (exit code {}).".format(exit_code))
                ...
                (_henio_home / ".update_response").unlink(missing_ok=True)
        ...
        if not exit_code_path.exists():
            ...
            await _flush_buffer()
            try:
                await adapter.send(chat_id, "❌ Henio update timed out after 30 minutes.")
            except Exception:
                pass
            for p in (pending_path, claimed_path, output_path,
                      exit_code_path, prompt_path):
            ...
            if adapter and chat_id:
                ...
                if output:
                    if len(output) > 3500:
                        output = "…" + output[-3500:]
                    if exit_code == 0:
                        msg = f"✅ Henio update finished.\n\n```\n{output}\n```"
                    else:
                        msg = f"❌ Henio update failed.\n\n```\n{output}\n```"
                else:
                    if exit_code == 0:
                        msg = "✅ Henio update finished successfully."
                    else:
                        msg = "❌ Henio update failed. Check the gateway logs or run `henio update` manually for details."
    ...
    async def _enrich_message_with_transcription(
        ...
        if not getattr(self.config, "stt_enabled", True):
            ...
            if self._has_setup_skill():
                disabled_note += (
                    " You have a skill called henio-agent-setup that can help "
                    "users configure Henio features including voice, tools, and more."
                )
            disabled_note += "]"
            if user_text:
            ...
            try:
                ...
                if result["success"]:
                    ...
                    if (
                        ...
                        if self._has_setup_skill():
                            _no_stt_note += (
                                " You have a skill called henio-agent-setup "
                                "that can help users configure Henio features "
    ...
    async def _run_agent(
        ...
        progress_mode = (
            _resolved_tp
            or os.getenv("HENIO_TOOL_PROGRESS_MODE")
            or "all"
        )
        ...
        def run_sync():
            ...
            os.environ["HENIO_SESSION_KEY"] = session_key or ""
        ...
        try:
            ...
            # HENIO_AGENT_TIMEOUT env var (env var takes precedence).
            # Default 1800s (30 min inactivity).  0 = unlimited.
            _agent_timeout_raw = float(os.getenv("HENIO_AGENT_TIMEOUT", 1800))
            _agent_timeout = _agent_timeout_raw if _agent_timeout_raw > 0 else None
            _agent_warning_raw = float(os.getenv("HENIO_AGENT_TIMEOUT_WARNING", 900))
            ...
            if pending and pending.strip().startswith("/"):
                ...
                if _pending_cmd_word:
                    try:
                        from henio_cli.commands import resolve_command as _rc_pending
...
def _start_cron_ticker(stop_event: threading.Event, adapters=None, loop=None, interval: int = 60):
    ...
    needing a separate `henio cron daemon` or system cron entry.
...
async def start_gateway(config: Optional[GatewayConfig] = None, replace: bool = False, verbosity: Optional[int] = 0) -> bool:
    ...
    # Prevent two gateways from running under the same HENIO_HOME.
    # The PID file is scoped to HENIO_HOME, so future multi-profile
    # setups (each profile using a distinct HENIO_HOME) will naturally
    ...
    if existing_pid is not None and existing_pid != os.getpid():
        if replace:
            ...
            henio_home = str(get_henio_home())
            logger.error(
                "Another gateway instance is already running (PID %d, HENIO_HOME=%s). "
                "Use 'henio gateway restart' to replace it, or 'henio gateway stop' first.",
                existing_pid, henio_home,
            )
            print(
                f"\n❌ Gateway already running (PID {existing_pid}).\n"
                f"   Use 'henio gateway restart' to replace it,\n"
                f"   or 'henio gateway stop' to kill it first.\n"
                f"   Or use 'henio gateway run --replace' to auto-replace.\n"
            )
            return False
    ...
    from henio_logging import setup_logging
    setup_logging(henio_home=_henio_home, mode="gateway")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Henio Gateway - Multi-platform messaging")
