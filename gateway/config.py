...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_henio_home() / "sessions")
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        sessions_dir = get_henio_home() / "sessions"
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.henio/config.yaml (primary user-facing config)
    3. ~/.henio/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_henio_home()
    gw_data: dict = {}
