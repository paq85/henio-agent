...
# Uses get_henio_home() so each profile gets its own Matrix store.
from henio_constants import get_henio_dir as _get_henio_dir
_STORE_DIR = _get_henio_dir("platforms/matrix/store", "matrix/store")
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(os.getenv("HENIO_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6"))
        self._text_batch_split_delay_seconds = float(os.getenv("HENIO_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0"))
    ...
    async def connect(self) -> bool:
        ...
        if self._encryption:
            ...
            try:
                ...
                _acct_id = self._user_id or "henio"
    ...
    async def _sync_loop(self) -> None:
        ...
        if sync_store is not None and hasattr(sync_store, "get_next_batch"):
            try:
                stored = sync_store.get_next_batch()
                next_batch = await stored if inspect.isawaitable(stored) else stored
            except Exception:
                next_batch = None
        while not self._closing:
            try:
                ...
                try:
                    ...
                    sync_kwargs.pop("since", None)
                    sync_data = await client.sync(**sync_kwargs)

                sync_error_message = str(getattr(sync_data, "message", "") or sync_data or "").lower()
                if any(token in sync_error_message for token in ("m_unknown_token", "401", "403", "unauthorized", "forbidden")):
                    logger.error("Matrix: permanent auth error: %s — stopping sync", getattr(sync_data, "message", sync_data))
                    return
                ...
                if isinstance(sync_data, dict):
                    ...
                    if nb:
                        ...
                        if sync_store is not None and hasattr(sync_store, "put_next_batch"):
                            try:
                                stored = sync_store.put_next_batch(nb)
