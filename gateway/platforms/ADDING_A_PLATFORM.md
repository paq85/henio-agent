...
Checklist for integrating a new messaging platform into the Henio gateway.
...
And add it to the `henio-gateway` composite:

```python
"henio-gateway": {
    "includes": [..., "henio-your-platform"]
}
```
...
## 12. Status Display (`henio_cli/status.py`)
...
## 13. Gateway Setup Wizard (`henio_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ henio_cli/ toolsets.py \
  --include="*.py" -l | sort -u
