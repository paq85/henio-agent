...
- Verification token validation as second auth layer
"""
...
_FEISHU_DEDUP_TTL_SECONDS = 24 * 60 * 60          # 24 hours
...
_FEISHU_WEBHOOK_RATE_LIMIT_MAX = 120               # max requests per window per IP
...
_FEISHU_WEBHOOK_ANOMALY_TTL_SECONDS = 6 * 60 * 60  # anomaly tracker TTL (6 hours)
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._dedup_state_path = get_henio_home() / "feishu_seen_message_ids.json"
    ...
    def _load_settings(extra: Dict[str, Any]) -> FeishuAdapterSettings:
        ...
        return FeishuAdapterSettings(
            ...
            dedup_cache_size=max(
                32,
                int(os.getenv("HENIO_FEISHU_DEDUP_CACHE_SIZE", str(_DEFAULT_DEDUP_CACHE_SIZE))),
            ),
            text_batch_delay_seconds=float(
                os.getenv("HENIO_FEISHU_TEXT_BATCH_DELAY_SECONDS", str(_DEFAULT_TEXT_BATCH_DELAY_SECONDS))
            ),
            text_batch_split_delay_seconds=float(
                os.getenv("HENIO_FEISHU_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0")
            ),
            text_batch_max_messages=max(
                1,
                int(os.getenv("HENIO_FEISHU_TEXT_BATCH_MAX_MESSAGES", str(_DEFAULT_TEXT_BATCH_MAX_MESSAGES))),
            ),
            text_batch_max_chars=max(
                1,
                int(os.getenv("HENIO_FEISHU_TEXT_BATCH_MAX_CHARS", str(_DEFAULT_TEXT_BATCH_MAX_CHARS))),
            ),
            media_batch_delay_seconds=float(
                os.getenv("HENIO_FEISHU_MEDIA_BATCH_DELAY_SECONDS", str(_DEFAULT_MEDIA_BATCH_DELAY_SECONDS))
            ),
            webhook_host=str(
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if not acquired:
                ...
                message = (
                    "Another local Henio gateway is already using this Feishu app_id"
    ...
    async def send_exec_approval(
        ...
        The buttons carry ``henio_action`` in their value dict so that
        ...
        try:
            ...
            def _btn(label: str, action_name: str, btn_type: str = "default") -> dict:
                return {
                    ...
                    "type": btn_type,
                    "value": {"henio_action": action_name, "approval_id": approval_id},
                }
    ...
    def _on_message_read_event(self, data: P2ImMessageMessageReadV1) -> None:
        """Ignore read-receipt events that Henio does not act on."""
        event = getattr(data, "event", None)
        message = getattr(event, "message", None)
    ...
    def _on_reaction_event(self, event_type: str, data: Any) -> None:
        ...
        # and Henio' own ACK emoji to avoid feedback loops.
        loop = self._loop
        if (
    ...
    async def _handle_card_action_event(self, data: Any) -> None:
        ...
        henio_action = action_value.get("henio_action") if isinstance(action_value, dict) else None
        if henio_action:
            ...
            choice = choice_map.get(henio_action, "deny")
    ...
    def _record_webhook_anomaly(self, remote_ip: str, status: str) -> None:
        ...
        Tracks webhook anomalies with a 6-hour TTL and logs every 25 consecutive
        ...
        *,
        ...
        async with httpx.AsyncClient(timeout=30.0, follow_redirects=True) as client:
            response = await client.get(
                file_url,
                headers={
                    "User-Agent": "Mozilla/5.0 (compatible; HenioAgent/1.0)",
                    "Accept": "*/*",
                },
            )
    ...
    async def _handle_webhook_request(self, request: Any) -> Any:
        ...
        # Rate limiting — composite key: app_id:path:remote_ip.
        ...
        if payload.get("encrypt"):
            logger.error("[Feishu] Encrypted webhook payloads are not supported by Henio webhook mode")
    ...
    def _check_webhook_rate_limit(self, rate_key: str) -> bool:
        ...
        The rate_key is composed as "{app_id}:{path}:{remote_ip}" so the limit is scoped to a
        specific (account, endpoint, IP) triple rather
    ...
    async def _resolve_sender_name_from_api(self, sender_id: Optional[str]) -> Optional[str]:
        ...
        ID-type detection: ou_ → open_id, on_ → union_id, else user_id.
