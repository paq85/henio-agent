...
# 5. Symlinks the 'henio' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Henio Agent Setup${NC}"
echo ""
...
# ============================================================================
# PATH setup — symlink henio into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up henio command..."

HENIO_BIN="$SCRIPT_DIR/venv/bin/henio"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$HENIO_BIN" "$COMMAND_LINK_DIR/henio"
echo -e "${GREEN}✓${NC} Symlinked henio → $COMMAND_LINK_DISPLAY_DIR/henio"
...
# ============================================================================
# Seed bundled skills into ~/.henio/skills/
# ============================================================================

HENIO_SKILLS_DIR="${HENIO_HOME:-$HOME/.henio}/skills"
mkdir -p "$HENIO_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.henio/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
	echo -e "${GREEN}✓${NC} Skills synced"
else
	# Fallback: copy if sync script fails (missing deps, etc.)
	if [ -d "$SCRIPT_DIR/skills" ]; then
		cp -rn "$SCRIPT_DIR/skills/"* "$HENIO_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  henio status        # Check configuration"
if is_termux; then
	echo "  henio gateway       # Run gateway in foreground"
else
	echo "  henio gateway install # Install gateway service (messaging + cron)"
fi
echo "  henio cron list     # View scheduled jobs"
echo "  henio doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
	...
	"$SCRIPT_DIR/venv/bin/python" -m henio_cli.main setup
fi
