# OpenAI-Compatible API Server for Henio Agent
...
SSE streaming. By exposing this endpoint, henio-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  henio-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.henio/.env`
2. Run `henio gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with henio-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns henio-agent as a model) |
...
Authorization: Bearer henio-api-key-here
...
But henio-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "henio-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured henio-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of henio-agent's core prompt
...
Once implemented, henio-agent works as a drop-in backend for:
