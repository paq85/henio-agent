"""ACP agent server — exposes Henio Agent via the Agent Client Protocol."""
...
try:
    from henio_cli import __version__ as HENIO_VERSION
except Exception:
    HENIO_VERSION = "0.0.0"
...
class HenioACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Henio AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Henio version",
        },
    )
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = getattr(state.agent, "enabled_toolsets", None) or ["henio-acp"]
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        if provider:
            auth_methods = [
                AuthMethodAgent(
                    ...
                    description=f"Authenticate Henio using the currently configured {provider} runtime credentials.",
                )
            ]
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="henio-agent", version=HENIO_VERSION),
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Henio on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
    ...
    def _cmd_model(self, args: str, state: SessionState) -> str:
        ...
        try:
            from henio_cli.models import parse_model_input, detect_provider_for_model
    ...
    def _cmd_tools(self, args: str, state: SessionState) -> str:
        try:
            from model_tools import get_tool_definitions
            toolsets = getattr(state.agent, "enabled_toolsets", None) or ["henio-acp"]
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Henio Agent v{HENIO_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Henio has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
