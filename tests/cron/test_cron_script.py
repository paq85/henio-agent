...
- Path resolution (absolute, relative to HENIO_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp HENIO_HOME."""
    henio_home = tmp_path / ".henio"
    henio_home.mkdir()
    (henio_home / "cron").mkdir()
    (henio_home / "cron" / "output").mkdir()
    (henio_home / "scripts").mkdir()
    monkeypatch.setenv("HENIO_HOME", str(henio_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "HENIO_DIR", henio_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", henio_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", henio_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", henio_home / "cron" / "output")
...
class TestCronjobToolScript:
    ...
    def test_create_with_script(self, cron_env, monkeypatch):
        monkeypatch.setenv("HENIO_INTERACTIVE", "1")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.henio/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("HENIO_SESSION_PLATFORM") is None
        assert os.environ.get("HENIO_SESSION_CHAT_ID") is None
        assert os.environ.get("HENIO_SESSION_CHAT_NAME") is None
