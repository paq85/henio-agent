...
def container_env(tmp_path, monkeypatch):
    """Set up a fake HENIO_HOME with .container-mode file."""
    henio_home = tmp_path / ".henio"
    henio_home.mkdir()
    monkeypatch.setenv("HENIO_HOME", str(henio_home))
    monkeypatch.delenv("HENIO_DEV", raising=False)

    container_mode = henio_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=henio-agent\n"
        "exec_user=henio\n"
        "henio_bin=/data/current-package/bin/henio\n"
    )
    return henio_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("henio_cli.config._is_inside_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "henio-agent"
    assert info["exec_user"] == "henio"
    assert info["henio_bin"] == "/data/current-package/bin/henio"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("henio_cli.config._is_inside_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_henio_dev(container_env, monkeypatch):
    """Returns None when HENIO_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("HENIO_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_henio_dev_zero(container_env, monkeypatch):
    """HENIO_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("HENIO_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        henio_home = Path(tmpdir) / ".henio"
        henio_home.mkdir()
        (henio_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("henio_cli.config._is_inside_container", return_value=False), \
             patch("henio_cli.config.get_henio_home", return_value=henio_home), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HENIO_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=henio-custom\n"
        "exec_user=myuser\n"
        "henio_bin=/opt/henio/bin/henio\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "henio-custom"
    assert info["exec_user"] == "myuser"
    assert info["henio_bin"] == "/opt/henio/bin/henio"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from henio_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "henio"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "henio-agent" in cmd
    assert "/data/current-package/bin/henio" in cmd
    assert "chat" in cmd
