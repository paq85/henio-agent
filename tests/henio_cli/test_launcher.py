"""Tests for the top-level `./henio` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./henio` should use `henio_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "henio"
    called = []

    fake_main_module = types.ModuleType("henio_cli.main")

    def fake_main():
        called.append("henio_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "henio_cli.main", fake_main_module)
