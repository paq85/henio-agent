...
import henio_cli.doctor as doctor
import henio_cli.gateway as gateway_cli
from henio_cli import doctor as doctor_mod
from henio_cli.doctor import _has_provider_env_config
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(doctor_mod, "HENIO_HOME", henio_home)
    monkeypatch.delenv("HENIO_INTERACTIVE", raising=False)

    seen = {}

    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("HENIO_INTERACTIVE")
        raise SystemExit(0)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_henio_home(self, tmp_path, provider=""):
        """Create a minimal HENIO_HOME with config.yaml."""
        home = tmp_path / ".henio"
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_henio_home(tmp_path, provider)
        monkeypatch.setattr(doctor_mod, "HENIO_HOME", home)
        ...
        try:
            from henio_cli import auth as _auth_mod
