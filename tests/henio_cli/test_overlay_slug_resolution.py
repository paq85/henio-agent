...
HENIO_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot") while
_PROVIDER_MODELS and config.yaml use Henio IDs ("copilot").  The slug
...
def test_copilot_uses_henio_slug():
...
def test_kimi_for_coding_alias():
    ...
    from henio_cli.auth import resolve_provider
...
def test_kimi_for_coding_overlay_uses_henio_slug():
...
def test_kilo_overlay_uses_henio_slug():
