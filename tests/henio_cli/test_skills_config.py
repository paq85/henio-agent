"""Tests for henio_cli/skills_config.py and skills_tool disabled filtering."""
import pytest
...
class TestGetDisabledSkills:
    def test_empty_config(self):
        from henio_cli.skills_config import get_disabled_skills
...
class TestSaveDisabledSkills:
    @patch("henio_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from henio_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("henio_cli.config.load_config")
    ...
    @patch("henio_cli.config.load_config")
    @patch.dict("os.environ", {"HENIO_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_explicit_platform_param(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("HENIO_PLATFORM", raising=False)
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """HENIO_SESSION_PLATFORM should be used when HENIO_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_henio_platform_takes_precedence(self, tmp_path, monkeypatch):
        """HENIO_PLATFORM should win over HENIO_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
        ...
        monkeypatch.setenv("HENIO_HOME", str(tmp_path))
        monkeypatch.setenv("HENIO_PLATFORM", "telegram")
        monkeypatch.setenv("HENIO_SESSION_PLATFORM", "discord")
    ...
    def test_explicit_param_overrides_env_vars(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HENIO_HOME", str(tmp_path))
        monkeypatch.setenv("HENIO_PLATFORM", "telegram")
        monkeypatch.setenv("HENIO_SESSION_PLATFORM", "telegram")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from henio_cli.skills_config import _get_categories
        skills = [
