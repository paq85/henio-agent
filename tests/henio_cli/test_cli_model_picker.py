...
def test_provider_selection_returns_slug_on_choice():
    ...
    result = HenioCLI._interactive_provider_selection(cli, providers, "gpt-5.4", "OpenRouter")
...
def test_provider_selection_default_is_current():
    ...
    HenioCLI._interactive_provider_selection(cli, providers, "gpt-5.4", "OpenRouter")
...
def test_model_selection_returns_model_on_choice():
    ...
    result = HenioCLI._interactive_model_selection(cli, provider_data["models"], provider_data)
...
def test_model_selection_empty_prompts_for_manual_input():
    ...
    result = HenioCLI._interactive_model_selection(cli, [], provider_data)
...
def test_prompt_text_input_uses_run_in_terminal_when_app_active():
    ...
    with (
        ...
        result = HenioCLI._prompt_text_input(cli, "Enter value: ")
...
def test_should_handle_model_command_inline_uses_command_name_resolution():
    ...
    with patch("henio_cli.commands.resolve_command", return_value=SimpleNamespace(name="model")):
        assert HenioCLI._should_handle_model_command_inline(cli, "/model") is True

    with patch("henio_cli.commands.resolve_command", return_value=SimpleNamespace(name="help")):
        assert HenioCLI._should_handle_model_command_inline(cli, "/model") is False

    assert HenioCLI._should_handle_model_command_inline(cli, "/model", has_images=True) is False
...
def test_process_command_model_without_args_opens_modal_picker_and_captures_draft():
    ...
    with (
        patch("henio_cli.model_switch.list_authenticated_providers", return_value=providers),
        patch("cli._cprint"),
    ):
...
def test_model_picker_provider_then_model_selection_applies_switch_result_and_restores_draft():
    ...
    with patch("henio_cli.models.provider_model_ids", return_value=["claude-opus-4.6", "claude-sonnet-4.6"]):
        HenioCLI._handle_model_picker_selection(cli)
    ...
    with (
        patch("henio_cli.model_switch.switch_model", return_value=switch_result) as switch_mock,
        patch("cli._cprint"),
    ):
        HenioCLI._handle_model_picker_selection(cli)
