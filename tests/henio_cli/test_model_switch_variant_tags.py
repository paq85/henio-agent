...
def _run_switch(raw_input: str, current_provider: str = "openrouter") -> str:
    ...
    with patch("henio_cli.model_switch.resolve_alias", return_value=None), \
         patch("henio_cli.model_switch.list_provider_models", return_value=[]), \
         patch("henio_cli.runtime_provider.resolve_runtime_provider",
               return_value={"api_key": "test", "base_url": "", "api_mode": "chat_completions"}), \
         patch("henio_cli.models.validate_requested_model", return_value=_MOCK_VALIDATION), \
         patch("henio_cli.model_switch.get_model_info", return_value=None), \
         patch("henio_cli.model_switch.get_model_capabilities", return_value=None), \
         patch("henio_cli.models.detect_provider_for_model", return_value=None):
        result = switch_model(
            raw_input=raw_input,
