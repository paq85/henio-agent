...
and haven't been migrated to the Henio auth store yet.

Root cause: list_authenticated_providers() checked the raw Henio auth
...
Henio auth store has no openai-codex tokens, and
...
def codex_cli_only_env(tmp_path, monkeypatch):
    ...
    # Empty Henio auth store
    (henio_home / "auth.json").write_text(
...
def test_codex_cli_tokens_detected_by_model_picker(codex_cli_only_env):
    ...
    from henio_cli.model_switch import list_authenticated_providers
...
def test_codex_cli_tokens_migrated_after_detection(codex_cli_only_env):
    ...
    migrated into the Henio auth store for subsequent fast lookups."""
    from henio_cli.model_switch import list_authenticated_providers

    # First call triggers migration
    list_authenticated_providers(current_provider="openai-codex")

    # Verify tokens are now in Henio auth store
    auth_path = codex_cli_only_env / "auth.json"
    store = json.loads(auth_path.read_text())
    providers = store.get("providers", {})
    assert "openai-codex" in providers, (
        f"openai-codex not migrated to Henio auth store: {list(providers.keys())}"
    )
...
@pytest.fixture()
def henio_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Henio auth store (no Codex CLI needed)."""
    henio_home = tmp_path / ".henio"
    henio_home.mkdir()
...
def test_normal_path_still_works(henio_auth_only_env):
    """openai-codex appears when tokens are already in Henio auth store."""
    from henio_cli.model_switch import list_authenticated_providers
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Henio
    auth store."""
    henio_home = tmp_path / ".henio"
    henio_home.mkdir()
