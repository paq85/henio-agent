"""Tests for Codex auth — tokens stored in Henio auth store (~/.henio/auth.json)."""
...
def _setup_henio_auth(henio_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Henio auth store."""
    ...
    auth_file = henio_home / "auth.json"
...
def test_read_codex_tokens_missing(tmp_path, monkeypatch):
    ...
    henio_home.mkdir(parents=True, exist_ok=True)
    # Empty auth store
    (henio_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
    monkeypatch.setenv("HENIO_HOME", str(henio_home))
...
def test_resolve_codex_runtime_credentials_missing_access_token(tmp_path, monkeypatch):
    henio_home = tmp_path / "henio"
    _setup_henio_auth(henio_home, access_token="")
    monkeypatch.setenv("HENIO_HOME", str(henio_home))
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_henio_auth(henio_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("HENIO_HOME", str(henio_home))
    ...
    monkeypatch.setattr("henio_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    henio_home = tmp_path / "henio"
    _setup_henio_auth(henio_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("HENIO_HOME", str(henio_home))
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Henio auth store, not ~/.codex/."""
    henio_home = tmp_path / "henio"
    codex_home = tmp_path / "codex-cli"
    henio_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "henio-at", "refresh_token": "henio-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Henio store
    assert not (codex_home / "auth.json").exists()

    # Henio auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "henio-at"
...
def test_refresh_codex_auth_tokens_writes_back_to_cli(tmp_path, monkeypatch):
    ...
    from henio_cli.auth import _refresh_codex_auth_tokens
    ...
    monkeypatch.setattr("henio_cli.auth.refresh_codex_oauth_pure", lambda *a, **kw: {
...
def test_resolve_returns_henio_auth_store_source(tmp_path, monkeypatch):
    henio_home = tmp_path / "henio"
    _setup_henio_auth(henio_home)
    monkeypatch.setenv("HENIO_HOME", str(henio_home))

    creds = resolve_codex_runtime_credentials()
    assert creds["source"] == "henio-auth-store"
