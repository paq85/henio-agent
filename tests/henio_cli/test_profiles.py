"""Comprehensive tests for henio_cli.profiles module.
...
from henio_cli.profiles import (
    ...
    _get_profiles_root,
    _get_default_henio_home,
)


# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and HENIO_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.henio/profiles)
    * HENIO_HOME  -> tmp_path/.henio  (so get_henio_home() agrees)
    * Creates the bare-minimum ~/.henio directory.
    """
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    default_home = tmp_path / ".henio"
    default_home.mkdir(exist_ok=True)
    monkeypatch.setenv("HENIO_HOME", str(default_home))
    return tmp_path
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("henio_cli.profiles._cleanup_gateway_service"):
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".henio" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_henio_home_returns_default(self, profile_env):
        # HENIO_HOME points to tmp_path/.henio which is the default
        assert get_active_profile_name() == "default"

    def test_profile_path_returns_profile_name(self, profile_env, monkeypatch):
        tmp_path = profile_env
        create_profile("coder", no_alias=True)
        profile_dir = tmp_path / ".henio" / "profiles" / "coder"
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom HENIO_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("HENIO_HOME", str(custom))
        # With Docker-aware roots, a custom HENIO_HOME is the default —
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("henio")
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        old_dir = tmp_path / ".henio" / "profiles" / "oldname"
        assert old_dir.is_dir()

        # Mock alias collision to avoid subprocess calls
        with patch("henio_cli.profiles.check_alias_collision", return_value="skip"):
            new_dir = rename_profile("oldname", "newname")

        assert not old_dir.is_dir()
        assert new_dir.is_dir()
        assert new_dir == tmp_path / ".henio" / "profiles" / "newname"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("henio-agent", ".worktrees", "profiles", "bin",
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".henio_history",
        ...
        excluded_prefixes = [
            "default/henio-agent", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.henio_history",
...
class TestCompletion:
    ...
    def test_bash_completion_has_henio_profiles_function(self):
        script = generate_bash_completion()
        assert "_henio_profiles" in script

    def test_zsh_completion_has_henio_function(self):
        script = generate_zsh_completion()
        assert "_henio" in script


# ===================================================================
# TestGetProfilesRoot / TestGetDefaultHenioHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_henio_home()."""

    def test_profiles_root_under_home(self, profile_env):
        tmp_path = profile_env
        root = _get_profiles_root()
        assert root == tmp_path / ".henio" / "profiles"

    def test_default_henio_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_henio_home()
        assert home == tmp_path / ".henio"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (HENIO_HOME outside ~/.henio), profiles go under HENIO_HOME."""
    ...
    def test_default_henio_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_henio_home() returns HENIO_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HENIO_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (HENIO_HOME under ~/.henio), profiles root is still ~/.henio/profiles."""
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under HENIO_HOME."""
        from henio_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HENIO_HOME", str(docker_home))
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under HENIO_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HENIO_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".henio" / "profiles" / "mybot"
        assert result == expected
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from henio_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".henio"
