"""Tests that `henio model` always shows the model selection menu for custom
...
def config_home(tmp_path, monkeypatch):
    """Isolated HENIO_HOME with a minimal config."""
    home = tmp_path / "henio"
    home.mkdir()
...
class TestCustomProviderModelSwitch:
    ...
    def test_saved_model_still_probes_endpoint(self, config_home):
        ...
        from henio_cli.main import _model_flow_named_custom
        ...
        with patch("henio_cli.models.fetch_api_models", return_value=["model-A", "model-B"]) as mock_fetch, \
    ...
    def test_can_switch_to_different_model(self, config_home):
        ...
        with patch("henio_cli.models.fetch_api_models", return_value=["model-A", "model-B"]), \
             patch.dict("sys.modules", {"simple_term_menu": None}), \
             patch("builtins.input", return_value="2"), \
             patch("builtins.print"):
            _model_flow_named_custom({}, provider_info)
    ...
    def test_probe_failure_falls_back_to_saved(self, config_home):
        ...
        with patch("henio_cli.models.fetch_api_models", return_value=[]), \
    ...
    def test_no_saved_model_still_works(self, config_home):
        ...
        with patch("henio_cli.models.fetch_api_models", return_value=["model-X"]), \
             patch.dict("sys.modules", {"simple_term_menu": None}), \
    ...
    def test_api_mode_set_from_provider_info(self, config_home):
        ...
        with patch("henio_cli.models.fetch_api_models", return_value=["claude-3"]), \
             patch.dict("sys.modules", {"simple_term_menu": None}), \
             patch("builtins.input", return_value="1"), \
             patch("builtins.print"):
            _model_flow_named_custom({}, provider_info)
    ...
    def test_api_mode_cleared_when_not_specified(self, config_home):
        ...
        with patch("henio_cli.models.fetch_api_models", return_value=["llama-3"]), \
             patch.dict("sys.modules", {"simple_term_menu": None}), \
             patch("builtins.input", return_value="1"), \
             patch("builtins.print"):
            _model_flow_named_custom({}, provider_info)
