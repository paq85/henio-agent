...
from henio_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        from henio_cli.auth import _resolve_verify
    ...
    def test_missing_ssl_cert_file_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.delenv("HENIO_CA_BUNDLE", raising=False)
        result = _resolve_verify(auth_state={"tls": {}})
        assert result is True

    def test_missing_henio_ca_bundle_env_falls_back(self, monkeypatch):
        from henio_cli.auth import _resolve_verify

        monkeypatch.setenv("HENIO_CA_BUNDLE", "/nonexistent/henio-ca.pem")
...
def _setup_nous_auth(
    ...
    *,
    ...
    (henio_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_refresh_token_persisted_when_mint_returns_insufficient_credits(tmp_path, monkeypatch):
    ...
    _setup_nous_auth(henio_home, refresh_token="refresh-old")
    ...
    monkeypatch.setattr("henio_cli.auth._refresh_access_token", _fake_refresh_access_token)
    monkeypatch.setattr("henio_cli.auth._mint_agent_key", _fake_mint_agent_key)
