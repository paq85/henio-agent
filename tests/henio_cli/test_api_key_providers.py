...
PROVIDER_ENV_VARS = (
    ...
    "OPENAI_BASE_URL", "HENIO_COPILOT_ACP_COMMAND", "COPILOT_CLI_PATH",
    "HENIO_COPILOT_ACP_ARGS", "COPILOT_ACP_BASE_URL",
)
...
def _clear_provider_env(monkeypatch):
    ...
    monkeypatch.setattr("henio_cli.auth._load_auth_store", lambda: {})
...
class TestApiKeyProviderStatus:
    ...
    def test_copilot_status_uses_gh_cli_token(self, monkeypatch):
        monkeypatch.setattr("henio_cli.copilot_auth._try_gh_cli_token", lambda: "gho_gh_cli_token")
    ...
    def test_copilot_acp_status_detects_local_cli(self, monkeypatch):
        monkeypatch.setenv("HENIO_COPILOT_ACP_ARGS", "--acp --stdio --debug")
        monkeypatch.setattr("henio_cli.auth.shutil.which", lambda command: f"/usr/local/bin/{command}")
        monkeypatch.setattr(
            "henio_cli.auth._probe_external_process_provider",
            lambda runtime: {
                "ok": True,
    ...
    def test_copilot_acp_status_keeps_configured_false_when_cli_missing(self, monkeypatch):
        monkeypatch.setattr("henio_cli.auth.shutil.which", lambda command: None)

        status = get_external_process_provider_status("copilot-acp")

        assert status["configured"] is False
        assert status["logged_in"] is False
        assert status["error_code"] == "missing_copilot_cli"

    def test_copilot_acp_status_distinguishes_configured_from_ready(self, monkeypatch):
        monkeypatch.setattr("henio_cli.auth.shutil.which", lambda command: f"/opt/bin/{command}")
        monkeypatch.setattr(
            "henio_cli.auth._probe_external_process_provider",
            lambda runtime: {
                "ok": False,
                "session_id": "",
                "error": "Timed out waiting for Copilot ACP response to initialize.",
                "error_code": "timeout",
                "stderr_tail": "copilot: login required",
            },
        )
        ...
        assert status["stderr_tail"] == "copilot: login required"
...
class TestResolveApiKeyProviderCredentials:
    ...
    def test_resolve_zai_with_key(self, monkeypatch):
        ...
        monkeypatch.setattr("henio_cli.auth.detect_zai_endpoint", lambda *a, **kw: None)
    ...
    def test_resolve_copilot_with_gh_cli_fallback(self, monkeypatch):
        monkeypatch.setattr("henio_cli.copilot_auth._try_gh_cli_token", lambda: "gho_cli_secret")
        creds = resolve_api_key_provider_credentials("copilot")
        assert creds["provider"] == "copilot"
    ...
    def test_try_gh_cli_token_uses_homebrew_path_when_not_on_path(self, monkeypatch):
        monkeypatch.setattr("henio_cli.auth.shutil.which", lambda command: None)
        monkeypatch.setattr(
            "henio_cli.auth.os.path.isfile",
            lambda path: path == "/opt/homebrew/bin/gh",
        )
        monkeypatch.setattr(
            "henio_cli.auth.os.access",
        ...
        monkeypatch.setattr("henio_cli.auth.subprocess.run", _fake_run)
    ...
    def test_resolve_copilot_acp_with_local_cli(self, monkeypatch):
        monkeypatch.setenv("HENIO_COPILOT_ACP_ARGS", "--acp --stdio")
        monkeypatch.setattr("henio_cli.auth.shutil.which", lambda command: f"/usr/local/bin/{command}")
        monkeypatch.setattr(
            "henio_cli.auth._probe_external_process_provider",
            lambda runtime: {
...
class TestRuntimeProviderResolution:
    ...
    def test_runtime_copilot_uses_responses_for_gpt_5_4(self, monkeypatch):
        monkeypatch.setattr("henio_cli.copilot_auth._try_gh_cli_token", lambda: "gho_cli_secret")
        monkeypatch.setattr(
            "henio_cli.runtime_provider._get_model_config",
            lambda: {"provider": "copilot", "default": "gpt-5.4"},
        )
        monkeypatch.setattr(
            "henio_cli.models.fetch_github_model_catalog",
...
class TestHasAnyProviderConfigured:
    ...
    def test_glm_key_counts(self, monkeypatch, tmp_path):
        from henio_cli import config as config_module
        monkeypatch.setenv("GLM_API_KEY", "test-key")
        henio_home = tmp_path / ".henio"
        henio_home.mkdir()
        monkeypatch.setattr(config_module, "get_env_path", lambda: henio_home / ".env")
        monkeypatch.setattr(config_module, "get_henio_home", lambda: henio_home)
        from henio_cli.main import _has_any_provider_configured
    ...
    def test_claude_code_creds_ignored_on_fresh_install(self, monkeypatch, tmp_path):
        """Claude Code credentials should NOT skip the wizard when Henio is unconfigured."""
        from henio_cli import config as config_module
        ...
        monkeypatch.setattr(config_module, "get_env_path", lambda: henio_home / ".env")
        monkeypatch.setattr(config_module, "get_henio_home", lambda: henio_home)
        monkeypatch.setattr("henio_cli.copilot_auth.resolve_copilot_token", lambda: ("", ""))
        ...
        monkeypatch.setattr("henio_cli.auth.get_auth_status", lambda _pid: {})
    ...
    def test_config_provider_counts(self, monkeypatch, tmp_path):
        ...
        henio_home = tmp_path / ".henio"
        henio_home.mkdir()
        config_file = henio_home / "config.yaml"
    ...
    def test_claude_code_creds_counted_when_henio_configured(self, monkeypatch, tmp_path):
        """Claude Code credentials should count when Henio has been explicitly configured."""
        import yaml
        from henio_cli import config as config_module
        henio_home = tmp_path / ".henio"
        henio_home.mkdir()
...
class TestZaiEndpointAutoDetect:
    ...
    def test_probe_success_returns_detected_url(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "henio_cli.auth.detect_zai_endpoint",
            lambda *a, **kw: {
                "id": "coding-global",
    ...
    def test_env_override_skips_probe(self, monkeypatch):
        ...
        monkeypatch.setattr("henio_cli.auth.detect_zai_endpoint", _never_called)
        creds = resolve_api_key_provider_credentials("zai")
...
class TestKimiMoonshotModelListIsolation:
    ...
    def test_moonshot_list_excludes_coding_plan_only_models(self):
        from henio_cli.main import _PROVIDER_MODELS
...
class TestHuggingFaceModels:
    ...
    def test_model_lists_match(self):
        ...
        from henio_cli.main import _PROVIDER_MODELS as main_models
        from henio_cli.models import _PROVIDER_MODELS as models_models
