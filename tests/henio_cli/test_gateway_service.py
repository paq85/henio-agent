...
import henio_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    def test_systemd_install_repairs_outdated_unit_without_force(self, tmp_path, monkeypatch):
        unit_path = tmp_path / "henio-gateway.service"
...
class TestLaunchdServiceRecovery:
    def test_get_restart_drain_timeout_prefers_env_then_config_then_default(self, monkeypatch):
        monkeypatch.delenv("HENIO_RESTART_DRAIN_TIMEOUT", raising=False)
        ...
        monkeypatch.setenv("HENIO_RESTART_DRAIN_TIMEOUT", "9")
        assert gateway_cli._get_restart_drain_timeout() == 9.0

        monkeypatch.setenv("HENIO_RESTART_DRAIN_TIMEOUT", "invalid")
        assert (
            gateway_cli._get_restart_drain_timeout()
            == DEFAULT_GATEWAY_RESTART_DRAIN_TIMEOUT
        )
    ...
    def test_launchd_install_repairs_outdated_plist_without_force(self, tmp_path, monkeypatch):
        plist_path = tmp_path / "ai.henio.gateway.plist"
...
class TestGatewaySystemServiceRouting:
    ...
    def test_gateway_install_reports_termux_manual_mode(self, monkeypatch, capsys):
        ...
        assert "Run manually: henio gateway" in out
    ...
    def test_gateway_status_on_termux_shows_manual_guidance(self, monkeypatch, capsys):
        ...
        assert "nohup henio gateway" in out
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.henio.gateway"])
            ),
        )
...
class TestSystemUnitHenioHome:
    """HENIO_HOME in system units must reference the target user, not root."""
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'HENIO_HOME=/home/alice/.henio' in unit
        assert '/root/.henio' not in unit

    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: HENIO_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("HENIO_HOME", "/root/.henio/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'HENIO_HOME=/home/alice/.henio/profiles/coder' in unit
        assert '/root/' not in unit

    def test_system_unit_preserves_custom_henio_home(self, monkeypatch):
        # Custom HENIO_HOME not under any user's home — keep as-is
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("HENIO_HOME", "/opt/henio-shared")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'HENIO_HOME=/opt/henio-shared' in unit

    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's HENIO_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        henio_home = str(gateway_cli.get_henio_home().resolve())
        assert f'HENIO_HOME={henio_home}' in unit


class TestHenioHomeForTargetUser:
    """Unit tests for _henio_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("HENIO_HOME", raising=False)

        result = gateway_cli._henio_home_for_target_user("/home/alice")
        assert result == "/home/alice/.henio"
    ...
    def test_remaps_profile_path(self, monkeypatch):
        ...
        result = gateway_cli._henio_home_for_target_user("/home/alice")
        assert result == "/home/alice/.henio/profiles/coder"

    def test_keeps_custom_path(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("HENIO_HOME", "/opt/henio")

        result = gateway_cli._henio_home_for_target_user("/home/alice")
        assert result == "/opt/henio"
...
class TestProfileArg:
    ...
    def test_default_henio_home_returns_empty(self, tmp_path, monkeypatch):
        """Default ~/.henio should not produce a --profile flag."""
        henio_home = tmp_path / ".henio"
        henio_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HENIO_HOME", str(henio_home))
        result = gateway_cli._profile_arg(str(henio_home))
        assert result == ""

    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.henio/profiles/mybot should return '--profile mybot'."""
        profile_dir = tmp_path / ".henio" / "profiles" / "mybot"
        profile_dir.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HENIO_HOME", str(tmp_path / ".henio"))
    ...
    def test_hash_path_returns_empty(self, tmp_path, monkeypatch):
        """Arbitrary non-profile HENIO_HOME should return empty string."""
        custom_home = tmp_path / "custom" / "henio"
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.henio/profiles/mybot/subdir should NOT match — too deep."""
        nested = tmp_path / ".henio" / "profiles" / "mybot" / "subdir"
    ...
    def test_invalid_profile_name_returns_empty(self, tmp_path, monkeypatch):
        ...
        bad_profile = tmp_path / ".henio" / "profiles" / "My Bot!"
    ...
    def test_systemd_unit_includes_profile(self, tmp_path, monkeypatch):
        ...
        profile_dir.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HENIO_HOME", str(profile_dir))
        monkeypatch.setattr(gateway_cli, "get_henio_home", lambda: profile_dir)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".henio" / "profiles" / "orcha"
        profile_dir.mkdir(parents=True)
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.henio.gateway-orcha.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".henio" / "henio-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".henio" / "henio-agent")

    def test_keeps_system_path_unchanged(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "root")
        (tmp_path / "root").mkdir()
        result = gateway_cli._remap_path_for_user("/opt/henio", str(tmp_path / "alice"))
        assert result == "/opt/henio"

    def test_noop_when_same_user(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "alice")
        (tmp_path / "alice").mkdir()
        original = str(tmp_path / "alice" / ".henio" / "henio-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        root_home.mkdir()
        project = root_home / ".henio" / "henio-agent"
        ...
        monkeypatch.setenv("HENIO_HOME", str(root_home / ".henio"))
        monkeypatch.setattr(gateway_cli, "get_henio_home", lambda: root_home / ".henio")
        ...
        assert "WorkingDirectory=/home/alice/.henio/henio-agent" in unit
