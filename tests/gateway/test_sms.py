...
class TestSmsToolset:
    def test_henio_sms_toolset_exists(self):
        from toolsets import get_toolset

        ts = get_toolset("henio-sms")
        assert ts is not None
        assert "tools" in ts

    def test_henio_sms_in_gateway_includes(self):
        from toolsets import get_toolset

        gw = get_toolset("henio-gateway")
        assert gw is not None
        assert "henio-sms" in gw["includes"]
