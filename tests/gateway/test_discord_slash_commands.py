...
def adapter():
    ...
    adapter._client = SimpleNamespace(
        ...
        user=SimpleNamespace(id=99999, name="HenioBot"),
    )
...
async def test_handle_thread_create_slash_dispatches_session_when_message_provided(adapter):
    ...
    await adapter._handle_thread_create_slash(interaction, "Planning", "Hello Henio", 1440)

    adapter._dispatch_thread_session.assert_awaited_once_with(
        interaction, "555", "Planning", "Hello Henio",
    )
...
def test_discord_auto_thread_config_bridge(monkeypatch, tmp_path):
    ...
    henio_dir = tmp_path / ".henio"
    henio_dir.mkdir()
    config_path = henio_dir / "config.yaml"
    config_path.write_text(yaml.dump({
        "discord": {"auto_thread": True},
    }))

    monkeypatch.delenv("DISCORD_AUTO_THREAD", raising=False)
    monkeypatch.setenv("HENIO_HOME", str(henio_dir))
