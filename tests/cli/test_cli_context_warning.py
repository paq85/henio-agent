...
def _isolate(tmp_path, monkeypatch):
    """Isolate HENIO_HOME so tests don't touch real config."""
    home = tmp_path / ".henio"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal HenioCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = HenioCLI.__new__(HenioCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["henio-core"]
        obj.compact = False
