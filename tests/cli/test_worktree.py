...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("henio/henio-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned henio/* and pr-* branches."""

    def test_prunes_orphaned_henio_branch(self, git_repo):
        """henio/henio-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "henio/henio-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "henio/henio-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "henio/henio-deadbeef" in result.stdout
        ...
        assert "henio/henio-deadbeef" in orphaned
        ...
        assert "henio/henio-deadbeef" not in result.stdout
