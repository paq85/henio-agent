...
def session_db(tmp_path):
    ...
    os.environ["HENIO_HOME"] = str(tmp_path / ".henio")
    os.makedirs(tmp_path / ".henio", exist_ok=True)
    from henio_state import SessionDB
    db = SessionDB(db_path=tmp_path / ".henio" / "test_sessions.db")
    yield db
    db.close()
...
def cli_instance(tmp_path, session_db):
    """Create a minimal HenioCLI-like object for testing _handle_branch_command."""
...
class TestBranchCommandCLI:
    ...
    def test_branch_creates_new_session(self, cli_instance, session_db):
        ...
        HenioCLI._handle_branch_command(cli_instance, "/branch")
    ...
    def test_branch_with_custom_name(self, cli_instance, session_db):
        ...
        HenioCLI._handle_branch_command(cli_instance, "/branch refactor approach")
...
class TestBranchCommandDef:
    ...
    def test_branch_in_registry(self):
        ...
        from henio_cli.commands import COMMAND_REGISTRY
