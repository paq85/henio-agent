...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "henio"
        assert config.workspace_id == "henio"
...
class TestFromGlobalConfig:
    ...
    def test_context_tokens_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "contextTokens": 1000,
            "hosts": {"henio": {"contextTokens": 2000}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"henio": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"henio": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="henio-agent"
        ):
            result = config.resolve_session_name("/home/user/henio-agent/subdir")
        assert result == "henio-agent"
...
class TestResolveConfigPath:
    def test_prefers_henio_home_when_exists(self, tmp_path):
        henio_home = tmp_path / "henio"
        henio_home.mkdir()
        local_cfg = henio_home / "honcho.json"
    ...
    def test_falls_back_to_global_when_no_local(self, tmp_path):
        henio_home = tmp_path / "henio"
        henio_home.mkdir()
        # No honcho.json in HENIO_HOME — also isolate ~/.henio so
    ...
    def test_falls_back_to_global_without_henio_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()

        with patch.dict(os.environ, {}, clear=False), \
             patch.object(Path, "home", return_value=fake_home):
            os.environ.pop("HENIO_HOME", None)
            result = resolve_config_path()
        assert result == GLOBAL_CONFIG_PATH
...
class TestResolveActiveHost:
    def test_default_returns_henio(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("HENIO_HONCHO_HOST", None)
            os.environ.pop("HENIO_HOME", None)
            assert resolve_active_host() == "henio"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"HENIO_HONCHO_HOST": "henio.coder"}):
            assert resolve_active_host() == "henio.coder"

    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HENIO_HONCHO_HOST", None)
            with patch("henio_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "henio.coder"

    def test_default_profile_returns_henio(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HENIO_HONCHO_HOST", None)
            with patch("henio_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "henio"

    def test_custom_profile_returns_henio(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HENIO_HONCHO_HOST", None)
            with patch("henio_cli.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "henio"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HENIO_HONCHO_HOST", None)
            # Temporarily remove henio_cli.profiles to simulate import failure
            saved = sys.modules.get("henio_cli.profiles")
            sys.modules["henio_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "henio"
            finally:
                if saved is not None:
                    sys.modules["henio_cli.profiles"] = saved
                else:
                    sys.modules.pop("henio_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="henio.coder")
        assert config.host == "henio.coder"
        assert config.workspace_id == "henio"  # shared workspace
        assert config.ai_peer == "henio.coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="henio")
        assert config.host == "henio"
        assert config.workspace_id == "henio"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "henio": {"aiPeer": "henio", "peerName": "alice"},
                "henio.coder": {
                    "aiPeer": "henio.coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="henio.coder", config_path=config_file,
        )
        assert config.host == "henio.coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "henio.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="henio.dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "henio.dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"henio": {"enabled": True, "aiPeer": "henio"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"henio": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestInitOnSessionStart:
    ...
    def test_host_block_overrides_root(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "initOnSessionStart": True,
            "hosts": {"henio": {"initOnSessionStart": False}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
    ...
    def test_host_block_true_overrides_root_absent(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"henio": {"initOnSessionStart": True}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.init_on_session_start is True
