"""
Tests for timezone support (henio_time module + integration points).
...
def _reset_henio_time_cache():
    """Reset the henio_time module cache (replacement for removed reset_cache)."""
    henio_time._cached_tz = None
    henio_time._cached_tz_name = None
    henio_time._cache_resolved = False


# =========================================================================
# henio_time.now() — core helper
# =========================================================================

class TestHenioTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_henio_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["HENIO_TIMEZONE"] = "Asia/Kolkata"
        result = henio_time.now()
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["HENIO_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="henio_time"):
            result = henio_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["HENIO_TIMEZONE"] = "Europe/London"
        tz = henio_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["HENIO_TIMEZONE"] = "Not/A/Timezone"
        tz = henio_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When HENIO_TIMEZONE is set, child process sees TZ env var."""
        import json as _json
        os.environ["HENIO_TIMEZONE"] = "Asia/Kolkata"
    ...
    def test_tz_not_injected_when_empty(self):
        """When HENIO_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("HENIO_TIMEZONE", None)
    ...
    def test_henio_timezone_not_leaked_to_child(self):
        """HENIO_TIMEZONE itself must NOT appear in child env (only TZ)."""
        import json as _json
        os.environ["HENIO_TIMEZONE"] = "Asia/Kolkata"

        with patch("model_tools.handle_function_call", side_effect=self._mock_handle):
            result = _json.loads(self._execute_code(
                code='import os; print(os.environ.get("HENIO_TIMEZONE", "NOT_SET"))',
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=henio_tz) which shifted
        absolute time when system-local tz != Henio tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_henio_tz(self):
        """Already-aware datetimes should be normalized to Henio tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Henio tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Henio tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Henio is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Henio tz
        ...
        # Use a Henio timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _henio_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["HENIO_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_henio_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Henio timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["HENIO_TIMEZONE"] = "US/Eastern"
        _reset_henio_time_cache()
