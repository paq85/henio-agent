"""
Tests for mcp_serve — Henio MCP server.
...
def _isolate_henio_home(tmp_path, monkeypatch):
    """Redirect HENIO_HOME to a temp directory."""
    monkeypatch.setenv("HENIO_HOME", str(tmp_path))
    try:
        import henio_constants
        monkeypatch.setattr(henio_constants, "get_henio_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking henio_state schema."""
