...
def test_round_robin_strategy_rotates_priorities(tmp_path, monkeypatch):
    ...
    config_path = tmp_path / "henio" / "config.yaml"
...
def test_mark_exhausted_and_rotate_persists_status(tmp_path, monkeypatch):
    ...
    auth_payload = json.loads((tmp_path / "henio" / "auth.json").read_text())
...
def test_try_refresh_current_updates_only_current_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "henio_cli.auth.refresh_codex_oauth_pure",
...
def test_load_pool_removes_stale_file_backed_singleton_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.read_henio_oauth_credentials",
        lambda: None,
    )
    monkeypatch.setattr(
...
def test_singleton_seed_does_not_clobber_manual_oauth_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("ANTHROPIC_TOKEN", raising=False)
    monkeypatch.delenv("CLAUDE_CODE_OAUTH_TOKEN", raising=False)
    monkeypatch.setattr("henio_cli.auth.is_provider_explicitly_configured", lambda pid: True)
    _write_auth_store(
        tmp_path,
        {
            "version": 1,
    ...
    assert {entry.source for entry in entries} == {"manual:henio_pkce", "henio_pkce"}
...
def test_get_custom_provider_pool_key(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HENIO_HOME", str(tmp_path / "henio"))
    (tmp_path / "henio").mkdir(parents=True, exist_ok=True)
    import yaml
    config_path = tmp_path / "henio" / "config.yaml"
    config_path.write_text(yaml.dump({
        "custom_providers": [
