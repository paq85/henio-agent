...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "Henio Tests")
...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_henio_paths(tmp_path: Path, monkeypatch):
    from agent.context_references import preprocess_context_references_async

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("HENIO_HOME", str(tmp_path / ".henio"))

    henio_env = tmp_path / ".henio" / ".env"
    henio_env.parent.mkdir(parents=True)
    henio_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
    ...
    result = await preprocess_context_references_async(
        "read @file:.henio/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,
