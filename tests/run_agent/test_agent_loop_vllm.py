"""Integration tests for HenioAgentLoop with a local vLLM server.

Tests the full Phase 2 flow: ManagedServer + tool calling with a real
vLLM backend, producing actual token IDs and logprobs for high-fidelity runs.
...
def _make_server_manager():
    ...
    sm = ServerManager([config], tool_parser="henio")
