...
class TestSyncSkills:
    ...
    def test_removed_bundled_skill_is_not_deleted_from_user_dir(self, tmp_path):
        """Cleaning stale manifest entries must not purge already-copied skills."""
        bundled = self._setup_bundled(tmp_path)
        skills_dir = tmp_path / "user_skills"
        manifest_file = skills_dir / ".bundled_manifest"

        removed_skill = skills_dir / "creative" / "removed-skill"
        removed_skill.mkdir(parents=True)
        (removed_skill / "SKILL.md").write_text("# Legacy user copy")

        skills_dir.mkdir(parents=True, exist_ok=True)
        manifest_file.write_text("removed-skill:def456\n")
        ...
        assert (removed_skill / "SKILL.md").read_text() == "# Legacy user copy"
...
class TestGetBundledDir:
    def test_env_var_override(self, tmp_path, monkeypatch):
        """HENIO_BUNDLED_SKILLS env var overrides the default path resolution."""
        custom_dir = tmp_path / "custom_skills"
        custom_dir.mkdir()
        monkeypatch.setenv("HENIO_BUNDLED_SKILLS", str(custom_dir))
    ...
    def test_default_without_env_var(self, monkeypatch):
        ...
        monkeypatch.delenv("HENIO_BUNDLED_SKILLS", raising=False)
        result = _get_bundled_dir()
        assert result.name == "skills"

    def test_env_var_empty_string_ignored(self, monkeypatch):
        """Empty HENIO_BUNDLED_SKILLS should fall back to default."""
        monkeypatch.setenv("HENIO_BUNDLED_SKILLS", "")
        result = _get_bundled_dir()
        assert result.name == "skills"
