...
class TestFirecrawlClientConfig:
    ...
    def setup_method(self):
        ...
        for key in (
            "HENIO_ENABLE_NOUS_MANAGED_TOOLS",
            "FIRECRAWL_API_KEY",
            "FIRECRAWL_API_URL",
        ...
        os.environ["HENIO_ENABLE_NOUS_MANAGED_TOOLS"] = "1"
    ...
    def test_nous_auth_token_respects_henio_home_override(self, tmp_path):
        """Auth lookup should read from HENIO_HOME/auth.json, not ~/.henio/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".henio").mkdir(parents=True)
...
class TestBackendSelection:
    ...
    ``henio tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
    ...
    def setup_method(self):
        ...
        for key in self._ENV_KEYS:
            if key != "HENIO_ENABLE_NOUS_MANAGED_TOOLS":
                os.environ.pop(key, None)
