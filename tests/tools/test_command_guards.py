...
def _clean_state():
    ...
    saved = {}
    for k in ("HENIO_INTERACTIVE", "HENIO_GATEWAY_SESSION", "HENIO_EXEC_ASK", "HENIO_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["HENIO_INTERACTIVE"] = "1"
        session_key = os.getenv("HENIO_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No HENIO_INTERACTIVE or HENIO_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
