...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._henio_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_henio_bin(self):
        """After _INSTALL_FAILED, manual install in HENIO_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        henio_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(henio_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(henio_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._henio_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == henio_bin
            assert _tirith_mod._resolved_path == henio_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# HENIO_HOME isolation
# ---------------------------------------------------------------------------

class TestHenioHomeIsolation:
    def test_henio_bin_dir_respects_henio_home(self):
        """_henio_bin_dir must use HENIO_HOME, not hardcoded ~/.henio."""
        from tools.tirith_security import _henio_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"HENIO_HOME": tmpdir}):
            result = _henio_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_henio_home(self):
        """_failure_marker_path must use HENIO_HOME, not hardcoded ~/.henio."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"HENIO_HOME": "/custom/henio"}):
            result = _failure_marker_path()
        assert result == "/custom/henio/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets HENIO_HOME; verify it's active."""
        henio_home = os.getenv("HENIO_HOME")
        assert henio_home is not None, "HENIO_HOME should be set by conftest"
        assert "henio_test" in henio_home, "Should point to test temp dir"

    def test_get_henio_home_fallback(self):
        """Without HENIO_HOME set, falls back to ~/.henio."""
        from tools.tirith_security import _get_henio_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove HENIO_HOME entirely
            os.environ.pop("HENIO_HOME", None)
            result = _get_henio_home()
        assert result == os.path.join(os.path.expanduser("~"), ".henio")
