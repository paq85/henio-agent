...
def _restore_tool_modules():
    original_henio_home = os.environ.get("HENIO_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_henio_home is None:
            os.environ.pop("HENIO_HOME", None)
        else:
            os.environ["HENIO_HOME"] = original_henio_home
        _reset_modules(("tools", "henio_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    henio_cli = types.ModuleType("henio_cli")
    henio_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["henio_cli"] = henio_cli
    henio_home = tmp_path / "henio-home"
    os.environ["HENIO_HOME"] = str(henio_home)
    sys.modules["henio_cli.config"] = types.SimpleNamespace(
        get_henio_home=lambda: henio_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="henio-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": henio_home / "modal_snapshots.json",
