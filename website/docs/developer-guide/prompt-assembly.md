...
description: "How Henio builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

Henio deliberately separates:
...
1. agent identity — `SOUL.md` from `HENIO_HOME` when available, otherwise falls back to `DEFAULT_AGENT_IDENTITY` in `prompt_builder.py`
...
```
# Layer 1: Agent Identity (from ~/.henio/SOUL.md)
You are Henio, an AI assistant created by Nous Research.
...
`SOUL.md` lives at `~/.henio/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_henio_md(cwd_path)       # 1. .henio.md / HENIO.md (walks to git root)
    ...
    # SOUL.md from HENIO_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.henio.md`, `HENIO.md` | CWD up to git root | Henio-native project config |
...
- **YAML frontmatter stripped** — `.henio.md` frontmatter is removed (reserved for future config overrides)
...
1. `.henio.md` / `HENIO.md` (walks to git root)
