...
sidebar_position: 1
title: "Architecture"
description: "Henio Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Henio Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ henio_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # HenioCLI — interactive terminal UI (~8,500 lines)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── henio_state.py           # SQLite session/state database with FTS5
├── henio_constants.py       # HENIO_HOME, profile-aware paths
...
│   ├── main.py               # Entry point — all `henio` subcommands (~5,500 lines)
...
│   ├── skills_config.py      # henio skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # henio tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # henio gateway start/stop
│
...
├── skills/                   # Bundled skills seeded on fresh installs
...
```text
User input → HenioCLI.process_input()
...
11. **[Environments, Benchmarks & Data Generation](./environments.md)** — evaluation workflows and trajectory generation
...
- **`prompt_builder.py`** — Assembles the system prompt from: personality (SOUL.md), memory (MEMORY.md, USER.md), skills, context files (AGENTS.md, .henio.md), tool-use guidance, and model-specific instructions
...
Central tool registry (`tools/registry.py`) with the built-in toolsets used across CLI, gateway, ACP, and automation flows. Each tool file self-registers at import time. The registry handles schema collection, dispatch, availability checking, and error wrapping. Terminal tools support 6 backends (local, Docker, SSH, Daytona, Modal, Singularity).
...
Three discovery sources: `~/.henio/plugins/` (user), `.henio/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `henio plugins` or `config.yaml`.

→ [Plugin Guide](../guides/build-a-henio-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Henio as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.

→ [ACP Internals](./acp-internals.md)

### Environments / Trajectories

Full environment framework for evaluation, benchmarking, and trajectory generation. Integrates with Atropos-friendly workflows, supports multiple tool-call parsers, and generates ShareGPT-format trajectories.
...
| **Loose coupling** | Optional subsystems (MCP, plugins, memory providers, environment tooling) use registry patterns and check_fn gating, not hard dependencies. |
| **Profile isolation** | Each profile (`henio -p <name>`) gets its own HENIO_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
