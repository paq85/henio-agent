...
description: "How Henio stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `henio_cli/cron.py` | CLI `henio cron` subcommands |
...
Jobs are stored in `~/.henio/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `henio cron` commands are run or during active CLI sessions.
...
```python
# ~/.henio/scripts/check_competitors.py
...
2. **Environment variable** — `HENIO_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.henio/cron/output/` |
...
The `henio cron` CLI provides direct job management:

```bash
henio cron list                    # Show all jobs
henio cron create                  # Interactive job creation (alias: add)
henio cron edit <job_id>           # Edit job configuration
henio cron pause <job_id>          # Pause a running job
henio cron resume <job_id>         # Resume a paused job
henio cron run <job_id>            # Trigger immediate execution
henio cron remove <job_id>         # Delete a job
```
