...
sidebar_position: 4
title: "Contributing"
description: "How to contribute to Henio Agent — dev setup, code style, PR process"
---

# Contributing

Thank you for contributing to Henio Agent! This guide covers setting up your dev environment, understanding the codebase, and getting your PR merged.
...
```bash
mkdir -p ~/.henio/{cron,sessions,logs,memories,skills}
...
- **Profile-safe paths**: Never hardcode `~/.henio` or `~/.henio` — use `get_henio_home()` from `henio_constants` for code paths and `display_henio_home()` for user-facing messages. See [AGENTS.md](https://github.com/paq85/henio-agent/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.

## Cross-Platform Compatibility

Henio officially supports Linux, macOS, and WSL2. Native Windows is **not supported**, but the codebase includes some defensive coding patterns to avoid hard crashes in edge cases. Key rules:
...
Henio has terminal access. Security matters.
...
2. **Test manually**: Run `henio` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/paq85/henio-agent/issues)
- Include: OS, Python version, Henio version (`henio version`), full error traceback
...
By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/paq85/henio-agent/blob/main/LICENSE).
