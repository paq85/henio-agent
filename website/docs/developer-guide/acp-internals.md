...
The ACP adapter wraps Henio' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
henio acp / henio-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> load ~/.henio/.env
  -> configure stderr logging
  -> construct HenioACPAgent
  -> acp.run_agent(agent)
```
...
- `allow_once` -> Henio `once`
- `allow_always` -> Henio `always`
- reject options -> Henio `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Henio tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["henio-acp"])
...
Instead it reuses Henio' runtime resolver:

- `acp_adapter/auth.py`
- `henio_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Henio provider/credentials.
...
- `toolsets.py` — `henio-acp` toolset definition
- `henio_cli/main.py` — `henio acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `henio-acp` script
