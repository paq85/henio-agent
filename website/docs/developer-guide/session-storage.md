...
Henio Agent uses a SQLite database (`~/.henio/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `henio_state.py`


## Architecture Overview

```
~/.henio/state.db (SQLite, WAL mode)
...
Multiple henio processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.henio/state.db
...
Default path: `~/.henio/state.db`

This is derived from `henio_constants.get_henio_home()` which resolves to
`~/.henio/` by default, or the value of `HENIO_HOME` environment variable.
