...
description: "Building evaluation environments, running benchmarks, and generating SFT data with the Henio-Agent environment framework"
---

# Environments, Benchmarks & Data Generation

Henio Agent includes a full environment framework that connects its tool-calling capabilities to the [Atropos](https://github.com/NousResearch/atropos) evaluation and data-generation stack. This enables three workflows:

1. **Environment evaluation** — Run agentic tasks against reproducible benchmarks
...
:::info Repo environments vs agent features
The Python environment framework documented here lives under the repo's `environments/` directory and is the implementation-level API for Henio/Atropos integration. It is separate from the end-user Henio toolsets and CLI workflows.
:::

:::tip Quick Links
- **Want to run benchmarks?** Jump to [Available Benchmarks](#available-benchmarks)
- **Want to run environments manually?** Jump to [Running Environments](#running-environments)
...
classDiagram
    ...
    BaseEnv <|-- HenioAgentBaseEnv
    HenioAgentBaseEnv <|-- TerminalTestEnv
    HenioAgentBaseEnv <|-- HenioSweEnv
    HenioAgentBaseEnv <|-- TerminalBench2EvalEnv
...
The henio-agent layer (`environments/henio_base_env.py`). Adds:
- **Terminal backend configuration** — sets `TERMINAL_ENV` for sandboxed execution (local, Docker, Modal, Daytona, SSH, Singularity)
- **Tool resolution** — `_resolve_tools_for_group()` calls henio-agent's `get_tool_definitions()` to get the right tool schemas based on enabled/disabled toolsets
- **Agent loop integration** — `collect_trajectory()` runs `HenioAgentLoop` and scores the result
...
Your environment inherits from `HenioAgentBaseEnv` and implements five methods:
...
`HenioAgentLoop` (`environments/agent_loop.py`) is the reusable multi-turn agent engine. It runs the same tool-calling pattern as henio-agent's main loop:
...
| **Generic** | `call_tool(name, args)` — escape hatch for any henio-agent tool |
...
parser = get_parser("henio")  # or "mistral", "llama3_json", "qwen", "deepseek_v3", etc.
content, tool_calls = parser.parse(raw_model_output)
```

Available parsers: `henio`, `mistral`, `llama3_json`, `qwen`, `qwen3_coder`, `deepseek_v3`, `deepseek_v3_1`, `kimi_k2`, `longcat`, `glm45`, `glm47`.
...
pip install "henio-agent[yc-bench]"
...
# Serve mode (connects to Atropos API for live environment runs)
...
```bash
python environments/henio_swe_env/henio_swe_env.py serve \
...
### `serve` — Connect to Atropos for live runs

Connects the environment to a running Atropos API server (`run-api`). Used during live environment execution and trajectory collection.
...
### Phase 2: VLLM ManagedServer (Full-fidelity runs)
...
- **Use for**: high-fidelity evaluation and trajectory collection with exact tokens/logprobs
- **Real tokens**, masks, and logprobs flow through the pipeline
- Set `tool_call_parser` in config to match your model's format (e.g., `"henio"`, `"qwen"`, `"mistral"`)
...
class MyEnvConfig(HenioAgentEnvConfig):
    my_custom_field: str = "default_value"

class MyEnv(HenioAgentBaseEnv):
    name = "my-env"
...
### HenioAgentEnvConfig Fields

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled_toolsets` | `List[str]` | `None` (all) | Which henio toolsets to enable |
...
| `tool_call_parser` | `str` | `"henio"` | Parser for Phase 2 raw output |
...
env:
  ...
  tokenizer_name: "NousResearch/Henio-3-Llama-3.1-8B"
  use_wandb: true
...
- [Modal](https://modal.com) account and CLI: `pip install "henio-agent[modal]"`
- `MODAL_TOKEN_ID` and `MODAL_TOKEN_SECRET` environment variables

### For YC-Bench

- `pip install "henio-agent[yc-bench]"` (installs the yc-bench CLI + SQLAlchemy)
...
```
environments/
├── henio_base_env.py          # Abstract base class (HenioAgentBaseEnv)
├── agent_loop.py               # Multi-turn agent engine (HenioAgentLoop)
├── tool_context.py             # Per-rollout tool access for reward functions
├── patches.py                  # Async-safety patches for Modal backend
│
├── tool_call_parsers/          # Phase 2 client-side parsers
│   ├── henio_parser.py        # Henio/ChatML <tool_call> format
...
├── henio_swe_env/             # SWE-bench training environment
│
