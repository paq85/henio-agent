...
title: "Git Worktrees"
description: "Run multiple Henio agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Henio Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Henio so each session has a clean, isolated working directory.

## Why Use Worktrees with Henio?

Henio treats the **current working directory** as the project root:

- CLI: the directory where you run `henio` or `henio chat`
...
git worktree add ../repo-feature feature/henio-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/henio-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Henio there:

```bash
cd ../repo-feature

# Start Henio in the worktree
henio
```
...
git worktree add ../repo-experiment-a feature/henio-a
git worktree add ../repo-experiment-b feature/henio-b
```
...
- Works on its own branch (`feature/henio-a` vs `feature/henio-b`).
...
- Henio checkpoint data under `~/.henio/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Henio experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/henio-checkpoints-docs`, `feature/henio-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Henio from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `henio -w` (Automatic Worktree Mode)

Henio has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `henio/henio-<hash>`).
...
```bash
henio -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `henio -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Henio session its own clean checkout.
