...
1. Enable via `henio gateway setup` or environment variables
2. Define routes in `config.yaml` **or** create them dynamically with `henio webhook subscribe`
...
| `deliver` | No | Where to send the response: `github_comment`, `telegram`, `discord`, `slack`, `signal`, `whatsapp`, `matrix`, `mattermost`, `homeassistant`, `email`, `dingtalk`, `feishu`, `wecom`, `weixin`, `bluebubbles`, or `log` (default). |
...
Add the `github-pr` route to your `~/.henio/config.yaml` as shown in the example above.
...
Open a pull request on the repository. The webhook fires, Henio processes the event, and posts a review comment on the PR.
...
In addition to static routes in `config.yaml`, you can create webhook subscriptions dynamically using the `henio webhook` CLI command. This is especially useful when the agent itself needs to set up event-driven triggers.

### Create a subscription

```bash
henio webhook subscribe github-issues \
  --events "issues" \
...
```bash
henio webhook remove github-issues
```

### Test a subscription

```bash
henio webhook test github-issues
henio webhook test github-issues --payload '{"issue": {"number": 42, "title": "Test"}}'
```

### How dynamic subscriptions work

- Subscriptions are stored in `~/.henio/webhook_subscriptions.json`
...
The agent can create subscriptions via the terminal tool when guided by the `webhook-subscriptions` skill. Ask the agent to "set up a webhook for GitHub issues" and it will run the appropriate `henio webhook subscribe` command.
...
- Run the gateway in foreground to see logs: `henio gateway run`
