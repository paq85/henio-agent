...
Connect Henio to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between Henio and the BlueBubbles server
...
Or set environment variables directly in `~/.henio/.env`:
...
When someone messages your iMessage, Henio automatically sends them a pairing code. Approve it with:
```bash
henio pairing approve bluebubbles <CODE>
```
Use `henio pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.henio/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.henio/.env`):
```bash
...
Henio will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → Henio
Henio → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** Henio sends messages via the BlueBubbles REST API.
...
### Chat Addressing
You can address chats by email or phone number — Henio resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `henio logs gateway` for webhook errors (or `henio logs -f` to follow in real-time)
