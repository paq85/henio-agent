...
sidebar_position: 9
title: "Matrix"
description: "Set up Henio Agent as a Matrix bot"
---

# Matrix Setup

Henio Agent integrates with Matrix, the open, federated messaging protocol. Matrix lets you run your own homeserver or use a public one like matrix.org — either way, you keep control of your communications. The bot connects via the `mautrix` Python SDK, processes messages through the Henio Agent pipeline (including tool use, memory, and reasoning), and responds in real time. It supports text, file attachments, images, audio, video, and optional end-to-end encryption (E2EE).

Henio works with any Matrix homeserver — Synapse, Conduit, Dendrite, or matrix.org.

Before setup, here's the part most people want to know: how Henio behaves once it's connected.

## How Henio Behaves

| Context | Behavior |
|---------|----------|
| **DMs** | Henio responds to every message. No `@mention` needed. Each DM has its own session. Set `MATRIX_DM_MENTION_THREADS=true` to start a thread when the bot is `@mentioned` in a DM. |
| **Rooms** | By default, Henio requires an `@mention` to respond. Set `MATRIX_REQUIRE_MENTION=false` or add room IDs to `MATRIX_FREE_RESPONSE_ROOMS` for free-response rooms. Room invites are auto-accepted. |
| **Threads** | Henio supports Matrix threads (MSC3440). If you reply in a thread, Henio keeps the thread context isolated from the main room timeline. Threads where the bot has already participated do not require a mention. |
| **Auto-threading** | By default, Henio auto-creates a thread for each message it responds to in a room. This keeps conversations isolated. Set `MATRIX_AUTO_THREAD=false` to disable. |
| **Shared rooms with multiple users** | By default, Henio isolates session history per user inside the room. Two people talking in the same room do not share one transcript unless you explicitly disable that. |
...
2. Choose a username like `henio` — the full user ID will be `@henio:your-server.org`.

### Option B: Use matrix.org or Another Public Homeserver

1. Go to [Element Web](https://app.element.io) and create a new account.
2. Pick a username for your bot (e.g., `henio-bot`).

### Option C: Use Your Own Account

You can also run Henio as your own user. This means the bot posts as you — useful for personal assistants.

## Step 2: Get an Access Token

Henio needs an access token to authenticate with the homeserver. You have two options:
...
Instead of providing an access token, you can give Henio the bot's user ID and password. Henio will log in automatically on startup. This is simpler but means the password is stored in your `.env` file.

```bash
MATRIX_USER_ID=@henio:your-server.org
MATRIX_PASSWORD=your-password
```

## Step 3: Find Your Matrix User ID

Henio Agent uses your Matrix User ID to control who can interact with the bot. Matrix User IDs follow the format `@username:server`.
...
Add the following to your `~/.henio/.env` file:
...
# MATRIX_USER_ID=@henio:matrix.example.org
...
Optional behavior settings in `~/.henio/config.yaml`:
...
:::tip
You can run `henio gateway` in the background or as a systemd service for persistent operation. See the deployment docs for details.
:::

## End-to-End Encryption (E2EE)

Henio supports Matrix end-to-end encryption, so you can chat with your bot in encrypted rooms.
...
# Or install with henio extras
pip install 'henio-agent[matrix]'
```
...
Add to your `~/.henio/.env`:

```bash
MATRIX_ENCRYPTION=true
```

When E2EE is enabled, Henio:

- Stores encryption keys in `~/.henio/platforms/matrix/store/` (legacy installs: `~/.henio/matrix/store/`)
...
On each startup, if `MATRIX_RECOVERY_KEY` is set, Henio imports cross-signing keys from the homeserver's secure secret storage and signs the current device. This is idempotent and safe to leave enabled permanently.

:::warning
If you delete the `~/.henio/platforms/matrix/store/` directory, the bot loses its encryption keys. You'll need to verify the device again in your Matrix client. Back up this directory if you want to preserve encrypted sessions.
:::
...
If you previously used Henio with `MATRIX_ENCRYPTION=true` and are upgrading to
...
1. **Generate a new access token** to get a fresh device ID. The simplest way:
   ...
   curl -X POST https://your-server/_matrix/client/v3/login \
     ...
     -d '{
       "type": "m.login.password",
       "identifier": {"type": "m.id.user", "user": "@henio:your-server.org"},
       "password": "***",
       "initial_device_display_name": "Henio Agent"
     }'
   ```

   Copy the new `access_token` and update `MATRIX_ACCESS_TOKEN` in `~/.henio/.env`.

2. **Delete old encryption state**:

   ```bash
   rm -f ~/.henio/platforms/matrix/store/crypto.db
   rm -f ~/.henio/platforms/matrix/store/crypto_store.*
   ```

3. **Set your recovery key** (if you use cross-signing — most Element users do). Add to `~/.henio/.env`:
...
a working E2EE setup with a previous version of Henio and are upgrading.
...
**Fix**: The sync loop automatically retries every 5 seconds on error. Check the Henio logs for sync-related warnings. If the bot consistently falls behind, ensure your homeserver has adequate resources.

### Bot is offline

**Cause**: The Henio gateway isn't running, or it failed to connect.

**Fix**: Check that `henio gateway` is running. Look at the terminal output for error messages. Common issues: wrong homeserver URL, expired access token, homeserver unreachable.

### "User not allowed" / Bot ignores you

**Cause**: Your User ID isn't in `MATRIX_ALLOWED_USERS`.

**Fix**: Add your User ID to `MATRIX_ALLOWED_USERS` in `~/.henio/.env` and restart the gateway. Use the full `@user:server` format.
...
For more information on securing your Henio Agent deployment, see the [Security Guide](../security.md).
...
- **Media support**: Henio can send and receive images, audio, video, and file attachments. Media is uploaded to your homeserver using the Matrix content repository API.
