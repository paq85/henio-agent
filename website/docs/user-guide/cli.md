...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Henio Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Henio Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.
...
henio -s henio-agent,opencode
henio chat -s opencode -q "review this branch and suggest cleanup"

# Resume previous sessions
henio --continue             # Resume the most recent CLI session (-c)
henio --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
henio chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
henio -w                         # Interactive mode in worktree
henio -w -q "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/img/docs/cli-layout.svg" alt="Stylized preview of the Henio CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Henio CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
When resuming a previous session (`henio -c` or `henio --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Henio to background (Unix only). Run `fg` in the shell to resume. |
| `Tab` | Accept auto-suggestion (ghost text) or autocomplete slash commands |

## Slash Commands

Type `/` to see the autocomplete dropdown. Henio supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Henio replies |
...
```bash
henio -s henio-agent,opencode
henio chat -s opencode -s webhook-subscriptions
```

Henio loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.henio/skills/` is automatically registered as a slash command. The skill name becomes the command:

```
/henio-agent help me set up a second profile
/opencode review the auth refactor
/webhook-subscriptions create a webhook for new GitHub issues
...
You can also define custom personalities in `~/.henio/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Henio..."
  # Add your own!
```
...
On Unix systems, press **`Ctrl+Z`** to suspend Henio to the background — just like any terminal process. The shell prints a confirmation:

```
Henio Agent has been suspended. Run `fg` to bring Henio Agent back.
```
...
Resume this session with:
  henio --resume 20260225_143052_a1b2c3
...
```bash
henio --continue                          # Resume the most recent CLI session
henio -c                                  # Short form
henio -c "my project"                     # Resume a named session (latest in lineage)
henio --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
henio --resume "refactoring auth"         # Resume by title
henio -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `henio sessions rename <id> <title>` from the command line. Use `henio sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Henio's SQLite state database under `~/.henio/state.db`. The database keeps:
...
Henio immediately confirms the task and gives you back the prompt:
