...
title: "Plugins"
description: "Extend Henio with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

Henio has a plugin system for adding custom tools, hooks, and integrations without modifying core code.

**→ [Build a Henio Plugin](/docs/guides/build-a-henio-plugin)** — step-by-step guide with a complete working example.

## Quick overview

Drop a directory into `~/.henio/plugins/` with a `plugin.yaml` and Python code:

```
~/.henio/plugins/my-plugin/
...
Start Henio — your tools appear alongside built-in tools. The model can call them immediately.

### Minimal working example

Here is a complete plugin that adds a `hello_world` tool and logs every tool call via a hook.

**`~/.henio/plugins/hello-world/plugin.yaml`**
...
**`~/.henio/plugins/hello-world/__init__.py`**

```python
"""Minimal Henio plugin — registers a tool and a hook."""
...
Drop both files into `~/.henio/plugins/hello-world/`, restart Henio, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

Project-local plugins under `./.henio/plugins/` are disabled by default. Enable them only for trusted repositories by setting `HENIO_ENABLE_PROJECT_PLUGINS=true` before starting Henio.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `henio <plugin> <subcommand>` |
| Inject messages | `ctx.inject_message(content, role="user")` — see [Injecting Messages](#injecting-messages) |
| Ship data files | `Path(__file__).parent / "data" / "file.yaml"` |
| Bundle skills | Copy `skill.md` to `~/.henio/skills/` at load time |
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `henio plugins install` |
| Distribute via pip | `[project.entry-points."henio_agent.plugins"]` |

## Plugin discovery

| Source | Path | Use case |
|--------|------|----------|
| User | `~/.henio/plugins/` | Personal plugins |
| Project | `.henio/plugins/` | Project-specific plugins (requires `HENIO_ENABLE_PROJECT_PLUGINS=true`) |
| pip | `henio_agent.plugins` entry_points | Distributed packages |
...
Henio has three kinds of plugins:

| Type | What it does | Selection | Location |
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, CLI commands | Multi-select (enable/disable) | `~/.henio/plugins/` |
...
```bash
henio plugins                  # unified interactive UI
henio plugins list             # table view with enabled/disabled status
henio plugins install user/repo  # install from Git
henio plugins update my-plugin   # pull latest
henio plugins remove my-plugin   # uninstall
henio plugins enable my-plugin   # re-enable a disabled plugin
henio plugins disable my-plugin  # disable without removing
```

### Interactive UI

Running `henio plugins` with no arguments opens a composite interactive screen:
...
See the **[full guide](/docs/guides/build-a-henio-plugin)** for handler contracts, schema format, hook behavior, error handling, and common mistakes.
