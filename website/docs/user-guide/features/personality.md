...
If the file is empty, whitespace-only, or cannot be read, Henio falls back to a built-in default identity ("You are Henio Agent, an intelligent AI assistant..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
