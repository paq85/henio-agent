...
Henio has two hook systems that run custom code at key lifecycle points:

| System | Registered via | Runs in | Use case |
|--------|---------------|---------|----------|
| **[Gateway hooks](#gateway-event-hooks)** | `HOOK.yaml` + `handler.py` in `~/.henio/hooks/` | Gateway only | Logging, alerts, webhooks |
...
Each hook is a directory under `~/.henio/hooks/` containing two files:
...
LOG_FILE = Path.home() / ".henio" / "hooks" / "my-hook" / "activity.log"
...
The gateway ships with a built-in `boot-md` hook that looks for `~/.henio/BOOT.md` on every startup. If the file exists, the agent runs its instructions in a background session. No installation needed — just create the file.

**Create `~/.henio/BOOT.md`:**

```markdown
# Startup Checklist

1. Check if any cron jobs failed overnight — run `henio cron list`
...
```yaml
# ~/.henio/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.henio/hooks/long-task-alert/handler.py
import os
import httpx
...
```yaml
# ~/.henio/hooks/command-logger/HOOK.yaml
name: command-logger
...
```python
# ~/.henio/hooks/command-logger/handler.py
import json
from datetime import datetime
from pathlib import Path

LOG = Path.home() / ".henio" / "logs" / "command_usage.jsonl"
...
```yaml
# ~/.henio/hooks/session-webhook/HOOK.yaml
...
```python
# ~/.henio/hooks/session-webhook/handler.py
import httpx

WEBHOOK_URL = "https://your-service.example.com/henio-events"
...
1. On gateway startup, `HookRegistry.discover_and_load()` scans `~/.henio/hooks/`
...
**Where context is injected:** Always the **user message**, never the system prompt. This preserves the prompt cache — the system prompt stays identical across turns, so cached tokens are reused. The system prompt is Henio's territory (model guidance, tool enforcement, personality, skills). Plugins contribute context alongside the user's input.
...
See the **[Build a Plugin guide](/docs/guides/build-a-henio-plugin)** for the full walkthrough including tool schemas, handlers, and advanced hook patterns.
