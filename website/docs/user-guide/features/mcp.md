...
description: "Connect Henio Agent to external tool servers via MCP — and control exactly which MCP tools Henio loads"
---

# MCP (Model Context Protocol)

MCP lets Henio Agent connect to external tool servers so the agent can use tools that live outside Henio itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted Henio to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

## What MCP gives you

- Access to external tool ecosystems without writing a native Henio tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want Henio to see
...
```bash
cd ~/.henio/henio-agent
uv pip install -e ".[mcp]"
```

2. Add an MCP server to `~/.henio/config.yaml`:
...
4. Ask Henio to use the MCP-backed capability.
...
Henio will discover the MCP server's tools and use them like any other tool.
...
HTTP MCP servers are remote endpoints Henio connects to directly.
...
- you do not want Henio spawning a local subprocess for that integration

## Basic configuration reference

Henio reads MCP config from `~/.henio/config.yaml` under `mcp_servers`.
...
| `enabled` | bool | If `false`, Henio skips the server entirely |
...
## How Henio registers MCP tools

Henio prefixes MCP tools so they do not collide with built-in names:
...
In practice, you usually do not need to call the prefixed name manually — Henio sees the tool and chooses it during normal reasoning.

## MCP utility tools

When supported, Henio also registers utility tools around MCP resources and prompts:
...
- Henio only registers resource utilities if the MCP session actually supports resource operations
- Henio only registers prompt utilities if the MCP session actually supports prompt operations

So a server that exposes callable tools but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which tools each MCP server contributes to Henio, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, Henio skips the server completely and does not even attempt a connection.
...
You can also separately disable Henio-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, Henio does not create an empty runtime MCP toolset for that server.
...
Henio discovers MCP servers at startup and registers their tools into the normal tool registry.

### Dynamic Tool Discovery

MCP servers can notify Henio when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When Henio receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
For stdio servers, Henio does not blindly pass your full shell environment.
...
Then verify your config and restart Henio.
...
Because Henio now only registers those wrappers when both are true:
...
MCP servers can request LLM inference from Henio via the `sampling/createMessage` protocol. This allows an MCP server to ask Henio to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
## Running Henio as an MCP server

In addition to connecting **to** MCP servers, Henio can also **be** an MCP server. This lets other MCP-capable agents (Claude Code, Cursor, Codex, or any MCP client) use Henio's messaging capabilities — list conversations, read message history, and send messages across all your connected platforms.

### When to use this

- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through Henio
- You want a single MCP server that bridges to all of Henio's connected messaging platforms at once
- You already have a running Henio gateway with connected platforms
...
Add Henio to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed Henio in a specific location:

```json
{
  "mcpServers": {
    "henio": {
      "command": "/home/user/.henio/henio-agent/venv/bin/henio",
      "args": ["mcp", "serve"]
    }
  }
}
...
The MCP server includes a live event bridge that polls Henio's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
...
```bash
henio mcp serve              # Normal mode
henio mcp serve --verbose    # Debug logging on stderr
```

### How it works

The MCP server reads conversation data directly from Henio's session store (`~/.henio/sessions/sessions.json` and the SQLite database). A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same `send_message` infrastructure as the Henio agent itself.
