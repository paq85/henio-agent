...
Henio Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
henio memory setup      # interactive picker + configuration
henio memory status     # check what's active
henio memory off        # disable external provider
```

You can also select the active memory provider via `henio plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.henio/config.yaml`:
...
When a memory provider is active, Henio automatically:
...
**Setup Wizard:**
```bash
henio honcho setup        # (legacy command) 
# or
henio memory setup        # select "honcho"
```

**Config:** `$HENIO_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$HENIO_HOME/honcho.json` > `~/.henio/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/henio-ai/henio-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/henio).
...
:::tip Migrating from `henio honcho`
If you previously used `henio honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-agent / Profiles:**

Each Henio profile gets its own Honcho AI peer while sharing the same workspace -- all profiles see the same user representation, but each agent builds its own identity and observations.

```bash
henio profile create coder --clone   # creates honcho peer "coder", inherits config from default
```

What `--clone` does: creates a `henio.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The peer is eagerly created in Honcho so it exists before first message.

For profiles created before Honcho was set up:

```bash
henio honcho sync   # scans all profiles, creates host blocks for any missing ones
```

This inherits settings from the default `henio` host block and creates new AI peers for each profile. Idempotent -- skips profiles that already have a host block.
...
See the [config reference](https://github.com/henio-ai/henio-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/henio).
...
henio memory setup    # select "openviking"
# Or manually:
henio config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.henio/.env
```
...
**Config:** `$HENIO_HOME/mem0.json`

| Key | Default | Description |
|-----|---------|-------------|
| `user_id` | `henio-user` | User identifier |
...
**Setup:**
```bash
henio memory setup    # select "hindsight"
# Or manually:
henio config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.henio/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p henio ui start`

**Config:** `$HENIO_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `henio` | Memory bank identifier |
...
See [plugin README](https://github.com/paq85/henio-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
henio memory setup    # select "holographic"
# Or manually:
henio config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.henio-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$HENIO_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
henio memory setup    # select "retaindb"
# Or manually:
henio config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.henio/.env
```
...
# Then configure Henio
henio memory setup    # select "byterover"
# Or manually:
henio config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$HENIO_HOME/byterover/` (profile-scoped)
...
**Setup:**
```bash
henio memory setup    # select "supermemory"
# Or manually:
henio config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.henio/.env
```

**Config:** `$HENIO_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `henio` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `henio-{identity}` → `henio-coder`) to isolate memories per Henio profile
...
- **Local storage providers** (Holographic, ByteRover) use `$HENIO_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$HENIO_HOME/` so each profile has its own credentials
