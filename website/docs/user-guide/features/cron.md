...
Schedule tasks to run automatically with natural language or cron expressions. Henio exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
:::warning
Cron-run sessions cannot recursively create more cron jobs. Henio disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
henio cron create "every 2h" "Check server status"
henio cron create "every 1h" "Summarize new feed items" --skill blogwatcher
henio cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill find-nearby \
  --name "Skill combo"
```
...
Henio will use the unified `cronjob` tool internally.
...
```bash
henio cron edit <job_id> --schedule "every 4h"
henio cron edit <job_id> --prompt "Use the revised task"
henio cron edit <job_id> --skill blogwatcher --skill find-nearby
henio cron edit <job_id> --add-skill find-nearby
henio cron edit <job_id> --remove-skill blogwatcher
henio cron edit <job_id> --clear-skills
```
...
```bash
henio cron list
henio cron pause <job_id>
henio cron resume <job_id>
henio cron run <job_id>
henio cron remove <job_id>
henio cron status
henio cron tick
```
...
sudo henio gateway install --system   # Linux: boot-time system service for servers
henio gateway             # Or run in foreground
...
1. loads jobs from `~/.henio/cron/jobs.json`
...
A file lock at `~/.henio/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
| `"local"` | Save to local files only (`~/.henio/cron/output/`) | Default on CLI |
...
If the agent's final response starts with `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.henio/cron/output/`), but no message is sent to the delivery target.
...
Or set the `HENIO_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 120s default.
...
The agent's final response is automatically delivered — you do **not** need to include `send_message` in the cron prompt for that same destination. If a cron run calls `send_message` to the exact target the scheduler will already deliver to, Henio skips that duplicate send and tells the model to put the user-facing content in the final response instead. Use `send_message` only for additional or different targets.
...
Jobs are stored in `~/.henio/cron/jobs.json`. Output from job runs is saved to `~/.henio/cron/output/{job_id}/{timestamp}.md`.
