...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, Henio automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
If you already have an API key set in `.env`, Henio auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:

```bash
# Add a second OpenRouter key
henio auth add openrouter --api-key sk-or-v1-your-second-key

# Add a second Anthropic key
henio auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (Claude Code subscription)
henio auth add anthropic --type oauth
...
Run `henio auth` with no subcommand for an interactive wizard:
...
| Command | Description |
|---------|-------------|
| `henio auth` | Interactive pool management wizard |
| `henio auth list` | Show all pools and credentials |
| `henio auth list <provider>` | Show a specific provider's pool |
| `henio auth add <provider>` | Add a credential (prompts for type and key) |
| `henio auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `henio auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `henio auth remove <provider> <index>` | Remove credential by 1-based index |
| `henio auth reset <provider>` | Clear all cooldowns/exhaustion status |

## Rotation Strategies

Configure via `henio auth` → "Set rotation strategy" or in `config.yaml`:
...
When you set up a custom endpoint via `henio model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via henio model:
henio auth list
# Shows:
...
henio auth add Together.ai --api-key sk-together-second-key
```
...
Henio automatically discovers credentials from multiple sources and seeds the pool on startup:
...
| Henio PKCE OAuth | `~/.henio/auth.json` | Yes (Anthropic) |
| Custom endpoint config | `model.api_key` in config.yaml | Yes (custom endpoints) |
| Manual entries | Added via `henio auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `henio auth add`) are never auto-pruned.
...
2. **`henio_cli/auth_commands.py`** — CLI commands and interactive wizard
3. **`henio_cli/runtime_provider.py`** — Pool-aware credential resolution
4. **`run_agent.py`** — Error recovery: 429/402/401 → pool rotation → fallback

## Storage

Pool state is stored in `~/.henio/auth.json` under the `credential_pool` key:
