...
Type `@` followed by a reference to inject content directly into your message. Henio expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Henio env: `$HENIO_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$HENIO_HOME/skills/.hub/`
