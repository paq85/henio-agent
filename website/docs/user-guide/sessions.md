...
Henio Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.henio/state.db`) — structured session metadata with FTS5 full-text search
2. **JSONL transcripts** (`~/.henio/sessions/`) — raw conversation transcripts including tool calls (gateway)
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`henio` or `henio chat`) |
...
henio -c "my project"   # → resumes "my project #3"
```
...
henio --resume 20250305_091523_a1b2c3d4
henio -r 20250305_091523_a1b2c3d4

# Resume by title
henio --resume "refactoring auth"

# Or with the chat subcommand
henio chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `henio sessions list`.

### Conversation Recap on Resume

When you resume a session, Henio displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Henio session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.henio/config.yaml`:
...
Henio automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `henio sessions list` or `henio sessions browse`.
...
```bash
henio sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Henio creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`henio -c "my project"`), it automatically picks the most recent session in the lineage.
...
Henio provides a full set of session management commands via `henio sessions`:
...
# Filter by platform
henio sessions list --source telegram

# Show more sessions
henio sessions list --limit 50
```
...
henio sessions export backup.jsonl

# Export sessions from a specific platform
henio sessions export telegram-history.jsonl --source telegram

# Export a single session
henio sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
henio sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
henio sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
henio sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
henio sessions prune

# Custom age threshold
henio sessions prune --older-than 30

# Only prune sessions from a specific platform
henio sessions prune --source telegram --older-than 60

# Skip confirmation
henio sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`henio insights`](/docs/reference/cli-commands#henio-insights).
...
When Henio cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Henio uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Henio in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.henio/state.db` | All session metadata + messages with FTS5 |
| Gateway transcripts | `~/.henio/sessions/` | JSONL transcripts per session + sessions.json index |
| Gateway index | `~/.henio/sessions/sessions.json` | Maps session keys to active session IDs |
...
henio sessions delete <session_id>
