...
sidebar_position: 2
title: "Configuration"
description: "Configure Henio Agent — config.yaml, providers, models, API keys, and more"
---

# Configuration

All settings are stored in the `~/.henio/` directory for easy access.
...
├── auth.json       # OAuth provider credentials (Henio Portal, etc.)
...
```bash
henio config              # View current configuration
henio config edit         # Open config.yaml in your editor
henio config set KEY VAL  # Set a specific value
henio config check        # Check for missing options (after updates)
henio config migrate      # Interactively add missing options

# Examples:
henio config set model anthropic/claude-opus-4
henio config set terminal.backend docker
henio config set OPENROUTER_API_KEY sk-or-...  # Saves to .env
```

:::tip
The `henio config set` command automatically routes values to the right file — API keys are saved to `.env`, everything else to `config.yaml`.
:::
...
1. **CLI arguments** — e.g., `henio chat --model anthropic/claude-sonnet-4` (per-invocation override)
2. **`~/.henio/config.yaml`** — the primary config file for all non-secret settings
3. **`~/.henio/.env`** — fallback for env vars; **required** for secrets (API keys, tokens, passwords)
...
Henio supports six terminal backends. Each determines where the agent's shell commands actually execute — your local machine, a Docker container, a remote server via SSH, a Modal cloud sandbox, a Daytona workspace, or a Singularity/Apptainer container.
...
For cloud sandboxes such as Modal and Daytona, `container_persistent: true` means Henio will try to preserve filesystem state across sandbox recreation. It does not promise that the same live sandbox, PID space, or background processes will still be running later.
...
:::warning
The agent has the same filesystem access as your user account. Use `henio tools` to disable tools you don't want, or switch to Docker for sandboxing.
:::
...
**Requirements:** Docker Desktop or Docker Engine installed and running. Henio probes `$PATH` plus common macOS install locations (`/usr/local/bin/docker`, `/opt/homebrew/bin/docker`, Docker Desktop app bundle).
...
**Credential forwarding:** Env vars listed in `docker_forward_env` are resolved from your shell environment first, then `~/.henio/.env`. Skills can also declare `required_environment_variables` which are merged automatically.
...
**Persistence:** When enabled, the sandbox filesystem is snapshotted on cleanup and restored on next session. Snapshots are tracked in `~/.henio/modal_snapshots.json`. This preserves filesystem state, not live processes, PID space, or background jobs.

**Credential files:** Automatically mounted from `~/.henio/` (OAuth tokens, etc.) and synced before each command.
...
**Persistence:** When enabled, sandboxes are stopped (not deleted) on cleanup and resumed on next session. Sandbox names follow the pattern `henio-{task_id}`.
...
**Scratch directory:** Resolved in order: `TERMINAL_SCRATCH_DIR` → `TERMINAL_SANDBOX_DIR/singularity` → `/scratch/$USER/henio-agent` (HPC convention) → `~/.henio/sandboxes/singularity`.
...
- **Docker** — Run `docker version` to verify Docker is working. If it fails, fix Docker or `henio config set terminal.backend local`.
- **SSH** — Both `TERMINAL_SSH_HOST` and `TERMINAL_SSH_USER` must be set. Henio logs a clear error if either is missing.
- **Modal** — Needs `MODAL_TOKEN_ID` env var or `~/.modal.toml`. Run `henio doctor` to check.
...
Henio resolves each listed variable from your current shell first, then falls back to `~/.henio/.env` if it was saved with `henio config set`.
...
Docker sandboxes stay isolated by default. Henio does **not** pass your current host working directory into the container unless you explicitly opt in.
...
When enabled:
- if you launch Henio from `~/projects/my-app`, that host directory is bind-mounted to `/workspace`
...
- `true` gives the sandbox direct access to the directory you launched Henio from
...
```bash
henio config set terminal.persistent_shell false
```
...
- `henio config migrate` scans all enabled skills, finds unconfigured settings, and offers to prompt you
- `henio config show` displays all skill settings under "Skill Settings" with the skill they belong to
- When a skill loads, its resolved config values are injected into the skill context automatically

**Setting values manually:**

```bash
henio config set skills.config.wiki.path ~/my-research-wiki
```
...
```yaml
worktree: true    # Always create a worktree (same as henio -w)
...
Henio automatically compresses long conversations to stay within your model's context window. The compression summarizer is a separate LLM call — you can point it at any provider or endpoint.
...
Plugin engines are **never auto-activated** — you must explicitly set `context.engine` to the plugin name. Available engines can be browsed and selected via `henio plugins` → Provider Plugins → Context Engine.
...
|---------|---------|----------------|---------|
| Socket read timeout | 120s | Auto-raised to 1800s | `HENIO_STREAM_READ_TIMEOUT` |
| Stale stream detection | 180s | Auto-disabled | `HENIO_STREAM_STALE_TIMEOUT` |
| API call (non-streaming) | 1800s | Unchanged | `HENIO_API_TIMEOUT` |

The **socket read timeout** controls how long httpx waits for the next chunk of data from the provider. Local LLMs can take minutes for prefill on large contexts before producing the first token, so Henio raises this to 30 minutes when it detects a local endpoint. If you explicitly set `HENIO_STREAM_READ_TIMEOUT`, that value is always used regardless of endpoint detection.
...
Henio uses lightweight "auxiliary" models for side tasks like image analysis, web page summarization, and browser screenshot analysis. By default, these use **Gemini Flash** via auto-detection — you don't need to configure anything.

### The universal config pattern

Every model slot in Henio — auxiliary tasks, compression, fallback — uses the same three knobs:
...
When `base_url` is set, Henio ignores the provider and calls that endpoint directly (using `api_key` or `OPENAI_API_KEY` for auth). When only `provider` is set, Henio uses that provider's built-in auth and base URL.
...
Or via environment variable (in `~/.henio/.env`):
...
| `"nous"` | Force Henio Portal | `henio auth` |
| `"codex"` | Force Codex OAuth (ChatGPT account). Supports vision (gpt-5.3-codex). | `henio model` → Codex |
| `"main"` | Use your active custom/main endpoint. This can come from `OPENAI_BASE_URL` + `OPENAI_API_KEY` or from a custom endpoint saved via `henio model` / `config.yaml`. Works with OpenAI, local models, or any OpenAI-compatible API. **Auxiliary tasks only — not valid for `model.provider`.** | Custom endpoint credentials + base URL |
...
`base_url` takes precedence over `provider`, so this is the most explicit way to route an auxiliary task to a specific endpoint. For direct endpoint overrides, Henio uses the configured `api_key` or falls back to `OPENAI_API_KEY`; it does not reuse `OPENROUTER_API_KEY` for that custom endpoint.
...
`provider: "main"` uses whatever provider Henio uses for normal chat — whether that's a named custom provider (e.g. `beans`), a built-in provider like `openrouter`, or a legacy `OPENAI_BASE_URL` endpoint.
...
:::tip
Run `henio config` to see your current auxiliary model settings. Overrides only show up when they differ from the defaults.
:::
...
Platforms without an override fall back to the global `tool_progress` value. Valid platform keys: `telegram`, `discord`, `slack`, `signal`, `whatsapp`, `matrix`, `mattermost`, `email`, `homeassistant`, `dingtalk`, `feishu`, `wecom`, `weixin`, `bluebubbles`.

`interim_assistant_messages` is gateway-only. When enabled, Henio sends completed mid-turn assistant updates as separate chat messages. This is independent from `tool_progress` and does not require gateway streaming.
...
If the requested provider is unavailable, Henio falls back automatically in this order: `local` → `groq` → `openai`.
...
:::note
Streaming is disabled by default. Enable it in `~/.henio/config.yaml` to try the streaming UX.
:::
...
- `false` reverts to the old shared-room behavior. That can be useful if you explicitly want Henio to treat a channel like one collaborative conversation, but it also means users share context, token costs, and interrupt state.
- Direct messages are unaffected. Henio still keys DMs by chat/DM ID as usual.
...
Control what Henio does when an unknown user sends a direct message:
...
- `pair` is the default. Henio denies access, but replies with a one-time pairing code in DMs.
...
quick_commands:
  status:
    type: exec
    command: systemctl status henio-agent
  disk:
    type: exec
    command: df -h /
  update:
    type: exec
    command: cd ~/.henio/henio-agent && git pull && pip install -e .
  gpu:
    type: exec
...
The `web_search`, `web_extract`, and `web_crawl` tools support four backend providers. Configure the backend in `config.yaml` or via `henio tools`:
...
browser:
  ...
  record_sessions: false         # Auto-record browser sessions as WebM videos to ~/.henio/browser_recordings/
  camofox:
...
Control how Henio handles potentially dangerous commands:
...
| `off` | Skip all approval checks. Equivalent to `HENIO_YOLO_MODE=true`. **Use with caution.** |
...
```yaml
checkpoints:
  enabled: true                  # Enable automatic checkpoints (also: henio --checkpoints)
...
**Direct endpoint override:** If you want the obvious custom-endpoint path, set `delegation.base_url`, `delegation.api_key`, and `delegation.model`. That sends subagents directly to that OpenAI-compatible endpoint and takes precedence over `delegation.provider`. If `delegation.api_key` is omitted, Henio falls back to `OPENAI_API_KEY` only.
...
Henio uses two different context scopes:

| File | Purpose | Scope |
|------|---------|-------|
| `SOUL.md` | **Primary agent identity** — defines who the agent is (slot #1 in the system prompt) | `~/.henio/SOUL.md` or `$HENIO_HOME/SOUL.md` |
| `.henio.md` / `HENIO.md` | Project-specific instructions (highest priority) | Walks to git root |
...
- If SOUL.md is missing, empty, or cannot be loaded, Henio falls back to a built-in default identity.
- **Project context files use a priority system** — only ONE type is loaded (first match wins): `.henio.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`. SOUL.md is always loaded independently.
- **AGENTS.md** is hierarchical: if subdirectories also have AGENTS.md, all are combined.
- Henio automatically seeds a default `SOUL.md` if one does not already exist.
...
| Context | Default |
|---------|---------|
| **CLI (`henio`)** | Current directory where you run the command |
| **Messaging gateway** | Home directory `~` (override with `MESSAGING_CWD`) |
| **Docker / Singularity / Modal / SSH** | User's home directory inside the container or remote machine |

Override the working directory:
```bash
# In ~/.henio/.env or ~/.henio/config.yaml:
