...
description: "Complete reference of all environment variables used by Henio Agent"
---

# Environment Variables Reference

All variables go in `~/.henio/.env`. You can also set them with `henio config set VAR value`.
...
| `HENIO_COPILOT_ACP_COMMAND` | Override Copilot ACP CLI binary path (default: `copilot`) |
| `COPILOT_CLI_PATH` | Alias for `HENIO_COPILOT_ACP_COMMAND` |
| `HENIO_COPILOT_ACP_ARGS` | Override Copilot ACP arguments (default: `--acp --stdio`) |
...
| `HENIO_MODEL` | Override model name at process level (used by cron scheduler; prefer `config.yaml` for normal use) |
| `VOICE_TOOLS_OPENAI_KEY` | Preferred OpenAI key for OpenAI speech-to-text and text-to-speech providers |
| `HENIO_LOCAL_STT_COMMAND` | Optional local speech-to-text command template. Supports `{input_path}`, `{output_dir}`, `{language}`, and `{model}` placeholders |
| `HENIO_LOCAL_STT_LANGUAGE` | Default language passed to `HENIO_LOCAL_STT_COMMAND` or auto-detected local `whisper` CLI fallback (default: `en`) |
| `HENIO_HOME` | Override Henio config directory (default: `~/.henio`). Also scopes the gateway PID file and systemd service name, so multiple installations can run concurrently |

## Provider Auth (OAuth)

For native Anthropic auth, Henio prefers Claude Code's own credential files when they exist because those credentials can refresh automatically. Environment variables such as `ANTHROPIC_TOKEN` remain useful as manual overrides, but they are no longer the preferred path for Claude Pro/Max login.
...
| `HENIO_DUMP_REQUESTS` | Dump API request payloads to log files (`true`/`false`) |
| `HENIO_PREFILL_MESSAGES_FILE` | Path to a JSON file of ephemeral prefill messages injected at API-call time |
| `HENIO_TIMEZONE` | IANA timezone override (for example `America/New_York`) |
...
For cloud sandbox backends, persistence is filesystem-oriented. `TERMINAL_LIFETIME_SECONDS` controls when Henio cleans up an idle terminal session, and later resumes may recreate the sandbox rather than keep the same live processes running.
...
| `TERMINAL_SANDBOX_DIR` | Host directory for workspaces and overlays (default: `~/.henio/sandboxes/`) |
...
| `MATRIX_USER_ID` | Matrix user ID (e.g. `@henio:matrix.org`) — required for password login, optional with access token |
...
| `API_SERVER_MODEL_NAME` | Model name advertised on `/v1/models`. Defaults to the profile name (or `henio-agent` for the default profile). Useful for multi-user setups where frontends like Open WebUI need distinct model names per connection. |
...
| Variable | Description |
|----------|-------------|
| `HENIO_MAX_ITERATIONS` | Max tool-calling iterations per conversation (default: 90) |
| `HENIO_TOOL_PROGRESS` | Deprecated compatibility variable for tool progress display. Prefer `display.tool_progress` in `config.yaml`. |
| `HENIO_TOOL_PROGRESS_MODE` | Deprecated compatibility variable for tool progress mode. Prefer `display.tool_progress` in `config.yaml`. |
| `HENIO_HUMAN_DELAY_MODE` | Response pacing: `off`/`natural`/`custom` |
| `HENIO_HUMAN_DELAY_MIN_MS` | Custom delay range minimum (ms) |
| `HENIO_HUMAN_DELAY_MAX_MS` | Custom delay range maximum (ms) |
| `HENIO_QUIET` | Suppress non-essential output (`true`/`false`) |
| `HENIO_API_TIMEOUT` | LLM API call timeout in seconds (default: `1800`) |
| `HENIO_STREAM_READ_TIMEOUT` | Streaming socket read timeout in seconds (default: `120`). Auto-increased to `HENIO_API_TIMEOUT` for local providers. Increase if local LLMs time out during long code generation. |
| `HENIO_STREAM_STALE_TIMEOUT` | Stale stream detection timeout in seconds (default: `180`). Auto-disabled for local providers. Triggers connection kill if no chunks arrive within this window. |
| `HENIO_EXEC_ASK` | Enable execution approval prompts in gateway mode (`true`/`false`) |
| `HENIO_ENABLE_PROJECT_PLUGINS` | Enable auto-discovery of repo-local plugins from `./.henio/plugins/` (`true`/`false`, default: `false`) |
| `HENIO_BACKGROUND_NOTIFICATIONS` | Background process notification mode in gateway: `all` (default), `result`, `error`, `off` |
| `HENIO_EPHEMERAL_SYSTEM_PROMPT` | Ephemeral system prompt injected at API-call time (never persisted to sessions) |

## Cron Scheduler

| Variable | Description |
|----------|-------------|
| `HENIO_CRON_TIMEOUT` | Inactivity timeout for cron job agent runs in seconds (default: `600`). The agent can run indefinitely while actively calling tools or receiving stream tokens — this only triggers when idle. Set to `0` for unlimited. |
| `HENIO_CRON_SCRIPT_TIMEOUT` | Timeout for pre-run scripts attached to cron jobs in seconds (default: `120`). Override for scripts that need longer execution (e.g., randomized delays for anti-bot timing). Also configurable via `cron.script_timeout_seconds` in `config.yaml`. |
...
For task-specific direct endpoints, Henio uses the task's configured API key or `OPENAI_API_KEY`. It does not reuse `OPENROUTER_API_KEY` for those custom endpoints.
...
These go in `~/.henio/config.yaml` under the `provider_routing` section:
...
:::tip
Use `henio config set` to set environment variables — it automatically saves them to the right file (`.env` for secrets, `config.yaml` for everything else).
:::
