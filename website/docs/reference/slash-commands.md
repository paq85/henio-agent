...
Henio has two slash-command surfaces, both driven by a central `COMMAND_REGISTRY` in `henio_cli/commands.py`:
...
Installed skills are also exposed as dynamic slash commands on both surfaces. Fresh installs seed a small bundled core, and any additional skills you install appear here automatically. Workflow commands such as `/plan` are built into Henio rather than coming from that seeded catalog.
...
| `/plan [request]` | Run plan mode to write a markdown plan instead of executing the work. Plans are saved under `.henio/plans/` relative to the active workspace/backend working directory. |
...
| Command | Description |
|---------|-------------|
| `/<skill-name>` | Load any installed skill as an on-demand command. Example: `/henio-agent`, `/opencode`, `/webhook-subscriptions`. |
...
User-defined quick commands map a short alias to a longer prompt. Configure them in `~/.henio/config.yaml`:
