...
description: "Official optional skills shipped with henio-agent — install via henio skills install official/<category>/<skill>"
---

# Optional Skills Catalog

Official optional skills ship with the henio-agent repository under `optional-skills/` but are **not active by default**. Install them explicitly:

```bash
henio skills install official/<category>/<skill>
```

For example:

```bash
henio skills install official/blockchain/solana
henio skills install official/mlops/flash-attention
```
...
```bash
henio skills uninstall <skill-name>
```
...
| **honcho** | Configure and use Honcho memory with Henio — cross-session user modeling, multi-profile peer isolation, observation config, and dialectic reasoning. |
...
| Skill | Description |
|-------|-------------|
| **blender-mcp** | Control Blender directly from Henio via socket connection to the blender-mcp addon. Create 3D objects, materials, animations, and run arbitrary Blender Python (bpy) code. |
...
| **henio-atropos-environments** | Build, test, and debug Henio Agent RL environments for Atropos training. Covers the HenioAgentBaseEnv interface, reward functions, agent loop integration, and evaluation. |
