...
sidebar_position: 2
title: "Installation"
description: "Install Henio Agent on Linux, macOS, WSL2, or Android via Termux"
---

# Installation

Get Henio Agent up and running in under two minutes with the one-line installer, or follow the manual steps for full control.
...
Henio now ships a Termux-aware installer path too:
...
:::warning Windows
Native Windows is **not supported**. Please install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and run Henio Agent from there. The install command above works inside WSL2.
:::

### What the Installer Does

The installer handles everything automatically — all dependencies (Python, Node.js, ripgrep, ffmpeg), the repo clone, virtual environment, global `henio` command setup, and LLM provider configuration. By the end, you're ready to chat.
...
henio gateway setup  # Set up messaging platforms
henio config set     # Set individual config values
henio setup          # Or run the full setup wizard to configure everything at once
```
...
:::tip
You do **not** need to activate the venv to use `henio`. The entry point has a hardcoded shebang pointing to the venv Python, so it works globally once symlinked.
:::
...
mkdir -p ~/.henio/{cron,sessions,logs,memories,skills,pairing,hooks,image_cache,audio_cache,whatsapp/session}

# Copy the example config file
cp cli-config.yaml.example ~/.henio/config.yaml
...
Open `~/.henio/.env` and add at minimum an LLM provider key:
...
```bash
henio config set OPENROUTER_API_KEY sk-or-v1-your-key-here
```

### Step 7: Add `henio` to Your PATH

```bash
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/henio" ~/.local/bin/henio
```
...
### Step 8: Configure Your Provider

```bash
henio model       # Select your LLM provider and model
```

### Step 9: Verify the Installation

```bash
henio version    # Check that the command is available
henio doctor     # Run diagnostics to verify everything is working
henio status     # Check your configuration
henio chat -q "Hello! What tools do you have available?"
```
...
cp cli-config.yaml.example ~/.henio/config.yaml
touch ~/.henio/.env
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.henio/.env

# Make henio available globally
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/henio" ~/.local/bin/henio
...
| Problem | Solution |
|---------|----------|
| `henio: command not found` | Reload your shell (`source ~/.bashrc`) or check PATH |
| `API key not set` | Run `henio model` to configure your provider, or `henio config set OPENROUTER_API_KEY your_key` |
| Missing config after update | Run `henio config check` then `henio config migrate` |

For more diagnostics, run `henio doctor` — it will tell you exactly what's missing and how to fix it.
