...
description: "Run Henio Agent directly on an Android phone with Termux"
---

# Henio on Android with Termux

This is the tested path for running Henio Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Henio from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Henio now ships a Termux-aware installer path:
...
- links `henio` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `henio` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/henio" "$PREFIX/bin/henio"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `henio` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.henio/.env`.
...
### `henio doctor` says ripgrep or Node is missing
