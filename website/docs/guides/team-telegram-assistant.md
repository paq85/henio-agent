...
This tutorial walks you through setting up a Telegram bot powered by Henio Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Henio Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.henio/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Henio itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Henio Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_henio_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Henio Agent. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Henio Gateway...
...
# View live logs
journalctl --user -u henio-gateway -f
...
henio gateway start
henio gateway stop
tail -f ~/.henio/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `henio gateway install` to update the plist.
:::
...
```bash
henio gateway stop && henio gateway start
```
...
henio pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.henio/.env`:
...
Control how much detail the bot shows when using tools. In `~/.henio/config.yaml`:
...
Customize how the bot communicates by editing `~/.henio/SOUL.md`:

For a full guide, see [Use SOUL.md with Henio](/docs/guides/use-soul-with-henio).
...
```bash
# From the CLI
henio cron list          # View all scheduled jobs
henio cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u henio-gateway` (Linux) or `~/.henio/logs/gateway.log` (macOS) |
| Cron job output | `~/.henio/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.henio/cron/jobs.json` |
| Pairing data | `~/.henio/pairing/` |
| Session history | `~/.henio/sessions/` |
