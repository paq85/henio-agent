---
sidebar_position: 7
title: "Use SOUL.md with Henio"
description: "How to use SOUL.md to shape Henio Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Henio

`SOUL.md` is the **primary identity** for your Henio instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Henio to feel like the same assistant every time you talk to it — or if you want to replace the Henio persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Henio should be
- what Henio should avoid stylistically
- how Henio should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Henio is and how Henio speaks
...
Henio now uses only the global SOUL file for the current instance:

```text
~/.henio/SOUL.md
```

If you run Henio with a custom home directory, it becomes:
...
Henio automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Henio does not overwrite it
- if the file exists but is empty, Henio adds nothing from it to the prompt

## How Henio uses it

When Henio starts a session, it reads `SOUL.md` from `HENIO_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Henio feels.
...
Henio already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Henio should sound.

# Avoid
What Henio should not do.

# Defaults
How Henio should behave when ambiguity appears.
```
...
Then restart Henio or start a new session.
...
### I edited SOUL.md but Henio still sounds the same

Check:
- you edited `~/.henio/SOUL.md` or `$HENIO_HOME/SOUL.md`
...
### Henio is ignoring parts of my SOUL.md
