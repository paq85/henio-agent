---
sidebar_position: 6
title: "Use MCP with Henio"
description: "A practical guide to connecting MCP servers to Henio Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Henio

This guide shows how to actually use MCP with Henio Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Henio tool
- you want Henio to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Henio to internal APIs, databases, or company systems without modifying Henio core

Do not use MCP when:
- a built-in Henio tool already solves the job well
...
- Henio remains the agent
- MCP servers contribute tools
- Henio discovers those tools at startup or reload time
...
If you installed Henio with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Henio banner/status should show MCP integration when configured
- ask Henio what tools it has available
...
There are two categories of MCP-exposed functionality in Henio:
...
2. Henio-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Henio will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Henio to reason over a bounded workspace.
...
Now Henio can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Henio core.
...
Because Henio now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
