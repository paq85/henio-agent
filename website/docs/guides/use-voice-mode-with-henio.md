---
sidebar_position: 8
title: "Use Voice Mode with Henio"
description: "A practical guide to setting up and using Henio voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Henio
...
- you want Henio sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Henio.
...
## Step 1: make sure normal Henio works first
...
Henio supports both local and cloud speech stacks.
...
### If you use `henio setup`

If you choose NeuTTS in the setup wizard, Henio checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Henio transcribes and responds
...
- asking Henio to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Henio loop.

## Tuning CLI behavior

### Silence threshold

If Henio starts/stops too aggressively, tune:
...
Henio stays a normal chat bot, but can speak replies.
...
- you want Henio to function like a portable research or ops assistant
...
Henio joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Henio detects speech boundaries
- transcripts are posted in the associated text channel
- Henio responds in text and audio
...
1. get text Henio working
2. install `henio-agent[voice]`
