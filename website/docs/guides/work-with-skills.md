...
description: "Find, install, use, and create skills — on-demand knowledge that teaches Henio new workflows"
---

# Working with Skills

Skills are on-demand knowledge documents that teach Henio how to handle specific tasks — from configuring Henio itself to wiring up webhook automations. This guide walks you through using them day to day.
...
Every fresh Henio installation seeds a small bundled core. See what's currently installed:
...
```
henio-agent           Complete guide to using and extending Henio Agent...
opencode              Delegate coding tasks to OpenCode CLI agent...
webhook-subscriptions Create and manage webhook subscriptions...
```

Fresh installs are intentionally minimal. Install optional or community skills from the Hub when you need a broader catalog.
...
/henio-agent help me set up a second profile and gateway
/opencode review this branch and suggest cleanup
...
You can also trigger skills through natural conversation — ask Henio to use a specific skill, and it will load it via the `skill_view` tool.
...
Official optional skills ship with Henio but aren't active by default. Install them explicitly:
...
What happens:
1. The skill directory is copied to `~/.henio/skills/`
...
```bash
# Check it's there
henio skills list | grep 1password

# Or in chat
/skills search 1password
```
...
When a skill with config is first loaded, Henio prompts you for the values. They're stored in `config.yaml` under `skills.config.*`.

Manage skill config from the CLI:

```bash
# Interactive config for an installed skill that declares config
henio skills config <installed-skill-name>

# View all skill config
henio config get skills.config
```
...
```bash
mkdir -p ~/.henio/skills/my-category/my-skill
```

### 2. Write SKILL.md

```markdown title="~/.henio/skills/my-category/my-skill/SKILL.md"
---
name: my-skill
...
```bash
henio chat -q "/my-skill help me with the thing"
```

The skill appears automatically — no registration needed. Drop it in `~/.henio/skills/` and it's live.

:::info
The agent can also create and update skills itself using `skill_manage`. After solving a complex problem, Henio may offer to save the approach as a skill for next time.
:::
...
**Let the agent create skills.** After a complex multi-step task, Henio will often offer to save the approach as a skill. Say yes — these agent-authored skills capture the exact workflow including pitfalls that were discovered along the way.

**Use categories.** Organize skills into subdirectories (`~/.henio/skills/devops/`, `~/.henio/skills/research/`, etc.). This keeps the list manageable and helps the agent find relevant skills faster.

**Update skills when they go stale.** If you use a skill and hit issues not covered by it, tell Henio to update the skill with what you learned. Skills that aren't maintained become liabilities.
