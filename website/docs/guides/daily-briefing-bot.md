...
2. **Henio spins up** a fresh agent session with your prompt
...
- **Henio Agent installed** — see the [Installation guide](/docs/getting-started/installation)
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.henio/cron/output/` and you can read them anytime.
:::
...
Henio will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Henio Agent
```
...
Henio will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Henio to delegate each topic to a sub-agent:
...
Henio will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
