...
description: "Diagnose and fix common Henio cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`henio gateway` or `henio serve`). For one-off debugging, you can manually trigger a tick with `henio cron tick`.
...
```bash
date
henio cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.henio/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.henio/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.henio/.env` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `henio cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `henio skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `henio skills list`.
...
2. `~/.henio/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `henio cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Henio config directory). Verify:
```bash
ls ~/.henio/scripts/your-script.py   # Must exist
henio cron edit <job_id> --script ~/.henio/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `henio skills install <skill-name>`.
...
Jobs are stored in `~/.henio/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.henio/cron/jobs.json
chmod 600 ~/.henio/cron/jobs.json   # Your user should own it
```
...
```bash
henio cron list                    # Show all jobs, states, next_run times
henio cron run <job_id>            # Schedule for next tick (for testing)
henio cron edit <job_id>           # Fix configuration issues
henio logs                         # View recent Henio logs
henio skills list                  # Verify installed skills
```
...
1. Run the job with `henio cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.henio/logs/agent.log` for scheduler messages and `~/.henio/logs/errors.log` for warnings
3. Open an issue at [github.com/paq85/henio-agent](https://github.com/paq85/henio-agent) with:
