"""Centralized logging setup for Henio Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.henio/logs/`` (profile-aware via ``get_henio_home()``).
...
from henio_constants import get_config_path, get_henio_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_henio_session_injector", False):
    ...
    _session_record_factory._henio_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``henio logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway",),
...
def setup_logging(
    *,
    ...
    """Configure the Henio logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    henio_home
        Override for the Henio home directory.  Falls back to
        ``get_henio_home()`` (profile-aware).
    log_level
    ...
    if _logging_initialized and not force:
        home = henio_home or get_henio_home()
        return home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_henio_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._henio_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the henio group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from henio_cli.config import is_managed
