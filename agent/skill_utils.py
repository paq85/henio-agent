...
from henio_constants import get_config_path, get_skills_dir
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``HENIO_PLATFORM`` or
            ``HENIO_SESSION_PLATFORM`` env vars.  Falls back to the
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.henio/skills/`` are silently skipped.
    """
    config_path = get_config_path()
    if not config_path.exists():
        return []
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.henio/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    henio = metadata.get("henio") or metadata.get("henio") or {}
    if not isinstance(henio, dict):
        henio = {}
    return {
        "fallback_for_toolsets": henio.get("fallback_for_toolsets", []),
        "requires_toolsets": henio.get("requires_toolsets", []),
        "fallback_for_tools": henio.get("fallback_for_tools", []),
        "requires_tools": henio.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    henio = metadata.get("henio") or metadata.get("henio")
    if not isinstance(henio, dict):
        return []
    raw = henio.get("config")
    if not raw:
        return []
