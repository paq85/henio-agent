...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.henio/models_dev_cache.json)
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Henio ↔ models.dev
# ---------------------------------------------------------------------------

# Henio provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → Henio (built lazily)
_MODELS_DEV_TO_PROVIDER: Optional[Dict[str, str]] = None


def _get_reverse_mapping() -> Dict[str, str]:
    """Return models.dev ID → Henio provider ID mapping."""
...
def _get_cache_path() -> Path:
    ...
    return get_henio_home() / "models_dev_cache.json"
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a Henio provider ID to its models dict from models.dev.
...
def search_models_dev(
    ...
    Args:
        ...
        provider: Optional Henio provider ID to restrict search scope.
    ...
    if provider is not None:
        ...
        for henio_prov, mdev_prov in PROVIDER_TO_MODELS_DEV.items():
            provider_data = data.get(mdev_prov, {})
            if isinstance(provider_data, dict):
                models = provider_data.get("models", {})
                if isinstance(models, dict):
                    for mid, mdata in models.items():
                        candidates.append((henio_prov, mid, mdata))
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a Henio provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve Henio ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Henio or models.dev provider ID.  Tries exact match then
