...
Resolution order for text tasks (auto mode):
  1. OpenRouter  (OPENROUTER_API_KEY)
    2. Henio Portal (~/.henio/auth.json active provider)
...
def _read_named_custom_provider(provider: Optional[str]) -> Optional[Dict[str, str]]:
    """Resolve a named custom provider directly from config.yaml.

    This mirrors the runtime-provider lookup logic but stays local to the
    auxiliary router so named custom providers still work if that helper is not
    importable yet during parallel test/import churn.
    """
    requested_norm = _normalize_custom_provider_name(provider or "")
    if not requested_norm or requested_norm in {"auto", "custom"}:
        return None

    if not requested_norm.startswith("custom:"):
        try:
            from henio_cli.auth import AuthError, resolve_provider

            try:
                resolve_provider(requested_norm)
            except AuthError:
                pass
            else:
                return None
    ...
    for entry in custom_providers:
        ...
        if requested_norm not in {name_norm, f"custom:{name_norm}"}:
            continue
...
_OR_HEADERS = {
    "HTTP-Referer": "https://paq85.github.io/henio-agent",
    "X-OpenRouter-Title": "Henio Agent",
    "X-OpenRouter-Categories": "productivity,cli-agent",
}

# Henio Portal extra_body for product attribution.
# Callers should pass this as extra_body in chat.completions.create()
# when the auxiliary client is backed by Henio Portal.
NOUS_EXTRA_BODY = {"tags": ["product=henio-agent"]}

# Set at resolve time — True if the auxiliary client points to Henio Portal
...
_AUTH_JSON_PATH = get_henio_home() / "auth.json"
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.henio/auth.json for an active Nous provider.
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Henio auth store.

    This intentionally reads only the current profile's auth state. Auto
    auxiliary routing should stay deterministic and must not silently import
    ambient machine credentials (for example from a shared ``~/.codex`` login)
    when the active Henio profile has no Codex auth configured.
    """

    try:
        from henio_cli.auth import _read_codex_tokens
...
def _try_codex_auto() -> Tuple[Optional[Any], Optional[str]]:
    """Try Codex only when the current profile explicitly opted into it."""
    if _read_codex_access_token():
        return _try_codex()
    try:
        from henio_cli.auth import is_provider_explicitly_configured
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from henio_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if pool_present:
            ...
            if "api.kimi.com" in base_url.lower():
                ...
                from henio_cli.models import copilot_default_headers
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    logger.debug("Auxiliary client: Henio Portal")
    ...
    try:
        from henio_cli.models import check_nous_free_tier
...
def _resolve_custom_runtime() -> Tuple[Optional[str], Optional[str], Optional[str]]:
    ...
    """
    try:
        from henio_cli.runtime_provider import resolve_runtime_provider
...
def _try_codex() -> Tuple[Optional[Any], Optional[str]]:
    ...
    allow_pool_lookup = bool(codex_token)
    if not allow_pool_lookup:
        try:
            from henio_cli.auth import is_provider_explicitly_configured

            allow_pool_lookup = is_provider_explicitly_configured("openai-codex")
        except Exception:
            allow_pool_lookup = False
    if not allow_pool_lookup:
        # Unit tests patch load_pool directly to inject synthetic pool entries.
        # Allow that path without consulting the real machine-level pool.
        allow_pool_lookup = getattr(load_pool, "__module__", "") != "agent.credential_pool"

    pool_present, entry = _select_pool_entry("openai-codex") if allow_pool_lookup else (False, None)
    if pool_present:
        pool_token = _pool_runtime_api_key(entry)
        if pool_token:
            codex_token = pool_token
...
def _try_anthropic() -> Tuple[Optional[Any], Optional[str]]:
    ...
    if not is_oauth and token and not token.startswith("sk-ant-api"):
        # Auxiliary flows historically accepted a few Henio/Claude-Code token
        # shapes that don't match Anthropic's strict prefix heuristics but do
        # behave like OAuth credentials for routing purposes.
        if pool_present:
            is_oauth = True
        elif token == os.getenv("CLAUDE_CODE_OAUTH_TOKEN", "").strip():
            is_oauth = True
        elif token.startswith(("cc-", "henio-")):
            is_oauth = True
...
def _resolve_auto(main_runtime: Optional[Dict[str, Any]] = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    #    scenario where a user switches providers via `henio model` but the
    #    old OPENAI_BASE_URL lingers in ~/.henio/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: henio model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.henio/.env",
...
def _normalize_resolved_model(model_name: Optional[str], provider: str) -> Optional[str]:
    ...
    if not model_name:
        return model_name
    try:
        from henio_cli.model_normalize import normalize_model_for_provider
...
def resolve_provider_client(
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Henio Portal not configured (run: henio auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model) if async_mode
                else (client, final_model))
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: henio model)")
                return None, None
    ...
    custom_entry = None
    try:
        from henio_cli.runtime_provider import _get_named_custom_provider

        custom_entry = _get_named_custom_provider(provider)
    except Exception as exc:
        logger.debug(
            "resolve_provider_client: runtime-provider custom lookup unavailable for %r: %s",
            provider, exc,
        )
    if not custom_entry:
        custom_entry = _read_named_custom_provider(provider)
    if custom_entry:
        ...
        if custom_base:
            ...
            if "api.kimi.com" in custom_base.lower():
                ...
                extra["default_headers"] = copilot_default_headers()
            client = OpenAI(api_key=custom_key, base_url=custom_base, **extra)
            if custom_entry.get("api_mode") == "codex_responses" and not raw_codex:
    ...
    try:
        ...
        logger.debug("henio_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from henio_cli.models import _should_use_copilot_responses_api
...
def resolve_vision_provider_client(
    ...
    *,
    ...
    if requested == "auto":
        ...
        #   3. Henio Portal (known vision-capable default model)
        #   4. Stop
        main_provider = _read_main_provider()
...
def get_vision_auxiliary_client(
    ...
    *,
    ...
    """Compatibility wrapper returning just ``(client, model)`` for vision tasks."""
    _resolved_provider, client, resolved_model = resolve_vision_provider_client(
        provider,
        model,
        base_url=base_url,
        api_key=api_key,
...
def get_auxiliary_extra_body() -> dict:
    ...
    Includes Henio Portal product tags when the auxiliary client is backed
    by Henio Portal. Returns empty dict otherwise.
    """
...
def _build_call_kwargs(
    ...
    if provider == "nous" or auxiliary_is_nous:
        merged_extra.setdefault("tags", []).extend(["product=henio-agent"])
    if merged_extra:
        ...
        if client is None:
            ...
            if _explicit and _explicit not in ("auto", "openrouter", "custom"):
                raise RuntimeError(
                    ...
                    f"variable, or switch to a different provider with `henio model`."
                )
